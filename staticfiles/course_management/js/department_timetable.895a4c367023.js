...
// Configuration
    ...
    function setupEventListeners() {
        ...
        $('#printBtn').click(function() {
            ...
            const fullUrl = pdfUrl + '?' + $.param(params);
            // Let the person choose PDF or Word (shared chooser, core/_export_format_chooser.html).
            if (window.exportWithFormat) {
                window.exportWithFormat(fullUrl, {newTab: false});
            } else {
    ...
    // Show error message
    // NOTE: no longer uses Bootstrap's Alert JS component (this page doesn't load
    // Bootstrap) — the close button just removes its own parent, and auto-dismiss
    // is a plain jQuery fade + remove.
