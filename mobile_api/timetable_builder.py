...
def _visibility_payload(schedule_type):
    """
    {'blocked', 'message', 'linkLabel', 'linkUrl'} for schedule_type
    ('main' or 'exam'), read from mobile_api.models.ScheduleVisibility.
    Folded into both the version hash (below) and the full serialized
    payload (serialize_timetable / exam_serialize_timetable) so toggling
    a schedule blocked/unblocked — or just editing its message/link —
    is itself treated as "the data changed": the app's staleness check
    (syncManager.js comparing versions) picks it up on the very next
    poll, the same way an actual timetable edit would.
    """
    from .models import ScheduleVisibility
    ...
    return {
        ...
        "_updatedAt": row.updated_at.isoformat(),  # version-hash input only, not for the app
    }
...
def compute_version(scope, personal_identity=None):
    ...
    base_str += f":V{_visibility_payload('main')['_updatedAt']}"
...
def serialize_timetable(scope, owner_name, personal_identity=None):
    visibility = _visibility_payload("main")
    if visibility["blocked"]:
        # Blocked: don't bother building the day/entry matrix at all — the
        # app shows the message/link instead of the grid regardless.
        visibility.pop("_updatedAt")
        return {
...
def exam_compute_version(scope):
    ...
    base_str = f"{len(ids)}:{max(ids) if ids else 0}:{sum(ids)}"
    base_str += f":V{_visibility_payload('exam')['_updatedAt']}"
    digest = hashlib.sha256(base_str.encode()).hexdigest()
    return digest[:16]


def exam_serialize_timetable(scope, owner_name):
    visibility = _visibility_payload("exam")
    if visibility["blocked"]:
        visibility.pop("_updatedAt")
        return {
            "owner": {"name": owner_name, "role": scope["role"]},
