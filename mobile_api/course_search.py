...
def search_courses(raw_query):
    ...
    for term in terms:
        ...
        candidates = list(
            ...
            # Concurrent Allocation Sets: don't surface courses from a set
            # that's still a plain DRAFT (not yet submitted anywhere) in
            # student/lecturer-facing search — those can still change.
            # Written as an explicit OR (not .exclude()) so rows with no
            # set at all (pre-upgrade/legacy data, allocation_set IS NULL)
            # are correctly kept rather than silently dropped by SQL's
            # three-valued NULL comparison logic under a plain exclude().
            .filter(
                Q(allocation_set__isnull=True)
                | ~Q(allocation_set__status=AllocationSet.STATUS_DRAFT)
            )
