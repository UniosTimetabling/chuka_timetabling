...
from course_allocation.allocation_scope import (
    resolve_allocation_set_for_request, get_or_default_legacy_set,
)
...
# ---------------------------------------------------------------
# Safe Undo's shared engine only ever scoped by department. That was fine
# before a department could have several AllocationSets open at once, but
# now it means switching to a brand-new set still surfaces (and lets you
# restore) deletes/edits that happened in a completely different set for
# the same department — the identical "silent wrong default" pattern fixed
# elsewhere in the COD panel, just showing up here in Safe Undo too.
def _resolve_log_allocation_set_id(table_name, data):
    if table_name == SpecializationStem._meta.db_table:
        category_id = data.get("category")
        if category_id is None:
            return None
        return (
            SpecializationCategory.objects
            .filter(pk=category_id)
            .values_list("allocation_set_id", flat=True)
            .first()
        )
    return data.get("allocation_set")


def _filter_logs_by_active_allocation_set(logs, request):
    """
    Narrows the already department-scoped log list down to whichever
    AllocationSet is currently active for this COD's session — the same
    set the rest of the COD panel is showing them.
    """
    if request is None:
        return logs
    ...
    active_set = resolve_allocation_set_for_request(request, dept)
    if active_set is None:
        # This department has no AllocationSet concept yet (no legacy set
        # backfilled, none created) — behave exactly as before.
        return logs

    # A snapshot with no allocation_set at all (pre-migration data that
    # somehow wasn't backfilled) is treated as belonging to the legacy set,
    # matching how the rest of the codebase treats untagged rows.
    legacy_set = get_or_default_legacy_set(dept)
    legacy_set_id = legacy_set.id if legacy_set else None

    def _matches(log):
        raw_id = _resolve_log_allocation_set_id(log.table_name, log.before_data or {})
        resolved_id = raw_id if raw_id is not None else legacy_set_id
        return resolved_id == active_set.id

    return [log for log in logs if _matches(log)]
...
config = PageUndoConfig(
    ...
    filter_logs=_filter_logs_by_active_allocation_set,
)
