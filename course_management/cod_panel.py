...
from core.doc_export import HTML
...
from course_allocation.models import (
    ...
    CourseCombinationTemplateProgram, AllocationSet,
    SpecializationStem,
)
...
from course_allocation.section_utils import (
    parse_course_code as _parse_course_code,
    strip_group_suffix as _su_strip_group_suffix,
    section_code as _section_code,
    with_group_letter as _with_group_letter,
    max_sections_for_code as _max_sections_for_code,
    distribute_students as _distribute_students,
)
...
def strip_group_suffix(code: str) -> Tuple[str, Optional[str]]:
    ...
    #   COSC 471(C)  COSC471(AC)  COSC 471-AA (Excel-style multi-letter groups)
    # and, since SECTIONS exist, a trailing section number after the letter
    # ("COSC 471-A2") or after a dash ("COSC 471-2") — the section is dropped
    # here (callers that care use section_utils.parse_course_code).
    return _su_strip_group_suffix(code)
...
def _lettered_code_for_group(code: str, group_letter: str) -> str:
    ...
    """
    return _with_group_letter(code, group_letter)


def _apply_enrollment(qs, number_of_students):
    """
    Set number_of_students on every row of `qs`. A course that has been split
    into numbered SECTIONS keeps its cohort headcount as the TOTAL across its
    sections (distributed evenly), instead of every section claiming the full
    headcount. Returns the list of touched ids.
    """
    rows = list(qs)
    ids = [r.id for r in rows]
    plain = [r.id for r in rows if r.section_number is None]
    if plain:
        CourseAllocation.objects.filter(pk__in=plain).update(number_of_students=number_of_students)
    fams = {}
    for r in rows:
        if r.section_number is not None:
            key = (r.program_course_id, r.allocation_set_id, r.intake, r.is_evening_weekend,
                   r.student_group_id, r.special_intake_group_id)
            fams.setdefault(key, []).append(r)
    for members in fams.values():
        members.sort(key=lambda r: (r.section_number, r.id))
        shares = _distribute_students(number_of_students, len(members))
        for r, n in zip(members, shares):
            CourseAllocation.objects.filter(pk=r.id).update(number_of_students=n)
    return ids
...
# ---------------------------------------------------------------
# Per-student-group ("combination") display expansion, mirroring
# _expand_rows_per_stem below but for the Student Groups panel.
#
# A CourseAllocation can be mapped to MORE than one StudentGroup at once
# (the single `student_group` FK plus the `additional_student_groups` M2M —
# see CourseAllocation.all_mapped_student_groups). Previously the Student
# Groups list/detail/pill-summary only ever looked at the `course_allocations`
# reverse relation, which is the FK side ONLY -- so a course shared across
# Groups A/B/C only ever showed up under whichever group happened to be its
# primary `student_group`, and the other groups' course counts/pill lists
# (and thus "total units at a glance") silently under-counted it.
# This helper returns the full set for ONE group so every mapped group's
# section lists the course, not just the primary one.
# ---------------------------------------------------------------
def _student_group_all_allocations(group, allocation_set=None):
    """
    Every CourseAllocation mapped to `group` -- via the primary
    `student_group` FK OR via the `additional_student_groups` M2M -- so a
    course shared across several groups is returned for EACH of them.
    """
    qs = CourseAllocation.objects.filter(
        Q(student_group=group) | Q(additional_student_groups=group)
    ).distinct()
    if allocation_set is not None:
        qs = qs.filter(allocation_set=allocation_set)
    return qs


def _attach_all_group_allocations(student_groups, allocation_set=None):
    """
    Given an iterable/queryset of StudentGroup rows, fetch every mapped
    CourseAllocation (primary + additional_student_groups, per
    _student_group_all_allocations) for ALL of them in one query, then
    stamp each group with `.all_course_allocations` -- the full per-group
    list the template/pills/count should use instead of the FK-only
    `course_allocations` reverse relation. Avoids one query per group.
    Returns the groups as a list (forces evaluation once).
    """
    ...
    qs = (
        ...
        .filter(Q(student_group_id__in=group_ids) | Q(additional_student_groups__id__in=group_ids))
        .distinct()
        .select_related("lecturer", "program_course")
    ...
    for a in qs:
        for g in a.all_mapped_student_groups:
            if g.id in group_ids:
                by_group[g.id].append(a)

    for g in groups:
        g.all_course_allocations = by_group.get(g.id, [])
    return groups


# ---------------------------------------------------------------
# Per-stem ("combination") display expansion for the /cod/ list.
#
# A CourseAllocation shared by several Specialization Stems used to be
# rendered ONCE (under "Compulsory Units") with a "Shared: A, B" badge, and
# compulsory (no-stem) courses only appeared in their own section.
# The COD wants both listed under EACH stem/combination.
# This is display-only: there is still exactly one CourseAllocation row
# in the DB, so the proxies below never get saved — they just let the
# template's {% regroup ... by specialization_stem %} place the same
# course under every stem it is a member of.
# ---------------------------------------------------------------
class _StemRow:
    """Read-only view of a CourseAllocation as displayed under ONE stem."""

    def __init__(self, alloc, stem, all_stems, is_mirror, is_common=False):
        object.__setattr__(self, "_alloc", alloc)
        # True for a compulsory (no-stem) course repeated under every stem.
        object.__setattr__(self, "is_common", is_common)
        # Section inside the stem's block: 0 = the stem's own courses,
        # 1 = compulsory units repeated from outside the stem,
        # 2 = electives (own or repeated) — always listed last.
        object.__setattr__(self, "section", _stem_section(alloc, is_common))
        object.__setattr__(self, "specialization_stem", stem)
        object.__setattr__(self, "specialization_stem_id", stem.id)
        object.__setattr__(self, "display_stems", all_stems)
        object.__setattr__(self, "other_stems", [s for s in all_stems if s.id != stem.id])
        object.__setattr__(self, "is_shared_stem", len(all_stems) > 1)
        object.__setattr__(self, "is_mirror", is_mirror)

    def __getattr__(self, name):
        # Only reached for attributes not set above -> delegate to the
        # real allocation (course_code, program, lecturer, cg_info, ...).
        if name == "_alloc":
            raise AttributeError(name)
        return getattr(self._alloc, name)

    def __setattr__(self, name, value):
        raise AttributeError("_StemRow is a read-only display proxy")


def _stem_section(alloc, is_common):
    if alloc.is_elective or alloc.selection_group_id:
        return 2
    return 1 if is_common else 0


def _allocation_stems(alloc):
    """Every stem this allocation belongs to (primary FK + M2M), de-duplicated
    and ordered by name so the primary/mirror split is deterministic."""
    stems, seen = [], set()
    if alloc.specialization_stem_id and alloc.specialization_stem is not None:
        stems.append(alloc.specialization_stem)
        seen.add(alloc.specialization_stem.id)
    for st in alloc.specialization_stems.all():
        if st.id not in seen:
            stems.append(st)
            seen.add(st.id)
    stems.sort(key=lambda st: ((st.name or "").casefold(), st.id))
    return stems


def _expand_rows_per_stem(rows):
    """
    Re-shape the /cod/ list so every stem/combination is complete:

      * a course in several stems is listed under EACH of them;
      * a course only linked to a stem through the many-to-many link is
        listed under that stem;
      * within a Program/Year that has at least one stem, every course that is
        NOT mapped to any stem (compulsory units) is listed under EVERY stem of
        that Program/Year, because students of any stem take them. Those rows
        are flagged `is_common` and sorted after the stem's own courses;
      * inside each stem, electives (own or repeated) form their own last
        section (`section` 2).

    Program/Years without stems are passed through untouched. Rows are re-ordered so the
    template's regroup (program -> year -> stem) sees each group contiguously.
    """
    prog_idx, year_idx = {}, {}
    year_stems = {}          # (program_id, year) -> {stem_id: stem}
    info = []                # (pos, alloc, pi, yi, stems, key)
    for pos, a in enumerate(rows):
        pi = prog_idx.setdefault(a.program_id, len(prog_idx))
        year = a.program_course.year if a.program_course_id else None
        key = (a.program_id, year)
        yi = year_idx.setdefault(key, len(year_idx))
        stems = _allocation_stems(a)
        for st in stems:
            year_stems.setdefault(key, {})[st.id] = st
        info.append((pos, a, pi, yi, stems, key))

    keyed = []
    for pos, a, pi, yi, stems, key in info:
        if not stems:
            all_year = sorted(
                year_stems.get(key, {}).values(),
                key=lambda st: ((st.name or "").casefold(), st.id),
            )
            if not all_year:
                keyed.append(((pi, yi, 0, "", 0, 0, pos), a))
                continue
            for i, st in enumerate(all_year):
                row = _StemRow(a, st, all_year, is_mirror=(i > 0), is_common=True)
                keyed.append(((pi, yi, 1, (st.name or "").casefold(), st.id, row.section, pos), row))
        else:
            # The stem's own courses (single or shared) — always wrapped so
            # they carry a `section` (electives are listed last in the stem).
            for i, st in enumerate(stems):
                row = _StemRow(a, st, stems, is_mirror=(i > 0))
                keyed.append(((pi, yi, 1, (st.name or "").casefold(), st.id, row.section, pos), row))
...
# ---------------------------------------------------------------
# Per-student-group display expansion for the /cod/ list -- the group
# equivalent of _expand_rows_per_stem above.
#
# In a Program/Year that has NO specialization stems, courses are instead
# split into lettered Student Groups. A course left untagged
# (student_group=NULL) there is still taken by every group's students --
# it just hasn't been split out yet. That used to render ONCE under a
# "Shared / Not yet split into groups" header, separate from the lettered
# groups' own sections, which under-represented each group's real course
# load. Now it is repeated under EVERY lettered group instead, the same
# way a compulsory (no-stem) course is repeated under every stem.
# Display-only: still exactly one CourseAllocation row in the DB.
# ---------------------------------------------------------------
class _GroupRow:
    """Read-only view of a CourseAllocation as displayed under ONE
    Student Group, for a course left untagged (student_group=NULL)."""

    def __init__(self, alloc, group, all_group_names, is_mirror):
        object.__setattr__(self, "_alloc", alloc)
        object.__setattr__(self, "student_group", group)
        object.__setattr__(self, "student_group_id", group.id)
        # True for a course repeated under every lettered group in a
        # stemless Program/Year (was previously a single "Shared" row).
        object.__setattr__(self, "is_common_group", True)
        object.__setattr__(self, "is_mirror_group", is_mirror)
        object.__setattr__(self, "all_groups_for_display", all_group_names)
    ...
    def __setattr__(self, name, value):
        raise AttributeError("_GroupRow is a read-only display proxy")


def _expand_shared_courses_per_group(rows):
    """
    Post-processes an already stem-expanded /cod/ list (own_dept_regular /
    cross_dept_regular): within a Program/Year that has NO specialization
    stems (a Program/Year is either entirely stemless or entirely
    stem-grouped -- see _expand_rows_per_stem), any course left untagged
    (student_group=NULL) is repeated under EVERY lettered Student Group
    found in that same Program/Year, instead of being listed once under a
    separate "Shared / Not yet split into groups" bucket. A Program/Year
    with no lettered groups yet is left untouched -- there's nothing to
    duplicate into, so the course still shows once (ungrouped), same as
    before.
    """
    out = []

    def _key(r):
        year = r.program_course.year if r.program_course_id else None
        return (r.program_id, year)

    for _key_val, group_iter in itertools.groupby(rows, key=_key):
        bucket = list(group_iter)
        # A Program/Year with any specialization stems is entirely
        # stem-grouped (never a mix) -- leave those buckets untouched here.
        if bucket and getattr(bucket[0], "specialization_stem_id", None) is not None:
            out.extend(bucket)
            continue

        # letter -> representative StudentGroup for this Program/Year.
        # Prefers a same-semester match when a letter recurs across
        # semesters (e.g. a separate "Group A" per semester).
        letters = {}
        for r in bucket:
            if r.student_group_id:
                letter = r.student_group.letter
                sem = r.program_course.semester if r.program_course_id else None
                existing = letters.get(letter)
                if existing is None or (
                    sem is not None and existing.semester != sem
                    and r.student_group.semester == sem
                ):
                    letters[letter] = r.student_group

        if not letters:
            out.extend(bucket)  # nothing split into groups here yet
            continue

        sorted_letters = sorted(letters.items())
        letter_names = [grp.name for _, grp in sorted_letters]

        paired = []  # (sort_key, original_pos, row) -- keeps same-letter
        # rows contiguous for the template's {% regroup by student_group.letter %}
        for pos, r in enumerate(bucket):
            if r.student_group_id:
                paired.append(((r.student_group.letter,), pos, r))
            else:
                for i, (letter, grp) in enumerate(sorted_letters):
                    paired.append((
                        (letter,), pos,
                        _GroupRow(r, grp, letter_names, is_mirror=(i > 0)),
                    ))
        paired.sort(key=lambda t: (t[0], t[1]))
        out.extend(row for _, _, row in paired)
...
def _parse_id_list(raw):
    """
    Accept an id list posted either as JSON (e.g. "[1,2,3]", from
    JS `JSON.stringify`) or as a plain comma-separated string
    (e.g. "1,2,3"), and return a clean list of ints. Used by every bulk
    action (bulk Add Group(s), bulk student-group/stem mapping) so the
    frontend doesn't have to care which form it sends.
    """
    if not raw:
        return []
    raw = raw.strip()
    ...
    try:
        ...
        # A bare JSON number/string still counts as one id.
        return [int(parsed)]
...
class SelectionGroupService:
    ...
    def list_groups(request):
        ...
        _active_set = get_active_allocation_set(request, dept) or get_or_default_legacy_set(dept)
        if _active_set is not None:
            groups = groups.filter(allocation_set=_active_set)
    ...
    def create_group(request):
        ...
        if _active_set is not None:
            elective_courses = elective_courses.filter(allocation_set=_active_set)
        ...
        with transaction.atomic():
            sg = SelectionGroup.objects.create(
                ...
                allocation_set=_active_set,
            )
            if course_ids:
                sg.courses.set(elective_courses)
                sg.sync_courses_into_mapped_stems()
    ...
    def list_elective_allocations(request):
        ...
        _active_set = get_active_allocation_set(request, dept) or get_or_default_legacy_set(dept)
        if _active_set is not None:
            qs = qs.filter(allocation_set=_active_set)
        data = [
            {
                "id": ca.id,
                "course_code": ca.course_code,
...
class SpecialIntakeGroupService:
    ...
    def list_groups(request):
        ...
        # Concurrent Allocation Sets: same class of bug as
        # list_special_intake_allocs — g.course_allocations is every
        # CourseAllocation FK'd to this group regardless of which
        # AllocationSet it's in, so without this filter the group's course
        # list kept showing rows from every concurrent set at once.
        from course_allocation.allocation_scope import get_active_allocation_set, get_or_default_legacy_set
        active_set = get_active_allocation_set(request, dept) or get_or_default_legacy_set(dept)
        ...
        for g in groups:
            ...
            if active_set is not None:
                allocations = allocations.filter(allocation_set=active_set)
            data.append({
                "id": g.id,
    ...
    def pull_courses(request):
        ...
        # Concurrent Allocation Sets: same resolution the main cod_panel
        # view uses — session-active set for this department, falling back
        # to the legacy set so pre-upgrade departments behave unchanged.
        ...
        with transaction.atomic():
            ...
            for pc in program_courses:
                ...
                # Concurrent Allocation Sets: special_intake_group itself is
                # shared across sets by design, so without also filtering by
                # allocation_set here, a course already pulled into this
                # group under a DIFFERENT set would be mistaken for "already
                # here" and silently skipped for the set you're actually
                ...
                allocation = CourseAllocation(
                    ...
                    special_intake_group=group,
                    # Concurrent Allocation Sets: tag with whichever set is
                    # active for this department (falls back to the legacy
                    # set) so a Special Intake pull lands in the same place
                    # as everything else the COD is currently working in,
                    # rather than being left with allocation_set=NULL.
                    allocation_set=active_allocation_set,
                )
    ...
    def pull_courses_normal(request):
        ...
        # Concurrent Allocation Sets: this is the "pull a whole semester in"
        # action the create-allocation wizard's auto_full flow relies on —
        # every row it creates must land in whichever set is currently
        # active for this department.
        active_allocation_set = get_active_allocation_set(request, dept) or get_or_default_legacy_set(dept)
        ...
        with transaction.atomic():
            for pc in program_courses:
                ...
                # AND in the allocation set currently active — without the
                # allocation_set filter, a row that already exists under a
                # DIFFERENT concurrent set would be mistaken for "already
                # pulled" and silently skipped here, leaving the active set
                # with no row for this course at all.
                exists = CourseAllocation.objects.filter(
                    program=program,
                    program_course=pc,
                    department=dept,
    ...
    def get_available_courses_normal(request):
        ...
        # allocation for this program/department IN THE ACTIVE ALLOCATION
        # SET, so the checklist only ever offers courses that haven't been
        # pulled into the set you're currently working on — a course
        # already pulled under a different concurrent set must still show
        # up here as available.
        active_allocation_set = get_active_allocation_set(request, dept) or get_or_default_legacy_set(dept)
    ...
    def get_available_courses(request):
        ...
        # If group_id provided, exclude courses already in the group WITHIN
        # THE ACTIVE ALLOCATION SET — the group itself is shared across
        # sets by design, so without this filter a course already pulled
        # into this group under a different set would wrongly disappear
        # from the picker for the set you're actually working in.
        active_allocation_set = get_active_allocation_set(request, dept) or get_or_default_legacy_set(dept)
        if group_id:
            try:
                group = SpecialIntakeGroup.objects.get(pk=group_id, program=program)
                existing_course_ids = group.course_allocations.filter(
                    allocation_set=active_allocation_set
                ).values_list("program_course_id", flat=True)
                qs = qs.exclude(id__in=existing_course_ids)
...
def _groupable_courses_qs(program, year, semester, intake, allocation_set=None):
    """
    ...
    Concurrent Allocation Sets: pass `allocation_set` (the COD's active/
    legacy set) so this only considers THAT set's rows for the given
    program_course — the StudentGroup itself is shared across sets (see
    StudentGroupService.list_groups), but the actual CourseAllocation row it
    clones/tags for a given course must come from the set the COD is
    currently working in, not some other concurrent set's row for the same
    curriculum course.
    """
    ...
    if allocation_set is not None:
        base = base.filter(allocation_set=allocation_set)
...
def _assign_course_to_group(program, program_course_id, intake, group, allocation_set=None):
    """
    ...
    # Concurrent Allocation Sets: without this filter, a course already
    # tagged to this group under a DIFFERENT set would be mistaken for
    # "already assigned" here and returned as-is — silently skipping the
    # add for the set the COD is actually working in.
    if allocation_set is not None:
        existing_for_group = existing_for_group.filter(allocation_set=allocation_set)
    existing_for_group = existing_for_group.first()
    ...
    # Concurrent Allocation Sets: without this filter, an untagged (shared)
    # row belonging to a DIFFERENT set could get claimed and renamed into
    # THIS group — silently moving another set's course into the set
    # currently active, rather than creating this set's own row.
    if allocation_set is not None:
    ...
    # Prefer a sibling row from the SAME active set (so the cloned row's
    # lecturer/venue reflect what's actually going on in this set); only
    # fall back to a sibling from another set if this set has no rows for
    # this course at all yet, purely to seed sensible defaults.
    template = None
    if allocation_set is not None:
    ...
    if template:
        clone = CourseAllocation.objects.create(
            ...
            # Concurrent Allocation Sets: the clone must land in the set the
            # COD is actively working in — NOT wherever the template row
            # (which may have been borrowed from another set purely to copy
            # field values like lecturer/venue) happens to live. Previously
            # this used template.allocation_set, which meant an "Add
            # Course" done while on Set A could silently create the new row
            # under Set B instead.
    ...
    fresh = CourseAllocation.objects.create(
        ...
        is_evening_weekend=False,
        student_group=group,
        # Concurrent Allocation Sets: falls back to the department's legacy
        # set if the caller didn't have an active one to pass in, so this
        # never leaves allocation_set NULL for a brand-new row.
        allocation_set=allocation_set or get_or_default_legacy_set(program.department),
    )
...
class StudentGroupService:
    ...
    def list_groups(request):
        ...
        # Concurrent Allocation Sets: the STUDENT GROUP itself is shared
        # across allocation sets (e.g. the same "Group A" for a program's
        # Year 1 Semester 1 is reused whether it's this year's Semester 1
        # set or a mixed set that pulls Semester 1 in) — so the group
        # listing below is deliberately NOT filtered by allocation_set.
        # Only which COURSES show up inside each group is scoped, so a COD
        # working in Semester 1 doesn't see Semester 2's courses sitting in
        # "their" Group A.
        ...
        data = []
        for g in groups:
            # Primary `student_group` FK + `additional_student_groups` M2M
            # combined, so a course shared across several groups is listed
            # (and counted) under EVERY group it's mapped to, not just its
            # primary one — matches the per-stem expansion used elsewhere
            # on /cod/ so "total units at a glance" is accurate per group.
            allocations = _student_group_all_allocations(g, _active_set).select_related(
                "lecturer", "program_course"
            )
            data.append({
                "id": g.id,
                "program_id": g.program_id,
    ...
    def create_group(request):
        ...
        with transaction.atomic():
            ...
            _active_set = get_active_allocation_set(request, dept) or get_or_default_legacy_set(dept)
            by_course = _groupable_courses_qs(program, year, semester, intake, allocation_set=_active_set)
            ...
            assigned_base_codes = []
            _active_set_for_group = get_active_allocation_set(request, dept) or get_or_default_legacy_set(dept)
            for pc_id in target_pc_ids:
                alloc, action = _assign_course_to_group(program, pc_id, intake, group, allocation_set=_active_set_for_group)
                if action == "tagged":
                    tagged_count += 1
    ...
    def assign_courses_to_group(request):
        ...
        if program_course_ids:
            ...
            _active_set = get_active_allocation_set(request, dept) or get_or_default_legacy_set(dept)
            by_course = _groupable_courses_qs(group.program, group.year, group.semester, group.intake, allocation_set=_active_set)
            if scope == "all":
                target_pc_ids = list(by_course.keys())
            else:
                selected_pc_ids = set(
        ...
        affected_allocations = []
        assigned_base_codes = []
        _active_set_for_group = get_active_allocation_set(request, group.program.department) or get_or_default_legacy_set(group.program.department)
        for pc_id in target_pc_ids:
            alloc, action = _assign_course_to_group(group.program, pc_id, group.intake, group, allocation_set=_active_set_for_group)
            if action == "tagged":
                tagged_count += 1
                affected_allocations.append(_alloc_row_payload(alloc, dept))
            elif action in ("cloned", "created_new"):
    ...
    def get_group_detail(request):
        ...
        _active_set = get_active_allocation_set(request, dept) or get_or_default_legacy_set(dept)
        # Primary `student_group` FK + `additional_student_groups` M2M
        # combined -- a course shared across several groups is now listed
        # (and counted) here for every group it belongs to.
        allocations = _student_group_all_allocations(group, _active_set).select_related(
            "lecturer", "program_course"
        )
    ...
    def update_group_enrollment(request):
        ...
        skipped_split = []
        if not course_id:
            # A unit that is one of several split units of the same course in
            # this group (EDCI 111-A / EDCI 111-B both mapped here), or that is
            # mapped to several groups, only holds SOME of this group's
            # students — one group headcount can't be stamped on it. Leave those
            # alone; set them per course (course_id) instead.
            rows = list(qs)
            by_base = {}
            for r in rows:
                by_base.setdefault((r.program_course_id, r.allocation_set_id, r.intake), []).append(r)
            multi_ids = set()
            for fam in by_base.values():
                if len({r.course_code for r in fam}) > 1 and not all(r.section_number is not None for r in fam):
                    multi_ids.update(r.id for r in fam)
            for r in rows:
                if r.additional_student_groups.exists():
                    multi_ids.add(r.id)
            if multi_ids:
                skipped_split = sorted(multi_ids)
                qs = qs.exclude(pk__in=multi_ids)
        if course_id:
        ...
        with transaction.atomic():
            ...
            updated_ids = _apply_enrollment(qs, number_of_students)
        ...
        return JsonResponse({
            ...
            "skipped_split_ids": skipped_split,
            "message": (f"Updated enrollment to {number_of_students} for {updated} course(s) in {group.name}."
                        + (f" {len(skipped_split)} split / multi-group unit(s) were left unchanged — "
                           f"set their headcount individually." if skipped_split else "")),
        })
...
class CombinedCourseGroupService:
    ...
    def list_available_for_combination(request):
        ...
        try:
            ...
            # Concurrent Allocation Sets: only offer courses from the COD's
            # currently active allocation as combinable candidates — a
            # Semester 1 session shouldn't surface Semester 2's sections of
            # the same course code as if they were sections to merge.
            _active_set = get_active_allocation_set(request, dept) or get_or_default_legacy_set(dept)
            if _active_set is not None:
                candidate_q &= Q(allocation_set=_active_set)

            candidates = (
                CourseAllocation.objects
                .filter(candidate_q)
                .select_related('department', 'origin_department', 'program', 'lecturer', 'student_group')
                .distinct()
            )
    ...
    def create_combined_group(request):
        ...
        with transaction.atomic():
            ...
            group = CombinedCourseGroup.objects.create(
                ...
                origin_department=dept,
                created_by=request.user,
                # Concurrent Allocation Sets: group_code is now only unique
                # PER SET (see model docstring) — tag it so a COD combining
                # the same course code in a different semester's set doesn't
    ...
    def list_combined_groups(request):
        ...
        # Concurrent Allocation Sets: only show groups from the allocation the
        # COD is currently working in — a Semester 1 session shouldn't list
        # Semester 2's combined groups.
        _active_set = get_active_allocation_set(request, dept) or get_or_default_legacy_set(dept)
        if _active_set is not None:
            groups = groups.filter(allocation_set=_active_set)
        ...
        with transaction.atomic():
            for idx, alloc_ids in enumerate(splits):
                ...
                new_group = CombinedCourseGroup.objects.create(
                    ...
                    # A split-off group belongs alongside the group it split from.
                    allocation_set=group.allocation_set,
                )
    ...
    def update_combined_group_name(request):
        ...
        if CombinedCourseGroup.objects.filter(
            group_code=new_name, allocation_set=group.allocation_set
        ).exclude(pk=group.id).exists():
            return JsonResponse(
                {"status": "error", "message": f'A combined group named "{new_name}" already exists in this allocation.'},
                status=400,
            )
    ...
    def detect_combinable_groups(request):
        ...
        # Concurrent Allocation Sets: only scan the COD's currently active
        # allocation for combinable candidates.
        _active_set = get_active_allocation_set(request, dept) or get_or_default_legacy_set(dept)
        if _active_set is not None:
            allocations = allocations.filter(allocation_set=_active_set)
    ...
    def smart_combine(request):
        ...
        with transaction.atomic():
            ...
            group = CombinedCourseGroup.objects.create(
                ...
                # The combined courses all belong to the same allocation set
                # (candidates are already scoped to it) — inherit it.
                allocation_set=primary.allocation_set,
            )
...
def cod_panel(request):
    ...
    # ── Concurrent Allocation Sets: resolve scope up front ──────────────────
    # Used by both the AJAX action handlers below and the GET-render section
    # further down. `session_active_allocation_set` is only set if the COD
    # explicitly picked/switched to a set THIS SESSION (used to decide
    # whether to redirect to the picker on a plain GET). `active_allocation_set`
    # additionally falls back to the department's legacy set so that AJAX
    # actions from an already-open tab (no picker redirect possible mid-AJAX)
    # keep working against the same data as before this upgrade.
    _dept_for_scope = detect_user_department(request.user)
    session_active_allocation_set = (
        get_active_allocation_set(request, _dept_for_scope) if _dept_for_scope else None
    )
    active_allocation_set = session_active_allocation_set or (
        get_or_default_legacy_set(_dept_for_scope) if _dept_for_scope else None
    )
    ...
    if request.method == "POST" and request.headers.get("x-requested-with") == "XMLHttpRequest":
        ...
        ALLOWED_ACTIONS = {
            ...
            # Sections — split ONE course (in a student group / stem / shared)
            # into numbered parallel classes that may run at the same time
            "get_section_info", "split_allocation_sections", "unsplit_allocation_sections",
            # Bulk Add Group(s) — same, but across MANY selected courses at
            # once, one shared group count applied to every course
            "get_group_clone_info_bulk", "add_allocation_groups_bulk",
            # Map a course-group row to multiple Student Groups / Stems
            "map_allocation_groups_and_stems", "get_allocation_group_mapping",
        ...
        if action == "get_combos_and_groups_for_program":
            ...
            _active_set = get_active_allocation_set(request, dept) or get_or_default_legacy_set(dept)
            if _active_set is not None:
                groups_qs = groups_qs.filter(allocation_set=_active_set)
            if program_id:
        ...
        if action == "get_mapped_lecturers":
            ...
            # Off by default: the lecturer dropdown only shows lecturers
            # already mapped to this course plus the COD's own department.
            # The COD panel form has an explicit "other department" toggle
            # that re-requests this same action with include_other_depts=1
            # to pull the rest in on demand, rather than always shipping
            # every lecturer in the university on every course-code change.
            include_other_depts = request.POST.get("include_other_depts") in ("1", "true", "True")

            # A course hasn't necessarily been picked yet -- the "other
            # department" toggle can be flipped before any course code is
            # entered, purely to widen the dropdown. That's not an error,
            # it just means there's nothing mapped yet.
            mapped_lecturer_ids = []
            ...
            lecturer_qs = Lecturer.objects.all()
            if not include_other_depts:
                # Restrict to: lecturers already mapped to this course
                # (regardless of which department they belong to -- an
                # existing mapping is stronger evidence than department
                # membership) OR lecturers in the COD's own department.
                dept_or_mapped = Q(id__in=mapped_lecturer_ids)
                if user_dept_id:
                    dept_or_mapped |= Q(department_id=user_dept_id)
                lecturer_qs = lecturer_qs.filter(dept_or_mapped)

            all_lecturers = lecturer_qs.annotate(
                is_mapped=Case(
        ...
        if action == "create_allocation":
            ...
            # ------------------------------------------------------------
            # duplication check — scoped EXACTLY like CourseAllocation.clean()'s
            # own uniqueness rule: (program, course_code, intake, student_group).
            # The same course code showing up in a DIFFERENT program is not a
            # duplicate at all (see the auto-allocator fix — a course shared
            # across programs is meant to keep the exact same code, e.g.
            # "COSC 333" in every program, not "COSC 333-A"/"-B"/"-C"), so we
            # never look outside this program here.
            # ------------------------------------------------------------
            qs = CourseAllocation.objects.filter(
                program=program, course_code__iexact=course_code,
                intake=intake_val, student_group=student_group_obj,
            )
            if allocation_id:
            ...
            if qs.exists():
                if student_group_obj:
                    return JsonResponse({
                        "status": "error",
                        "message": f"This course code is already allocated to that program, intake, and student group ({student_group_obj.name}).",
                    }, status=400)
                return JsonResponse({
                    "status": "error",
                    "message": "This course code is already allocated to that program for the same intake.",
                }, status=400)

            # ------------------------------------------------------------
            # DISABLED: cross-program group-letter forcing.
            #
            # This used to search for the base course code across EVERY
            # program (no program= filter on similar_qs) and, if it found
            # a match anywhere in the whole allocation database, refused to
            # save until the COD picked a group letter — which is exactly
            # how the same course ended up stored as "COSC 333-A" in one
            # program, "COSC 333-B" in another, "COSC 333-C" in a third
            # (and, before that, tagged with program initials/codes). That
            # caused a lot of confusion across the university, so a shared
            # course code is now left completely unchanged between
            # programs and this restriction is switched off. Real,
            # same-program duplicates are still caught above (and, for a
            # genuine same-program/same-year re-entry, below we ask which
            # student group the new section belongs to instead of forcing
            # a letter or blocking it outright).
            # ------------------------------------------------------------
            ...
            # ------------------------------------------------------------
            # Same course, same program, same year — likely a second
            # physical section, not a mistake. Rather than forcing a
            # letter onto the course code (see above) or blocking it
            # outright, ask which student group this new section belongs
            # to. Skipped for electives/specialization-stem courses, which
            # can never carry a student group (see CourseAllocation.clean()).
            # ------------------------------------------------------------
            if not allocation_id and not student_group_obj and not is_elective:
                other_group_qs = CourseAllocation.objects.filter(
                    program=program, course_code__iexact=course_code, intake=intake_val,
                )
                if other_group_qs.exists():
                    try:
                        _py = int(program_year) if program_year else None
                        _psem = int(program_semester) if program_semester else None
                    except (ValueError, TypeError):
                        _py = _psem = None
                    ...
                    if _py and _psem:
                        sg_list = list(
                            StudentGroup.objects.filter(
                                program=program, year=_py, semester=_psem, intake=intake_val
                            ).order_by("letter")
                        )

                    return JsonResponse({
                        "status": "need_student_group",
                        "message": (
                            f"'{course_code}' already has an allocation in this program. "
                            "Which student group is this new section for?"
                        ),
                        "existing_groups": [
                            {"id": g.id, "label": g.display_name, "letter": g.letter}
                            for g in sg_list
                        ],
                    }, status=409)
            ...
            if is_elective and program_course_instance and not selection_group_obj:
                ...
                # Concurrent Allocation Sets: scope the lookup AND the tag by
                # the active set, same as the manually-created allocation
                # below — otherwise this could match (and silently reuse) a
                # same-named group that belongs to a different set, or create
                # a new one left unscoped (allocation_set=NULL).
            ...
            with transaction.atomic():
                if allocation_id:
                    ...
                    allocation = CourseAllocation(
                        ...
                        # Concurrent Allocation Sets: a manually-created row
                        # (as opposed to one populated by the auto-allocator
                        # from a semester composition) lands in whichever
                        # AllocationSet the COD currently has active — falls
                        # back to the department's legacy set if somehow
                        # none is active, never left NULL for a new row.
                        allocation_set=active_allocation_set,
                    )
        ...
        # -----------------------
        # SECTIONS — split ONE course into numbered parallel classes
        # (COSC 103 with 400 students -> COSC 103-A, COSC 103-A2 ... for
        # Group A; COSC 103, COSC 103-2 ... for a stem / shared course).
        # Unlike "Add Group(s)" the new rows STAY in the source's cohort:
        # same student group(s), stem(s) and elective pool(s). Sections
        # partition the cohort's students, so every scheduler/panel lets
        # them share a slot; each section still clashes with everything the
        # parent course would clash with.
        # -----------------------
        if action in ("get_section_info", "split_allocation_sections", "unsplit_allocation_sections"):
            dept = detect_user_department(request.user)
            err = _assert_owns_department(request, dept)
            if err:
                return err
            source = get_object_or_404(CourseAllocation, pk=request.POST.get("id"))
            if source.department_id != dept.pk and source.origin_department_id != dept.pk:
                from django.http import Http404
                raise Http404

            if action == "get_section_info":
                fam = list(source.section_family()) if source.section_number else [source]
                return JsonResponse({
                    "status": "success",
                    "course_code": source.course_code,
                    "current_sections": len(fam),
                    "total_students": sum(r.number_of_students or 0 for r in fam),
                    "max_sections": min(20, _max_sections_for_code(source.course_code)),
                    "sections": [
                        {"id": r.id, "course_code": r.course_code,
                         "number_of_students": r.number_of_students or 0,
                         "section_number": r.section_number or 1}
                        for r in fam
                    ],
                })

            if action == "unsplit_allocation_sections":
                fam = list(source.section_family())
                if len(fam) < 2:
                    return JsonResponse({"status": "error", "message": "This course is not split into sections."}, status=400)
                keeper = fam[0]
                with transaction.atomic():
                    total = sum(r.number_of_students or 0 for r in fam)
                    removed = [r.id for r in fam[1:]]
                    CourseAllocation.objects.filter(pk__in=removed).delete()
                    keeper.course_code = _section_code(keeper.course_code, 1)
                    keeper.section_number = None
                    keeper.number_of_students = total
                    keeper.save(update_fields=["course_code", "section_number", "number_of_students"])
                return JsonResponse({
                    "status": "success",
                    "message": f"Merged {len(fam)} sections back into {keeper.course_code}.",
                    "kept": _alloc_row_payload(keeper, dept),
                    "removed_ids": removed,
                })

            # ---- split ----
            try:
                target = int(request.POST.get("num_sections", ""))
            except (TypeError, ValueError):
                return JsonResponse({"status": "error", "message": "Enter a valid number of sections."}, status=400)
            cap = min(20, _max_sections_for_code(source.course_code))
            if target < 2 or target > cap:
                return JsonResponse({"status": "error", "message": f"Choose between 2 and {cap} sections."}, status=400)
            keep_lecturer = request.POST.get("keep_lecturer") in ("1", "true", "on")
            custom = None
            raw_counts = (request.POST.get("counts") or "").strip()
            if raw_counts:
                try:
                    custom = [max(int(x), 0) for x in raw_counts.split(",") if x.strip() != ""]
                except ValueError:
                    return JsonResponse({"status": "error", "message": "Section sizes must be whole numbers."}, status=400)
                if len(custom) != target:
                    return JsonResponse({"status": "error", "message": f"Give exactly {target} section sizes."}, status=400)

            created = []
            with transaction.atomic():
                fam = list(source.section_family()) if source.section_number else [source]
                fam = sorted(fam, key=lambda r: (r.section_number or 1, r.id))
                if len(fam) >= target:
                    return JsonResponse({"status": "error", "message": f"Already split into {len(fam)} section(s)."}, status=400)
                if source.section_number is None:
                    source.section_number = 1
                    source.save(update_fields=["section_number"])
                total = sum(r.number_of_students or 0 for r in fam)
                shares = custom or _distribute_students(total, target)
                used = {r.section_number for r in fam}
                new_numbers = [n for n in range(1, target + 1) if n not in used]
                rows = list(fam)
                for n in new_numbers:
                    new_code = _section_code(source.course_code, n)
                    if CourseAllocation.objects.filter(
                        program_id=source.program_id, course_code__iexact=new_code,
                        intake=source.intake, student_group_id=source.student_group_id,
                    ).exists():
                        return JsonResponse({"status": "error", "message": f"{new_code} already exists for this program."}, status=400)
                    clone = CourseAllocation.objects.create(
                        ...
                        number_of_students=0,
                        lecturer=source.lecturer if keep_lecturer else None,
                        allocation_set=source.allocation_set,
                        student_group=source.student_group,
                        special_intake_group=source.special_intake_group,
                        selection_group=source.selection_group,
                        specialization_stem=source.specialization_stem,
                        section_number=n,
                    )
                    # Same cohort: extra student groups, every stem, every pool.
                    clone.additional_student_groups.set(source.additional_student_groups.all())
                    for stem in source.specialization_stems.all():
                        stem.courses.add(clone)
                    for pool in source.selection_groups.all():
                        pool.courses.add(clone)
                    rows.append(clone)
                    created.append(clone)
                rows.sort(key=lambda r: r.section_number)
                for r, n_students in zip(rows, shares):
                    if r.number_of_students != n_students:
                        CourseAllocation.objects.filter(pk=r.pk).update(number_of_students=n_students)
                        r.number_of_students = n_students
            return JsonResponse({
                "status": "success",
                "message": f"{source.course_code} split into {target} sections ({len(created)} new).",
                "created": [_alloc_row_payload(c, dept) for c in created],
                "updated": [{"id": r.id, "number_of_students": r.number_of_students} for r in rows],
                "sections": target,
            })
            ...
            with transaction.atomic():
                ...
                for i in range(1, num_groups + 1):
                    ...
                    clone = CourseAllocation.objects.create(
                        ...
                        number_of_students=0,
                        lecturer=None,
                        # Concurrent Allocation Sets: a new lettered group of
                        # an existing course belongs in the same set as the
                        # course it's a group of.
                        allocation_set=source.allocation_set,
                    )
                    created.append({
                        "id": clone.id,
        ...
        # -----------------------
        # Bulk Add Group(s) — the same lettered-clone logic as
        # add_allocation_groups above, but for MANY selected courses at
        # once. Each course keeps its OWN base code and its OWN next
        # letter (courses aren't required to share a base) — only the
        # group COUNT is shared across the whole selection, e.g. select
        # EDFO 111, EDFO 112, EDFO 113, EDFO 114 and create 10 groups on
        # each in one go. Optionally, the newly-created groups can be
        # mapped straight away to one or more Student Groups and/or one
        # or more Specialization Stems (the same mapping
        # map_allocation_groups_and_stems below also supports later).
        # -----------------------
        if action == "get_group_clone_info_bulk":
            dept = detect_user_department(request.user)
            err = _assert_owns_department(request, dept)
            if err:
                return err
            ids = _parse_id_list(request.POST.get("ids"))
            if not ids:
                return JsonResponse({"status": "error", "message": "Select at least one course."}, status=400)

            sources = list(CourseAllocation.objects.filter(pk__in=ids))
            if not sources:
                return JsonResponse({"status": "error", "message": "No matching courses found."}, status=404)
            for s in sources:
                if s.department_id != dept.pk and s.origin_department_id != dept.pk:
                    from django.http import Http404
                    raise Http404
            ...
            return JsonResponse({"status": "success", "courses": info})

        if action == "add_allocation_groups_bulk":
            dept = detect_user_department(request.user)
            err = _assert_owns_department(request, dept)
            if err:
                return err
            ...
            # Optional: map every newly-created group straight away to one
            # or more Student Groups and/or Specialization Stems.
            student_group_ids = _parse_id_list(request.POST.get("student_group_ids"))
            stem_ids = _parse_id_list(request.POST.get("stem_ids"))
            ...
            student_groups = list(StudentGroup.objects.filter(pk__in=student_group_ids)) if student_group_ids else []
            stems = list(SpecializationStem.objects.filter(pk__in=stem_ids)) if stem_ids else []
            ...
            with transaction.atomic():
                for source in sources:
                    ...
                    # Same select_for_update locking pattern as the
                    # single-course add_allocation_groups, applied per
                    # source course so two courses in the same bulk
                    # request never race each other either.
                    lock_qs = CourseAllocation.objects.select_for_update().filter(
                        program_course_id=source.program_course_id
                    )
                    if source.program_id:
                    ...
                    for i in range(1, num_groups + 1):
                        ...
                        if CourseAllocation.objects.filter(
                            ...
                            errors.append(f"{new_code} already exists for this program — skipped.")
                            continue
                        clone = CourseAllocation.objects.create(
                            course_code=new_code,
                            course_name=source.course_name,
                        ...
                        if student_groups:
                            # Primary student_group stays the first pick
                            # (mirrors the single-group convention); the
                            # rest go on the new additional_student_groups
                            # many-to-many so the group is genuinely
                            # mapped to every selected Student Group.
                            clone.student_group = student_groups[0]
                            clone.save(update_fields=["student_group"])
                            if len(student_groups) > 1:
                                clone.additional_student_groups.set(student_groups[1:])
                        if stems:
                            for stem in stems:
                                stem.courses.add(clone)
            ...
            message = f"Added {len(created_all)} group(s) across {len(sources)} course(s)."
            if errors:
                message += " " + " ".join(errors)
        ...
        # -----------------------
        # Map a course-group allocation row to multiple Student Groups
        # and/or multiple Specialization Stems at once (the RE — Resource
        # Editor / mapping surface). A group can be left mapped to no
        # student group and no stem/combination-stem at all (fully
        # shared), mapped to exactly one, or mapped to several of either.
        # -----------------------
        if action == "get_allocation_group_mapping":
            dept = detect_user_department(request.user)
            err = _assert_owns_department(request, dept)
            if err:
                return err
            ...
            return JsonResponse({
                "status": "success",
                "student_group_ids": [g.id for g in alloc.all_mapped_student_groups],
                "stem_ids": list(alloc.specialization_stems.values_list("id", flat=True)),
            })

        if action == "map_allocation_groups_and_stems":
            dept = detect_user_department(request.user)
            err = _assert_owns_department(request, dept)
            if err:
                return err
            # One row (`id`) or many (`ids`, JSON/comma list) — the right-click
            # "Map to Student Group(s)…" works on a single course or on every
            # ticked course in "Select multiple…" mode.
            ids = _parse_id_list(request.POST.get("ids")) or _parse_id_list(request.POST.get("id"))
            if not ids:
                return JsonResponse({"status": "error", "message": "Select at least one course."}, status=400)
            allocs = list(CourseAllocation.objects.filter(pk__in=ids))
            if len(allocs) != len(set(ids)):
                from django.http import Http404
                raise Http404
            for alloc in allocs:
                if alloc.department_id != dept.pk and alloc.origin_department_id != dept.pk:
            ...
            # groups_only=1 -> leave stem membership untouched (the student-group
            # right-click must never silently pull a course out of its stems).
            groups_only = request.POST.get("groups_only") in ("1", "true", "on")
            # mode=replace -> the ticked set becomes the mapping (empty = shared,
            #                 mapped to no group);  mode=add -> ticked groups are
            #                 ADDED to whatever each course is already mapped to.
            ...
            if student_group_ids:
                wanted_groups = list(StudentGroup.objects.filter(pk__in=student_group_ids))
                if len(wanted_groups) != len(set(student_group_ids)):
                    return JsonResponse({"status": "error", "message": "Student group not found."}, status=404)
                for alloc in allocs:
                    bad = [g for g in wanted_groups if alloc.program_id and g.program_id != alloc.program_id]
                    if bad:
                        return JsonResponse({
                            "status": "error",
                            "message": f"{alloc.course_code} belongs to a different program than "
                                       f"{', '.join(g.name for g in bad)} — a unit can only be mapped to "
                                       f"student groups of its own program.",
                        }, status=400)
            else:
                wanted_groups = []
            ...
            with transaction.atomic():
                for alloc in allocs:
                    if mode == "add":
                        merged = list(alloc.all_mapped_student_groups)
                        have = {g.id for g in merged}
                        merged += [g for g in wanted_groups if g.id not in have]
                        groups = merged
                    else:
                        groups = wanted_groups
                    if groups:
                        alloc.student_group = groups[0]
                        alloc.additional_student_groups.set(groups[1:])
                    else:
                        alloc.student_group = None
                        alloc.additional_student_groups.clear()
                    alloc.save(update_fields=["student_group"])

                    if not groups_only:
                        # Replace this allocation's stem membership with exactly
                        # the selected set — remove it from any stem no longer
                        # ticked, add it to every newly-ticked one.
                        current_stem_ids = set(alloc.specialization_stems.values_list("id", flat=True))
                        wanted_stem_ids = {st.id for st in stems}
                        for stem in SpecializationStem.objects.filter(id__in=(current_stem_ids - wanted_stem_ids)):
                            stem.courses.remove(alloc)
                        for stem in stems:
                            if stem.id not in current_stem_ids:
                                stem.courses.add(alloc)
                    results.append({
                        "id": alloc.id,
                        "student_group_ids": [g.id for g in alloc.all_mapped_student_groups],
                    })
            ...
            return JsonResponse({
                "status": "success",
                "message": ("Mapping updated." if len(allocs) == 1
                            else f"Mapping updated for {len(allocs)} courses."),
                "student_group_ids": results[0]["student_group_ids"] if len(results) == 1 else [],
                "results": results,
        ...
        if action == "get_allocation_summary":
            ...
            if active_allocation_set is not None:
                qs = qs.filter(allocation_set=active_allocation_set)
        ...
        if action == "list_special_intake_allocs":
            ...
            # Concurrent Allocation Sets: this was the one listing endpoint
            # that never got scoped to active_allocation_set (list_allocations,
            # right above, already does this) — it always returned every
            # special-intake row for the whole department, from every
            # concurrent set at once. That's why "Archive & Delete" on
            # cod_panel_clear could correctly report 0 rows for the active
            # set while this table kept showing rows that actually belonged
            # to a different set.
            if active_allocation_set is not None:
                qs = qs.filter(allocation_set=active_allocation_set)
            qs = (
    ...
    # ── Concurrent Allocation Sets: which allocation is the COD editing? ────
    # If nothing has been explicitly chosen yet THIS SESSION for this
    # department, send them to the picker ("which allocation are you
    # editing? / create new") rather than guessing. Once chosen it stays
    # active (in-session) across page loads/switches within cod_panel, so
    # this only interrupts the very first visit of a session, not every
    # request. `?skip_allocation_picker=1` is an escape hatch for internal
    # redirects (e.g. back-links) that already know the scope is fine.
    if (detected_dept and request.method == "GET"
            and not request.GET.get("skip_allocation_picker")
            and session_active_allocation_set is None):
        from django.urls import reverse
        return redirect(reverse("allocation_picker"))
    ...
    if detected_dept:
        ...
        allocations = (
            ...
            .select_related(
                ...
                "student_group", "specialization_stem"
            )
        ...
        # Scope the listing to the AllocationSet the COD chose/switched to.
        # If for some reason none resolved (shouldn't happen given the
        # redirect above, but guards the ?skip_allocation_picker=1 escape
        # hatch and any other internal call into this branch), fall back to
        # the department's legacy set so nothing disappears silently.
        if active_allocation_set is None:
            active_allocation_set = get_or_default_legacy_set(detected_dept)
        if active_allocation_set is not None:
            allocations = allocations.filter(allocation_set=active_allocation_set)

        departments = Department.objects.select_related("faculty").all()

        # Server-rendered fallback for the Lecturer <select> before any JS
        # AJAX call has run (e.g. no course code typed yet). Scoped to the
        # COD's own department by default -- lecturers from other
        # departments are only pulled in on demand via the "select from
        # another department" toggle in the template, which re-fetches
        # through the get_mapped_lecturers action with
        # include_other_depts=1 instead of shipping every lecturer in the
        # university on every page load.
        lecturers = Lecturer.objects.filter(department=detected_dept).annotate(
            is_dept=Case(
        ...
        # Concurrent Allocation Sets: unlike SpecialIntakeGroup/StudentGroup
        # (which are shared across sets by design and only have their
        # *courses* scoped via the prefetch below), SelectionGroup itself
        # has its own allocation_set field and is per-set — the same
        # "Semester 2 Electives" name can legitimately exist once per set.
        # Without this filter, switching sets kept showing every group ever
        # created for this department regardless of which set is active.
        if active_allocation_set is not None:
            selection_groups = selection_groups.filter(allocation_set=active_allocation_set)
        ...
        # Concurrent Allocation Sets: prefetch each group's course_allocations
        # scoped to the active set, so the server-rendered table (visible
        # before any JS refresh runs, and as a fallback if it doesn't) never
        # shows another set's courses sitting inside "this" group. Groups
        # themselves (SpecialIntakeGroup / StudentGroup) stay unscoped and
        # shared across sets by design — only the courses listed inside
        # each one are filtered.
        _course_alloc_prefetch = (
            CourseAllocation.objects.filter(allocation_set=active_allocation_set)
            if active_allocation_set is not None else CourseAllocation.objects.all()
        )
        ...
        special_intake_groups = (
            ...
            .select_related("program")
            .prefetch_related(Prefetch("course_allocations", queryset=_course_alloc_prefetch))
        ...
        # Stamp each group with the FULL per-group course list (primary
        # `student_group` FK + `additional_student_groups` M2M combined) so
        # a course shared across several groups shows up in EVERY mapped
        # group's pill-summary/count on the /cod/ page, not just its
        # primary one. See _attach_all_group_allocations.
        student_groups = _attach_all_group_allocations(student_groups, active_allocation_set)
    else:
    ...
    # A course shared by several stems/combinations is listed under EACH one
    # (not once with a "Shared" badge). Only the main regular table groups by
    # stem, so only these two lists are expanded.
    own_dept_regular   = _expand_rows_per_stem(_annotate_cg(own_dept_regular))
    cross_dept_regular = _expand_rows_per_stem(_annotate_cg(cross_dept_regular))
    # In a stemless Program/Year, a course not yet tagged to a specific
    # lettered group is still taken by every group's students -- repeat it
    # under each group instead of a standalone "Shared / Not yet split
    # into groups" bucket.
    own_dept_regular   = _expand_shared_courses_per_group(own_dept_regular)
    cross_dept_regular = _expand_shared_courses_per_group(cross_dept_regular)
    ...
    return render(request, "course_management/cod_panel.html", {
        ...
        "allocation_sets_for_switcher": (
            AllocationSet.objects.filter(department=detected_dept, is_archived=False).order_by("-created_at")
            if detected_dept else []
        ),
