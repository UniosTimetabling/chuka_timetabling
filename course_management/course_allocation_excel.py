...
def export_course_allocation_template(request):
    ...
    # Concurrent Allocation Sets: scope the export to whichever set the COD
    # currently has active, so the workbook (and anything re-imported from
    # it) only ever reflects that set — otherwise every set's rows for this
    # department got exported together and re-importing could cross-wire
    # them (see import_course_allocation_template above). An explicit
    # allocation_set_id (from this page's switcher) takes priority over the
    # session's active set.
    from course_allocation.allocation_scope import (
        get_active_allocation_set, get_or_default_legacy_set,
        eligible_program_courses_for_component,
    )
    from course_allocation.models import AllocationSet
    ...
    def _section_for(program, year, semester):
        ...
        if key not in sections:
            sections[key] = {"program": program, "year": year, "semester": semester, "rows": []}
        return sections[key]
    ...
    visible_widths = [16, 34, 10, 20, 20, 28, 10, 16, 12, 22, 22, 10]
