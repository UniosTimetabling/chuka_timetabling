...
def create_allocation_step2(request, allocation_set_id):
    ...
    if request.method == "POST":
        ...
        if action == "add_semester":
            ...
            component, _created = allocation_set.semester_components.get_or_create(
                semester_number=semester_number, defaults={"scope": scope},
            )
            if scope == AllocationSetSemesterComponent.SCOPE_SELECTED:
                program_ids = request.POST.getlist("program_ids")
                if program_ids:
                    component.programs.set(program_ids)
            messages.success(request, f"Semester {semester_number} added to this allocation.")
