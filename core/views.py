...
def login_view(request):
    if request.method == "POST":
        ...
        if user is not None:
            ...
            # ✅ Role-based redirect — single source of truth in core/rbac.py
            # (resolve_dashboard_url_name), shared with the public homepage so
            # both places always agree on where a given role lands. Add new
            # roles to core.rbac.ROLE_DASHBOARD_URL_NAMES, not here.
            from core.rbac import resolve_dashboard_url_name

            return redirect(resolve_dashboard_url_name(user))
