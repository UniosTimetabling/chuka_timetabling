...
class SiteSettings(models.Model):
    ...
    mobile_app_share_url = models.URLField(
        max_length=500,
        blank=True,
        default="",
        help_text="Optional custom link people are sent to when they share/install the app "
                  "(Play Store, a Drive/OneDrive link, your own download page, etc.). When set it "
                  "overrides the uploaded APK and the default store link — on the Mobile Analytics "
                  "QR code AND inside the mobile app's own \"Invite someone\" screen. Leave blank to "
                  "use the default.",
    )
