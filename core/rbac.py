...
# ─────────────────────────────────────────────────────────────────────────────
# Canonical "where does this user's dashboard live" mapping.
# Single source of truth shared by login_view (post-login redirect) and the
# public homepage (redirect already-logged-in visitors straight to their
# dashboard instead of showing the marketing/public page).
# ─────────────────────────────────────────────────────────────────────────────
...
def resolve_dashboard_url_name(user) -> str | None:
    """
    Return the url name of the dashboard this authenticated user should land
    on, based on their canonical role(s), or None if the user is anonymous.

    Superusers -> "sudo_homepage".
    Users holding no mapped role -> "user_dashboard" (safe fallback).
    Keep this mapping here (not duplicated in views) so login_view and the
    public homepage redirect always agree on where a given role lands.
    """
    if not user or not user.is_authenticated:
        return None
    if user.is_superuser:
    ...
    for role, url_name in ROLE_DASHBOARD_URL_NAMES.items():
