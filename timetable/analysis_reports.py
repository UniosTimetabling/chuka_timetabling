...
from reportlab.platypus import (
    Table, TableStyle, Paragraph, Spacer, Image, HRFlowable, KeepInFrame)
from core.doc_export import (
    DOCX_CONTENT_TYPE, SimpleDocTemplate, pdf_to_docx_bytes, swap_extension, wants_docx,
)
...
from timetable.models import Timetable, ExamTimetable, SchedulerConfig, ExamSchedulerConfig, AnalysisAccessShare
...
from course_management.cod_panel import strip_group_suffix, normalize_code, detect_user_department
...
# ═══════════════════════════════════════════════════════════════════════════
# ACCESS CONTROL — /timetable/analysis/ is normally SUDO/Director/Timetable
# Admin only. AnalysisAccessShare (timetable/models.py) is the single global
# switch that lets one of those roles "share" read-only access to it with
# COD and/or COD Admin accounts too — either every department's, or a
# chosen subset. A shared COD/COD Admin never gets the free "any
# department" picker a real timetable admin has: their view is always
# pinned to their OWN department (see _effective_department_id below),
# even when the share itself covers every department.
# ═══════════════════════════════════════════════════════════════════════════

def _can_access_analysis(user):
    """Return (allowed: bool, locked_department_id: int | None).

    locked_department_id is None for a true timetable admin (or a "whole
    university" share) — they keep the page's normal free department
    choice. It's set to a department's pk for a COD/COD Admin who was
    let in via a department-scoped share, meaning every department_id
    this request asks for must be silently overridden to that one value
    (done in _effective_department_id) rather than trusted from the
    querystring.
    """
    if not user or not user.is_authenticated:
        return False, None
    if user.is_superuser or user_has_role(user, Role.SUDO, Role.DIRECTOR, Role.TIMETABLE_ADMIN):
        return True, None

    share = AnalysisAccessShare.get_solo()
    if not share.is_enabled:
        return False, None

    is_cod = user_has_role(user, Role.COD)
    is_cod_admin = user_has_role(user, Role.COD_ADMIN)
    if not ((is_cod and share.share_with_cod) or (is_cod_admin and share.share_with_cod_admin)):
        return False, None

    if share.apply_to_all_departments:
        return True, None

    dept = detect_user_department(user)
    if dept and share.departments.filter(pk=dept.pk).exists():
        return True, dept.id
    return False, None


def analysis_access_required(view_func):
    """Replaces @allowed_roles(Role.SUDO, Role.DIRECTOR, Role.TIMETABLE_ADMIN)
    on every read-only report/export endpoint in this module, adding the
    AnalysisAccessShare check above on top of the usual admin-only pass.
    Stamps request.analysis_locked_department_id (None, or a department pk)
    for _effective_department_id() to enforce. Endpoints that send email
    (department_email_recipients_api, send_department_timetable_email)
    deliberately keep the plain @allowed_roles(...) decorator instead —
    sharing view access to the dashboard is not the same as trusting
    someone to send official cross-department email.
    """
    @wraps(view_func)
    def _wrapped(request, *args, **kwargs):
        allowed, locked_department_id = _can_access_analysis(request.user)
        if not allowed:
            if request.user.is_authenticated:
                logger.warning(
                    "Analysis dashboard access denied | user=%s | view=%s",
                    request.user.username, view_func.__name__,
                )
            return redirect("login")
        request.analysis_locked_department_id = locked_department_id
        return view_func(request, *args, **kwargs)
    return login_required(_wrapped)


def _effective_department_id(request):
    """department_id for analysis endpoints: the normal GET param, UNLESS
    analysis_access_required() pinned this request to one department (a
    shared COD/COD Admin), in which case that department always wins —
    whatever department_id (if any) was actually requested is ignored."""
    locked = getattr(request, 'analysis_locked_department_id', None)
    if locked:
        return str(locked)
    return request.GET.get('department_id')
...
def _apply_allocation_scope(qs, request, prefix='allocation_set'):
    """
    Narrow `qs` down to whichever AllocationSet(s) are the TT's active
    session scope — the exact same "active allocation set" picked on
    /timetable/dashboard/ (course_allocation.allocation_scope), and now
    ALSO switchable from the Allocation Set panel on this analysis
    dashboard (see analysis_allocation_sets_api / apply_analysis_allocation_sets
    below). There is one shared, session-wide selection — not a separate
    one per page — so switching it in either place changes it for both,
    and every analysis report/search on this page reflects whatever is
    currently active, exactly like the timetable/exam/lab panels do.

    `prefix` is the lookup path from `qs`'s model to AllocationSet:
    'allocation_set' for a CourseAllocation queryset (the default), or
    e.g. 'course_allocation__allocation_set' for a Timetable/ExamTimetable
    queryset joined back to its CourseAllocation.
    """
    return apply_tt_scope(qs, request=request, prefix=prefix)


# ---------------------------------------------------------------------------
# Combination (Specialization Stem) / Student Group scope
# ---------------------------------------------------------------------------
# Picking a Program in the Scope card can be narrowed further to ONE
# Specialization Stem ("combination") and/or ONE Student Group, sent as
# ?stem_id= / ?student_group_id=. The narrowed scope is NOT just "the rows
# tagged with that stem/group": students of a stem/group also take every
# course that is shared with them, so those are kept in scope too:
#
#   stem S      -> S's own courses (primary FK or M2M, including courses S
#                  shares with other stems)  +  every course of the program
#                  that belongs to NO stem at all (compulsory units AND
#                  electives / selection-group courses), limited to the
#                  Year/Semester the stem's category applies to when set.
#   group G     -> courses of G (student_group=G or mapped via
#                  additional_student_groups)  +  every ungrouped course of
#                  G's Year/Semester/intake that is not tied to a specific
#                  other group (shared across all groups), minus courses
#                  that only sit in stems restricted to other groups.
#   both        -> both conditions must hold (a student in group G who took
#                  combination S).
...
def _partition_scope_q(request, p=""):
    """Q over CourseAllocation (reached through path prefix `p`, e.g.
    'course_allocation__' for Timetable rows) for the stem / student-group
    scope, or None when neither was requested."""
    stem_id = _int_or_none(request.GET.get('stem_id'))
    group_id = _int_or_none(request.GET.get('student_group_id'))
    if not stem_id and not group_id:
        return None

    StemThrough = SpecializationStem.courses.through
    GroupThrough = CourseAllocation.additional_student_groups.through

    def f(**kw):
        return Q(**{f'{p}{k}': v for k, v in kw.items()})

    def id_in(subquery):
        return Q(**{f'{p}id__in': subquery})

    nothing = id_in([])
    in_any_stem = id_in(StemThrough.objects.values('courseallocation_id'))
    no_stem = f(specialization_stem__isnull=True) & ~in_any_stem

    combined = Q()

    if stem_id:
        stem = SpecializationStem.objects.select_related('category').filter(id=stem_id).first()
        if stem is None:
            return nothing
        cat = stem.category
        in_stem = f(specialization_stem_id=stem.id) | id_in(
            StemThrough.objects.filter(specializationstem_id=stem.id).values('courseallocation_id')
        )
        shared = no_stem
        if cat.year:
            shared = shared & f(program_course__year=cat.year)
        if cat.semester:
            shared = shared & f(program_course__semester=cat.semester)
        combined = combined & (in_stem | shared) & f(program_id=cat.program_id)

    if group_id:
        grp = StudentGroup.objects.filter(id=group_id).first()
        if grp is None:
            return nothing
        own = f(student_group_id=grp.id) | id_in(
            GroupThrough.objects.filter(studentgroup_id=grp.id).values('courseallocation_id')
        )
        any_group_m2m = id_in(GroupThrough.objects.values('courseallocation_id'))
        shared_to_all_groups = (
            f(student_group__isnull=True) & ~any_group_m2m
            & f(program_course__year=grp.year)
            & f(program_course__semester=grp.semester)
            & f(intake=grp.intake)
        )
        # Courses that only live in stems restricted to OTHER groups are not
        # this group's courses.
        blocked_stems = (
            SpecializationStem.objects.filter(restricted_to_groups__isnull=False)
            .exclude(restricted_to_groups=grp).values('id')
        )
        ok_stems = (
            SpecializationStem.objects
            .filter(Q(restricted_to_groups__isnull=True) | Q(restricted_to_groups=grp)).values('id')
        )

        def in_stems(stems_qs):
            return id_in(
                StemThrough.objects.filter(specializationstem_id__in=stems_qs).values('courseallocation_id')
            ) | f(specialization_stem_id__in=stems_qs)

        blocked = in_stems(blocked_stems) & ~in_stems(ok_stems)
        combined = combined & (own | shared_to_all_groups) & ~blocked & f(program_id=grp.program_id)

    return combined


def _apply_partition_scope(qs, request, alloc_field_prefix=""):
    q = _partition_scope_q(request, alloc_field_prefix)
    return qs.filter(q) if q is not None else qs


def _apply_scope_filters(qs, request, alloc_field_prefix=""):
    """
    Apply faculty_id / department_id / program_id / year GET params, AND
    the session's active AllocationSet(s) (see _apply_allocation_scope
    above), to a CourseAllocation (or Timetable, via
    alloc_field_prefix='course_allocation__') queryset. All params are
    ...
    department_id = _effective_department_id(request)
    ...
    qs = _apply_partition_scope(qs, request, p)
    qs = _apply_allocation_scope(qs, request, prefix=f'{p}allocation_set')
    return qs
...
def _scope_label(request):
    ...
    stem_id = _int_or_none(request.GET.get('stem_id'))
    if stem_id:
        st = SpecializationStem.objects.select_related('category').filter(id=stem_id).first()
        if st:
            parts.append(f"Combination: {st.name} (incl. shared/compulsory/elective courses)")
    group_id = _int_or_none(request.GET.get('student_group_id'))
    if group_id:
        grp = StudentGroup.objects.filter(id=group_id).first()
        if grp:
            parts.append(f"Student Group: {grp.name} (Year {grp.year}, Sem {grp.semester})")

    active_set_ids = get_tt_active_allocation_set_ids(request)
    if active_set_ids:
        names = list(
            AllocationSet.objects.filter(id__in=active_set_ids)
            .select_related('department')
            .values_list('department__name', 'name')
        )
        if names:
            parts.append("Allocation Set(s): " + ", ".join(f"{d} — {n}" for d, n in names))
...
def _apply_scope_filters_origin(qs, request):
    ...
    qs = _apply_partition_scope(qs, request)
    return _apply_allocation_scope(qs, request)
...
def _expand_code_to_allocations(raw_code, request=None):
    """
    ...
    narrowed = _apply_allocation_scope(narrowed, request)
...
def _expand_term_to_allocations(raw_term, request=None):
    """
    ...
    `request`, when given, narrows both the code match and the free-text
    fallback to the session's active AllocationSet(s) — see
    _apply_allocation_scope.
    """
    matched = {a.id: a for a in _expand_code_to_allocations(raw_term, request=request)}

    if not matched:
        for a in _universal_search_qs(raw_term, request=request):
            matched[a.id] = a
...
def analysis_dashboard(request):
    ...
    locked_department_id = getattr(request, 'analysis_locked_department_id', None)
    locked_department = Department.objects.filter(pk=locked_department_id).first() if locked_department_id else None

    # Only the true owners of this page (never a shared COD/COD Admin, even
    # one with "whole university" access) can change who else it's shared
    # with — see analysis_sharing_settings_api / update_analysis_sharing_settings.
    can_manage_sharing = request.user.is_superuser or user_has_role(
    ...
    context = {
        ...
        'is_department_locked': locked_department is not None,
        'locked_department_id': locked_department.id if locked_department else None,
        'locked_department_name': locked_department.name if locked_department else None,
...
def analysis_sharing_settings_api(request):
    """Current AnalysisAccessShare config, for the Sharing panel to load.
    Admin-only — same reasoning as the email endpoints: seeing/changing who
    else can view the dashboard isn't something a shared viewer should do,
    even if their own share happens to cover the whole university."""
    share = AnalysisAccessShare.get_solo()
    return JsonResponse({
        'status': 'success',
        'is_enabled': share.is_enabled,
        'share_with_cod': share.share_with_cod,
        'share_with_cod_admin': share.share_with_cod_admin,
        'apply_to_all_departments': share.apply_to_all_departments,
        'department_ids': list(share.departments.values_list('id', flat=True)),
    })


@require_POST
@allowed_roles(Role.SUDO, Role.DIRECTOR, Role.TIMETABLE_ADMIN)
def update_analysis_sharing_settings(request):
    """Save the AnalysisAccessShare config from the Sharing panel."""
    share = AnalysisAccessShare.get_solo()
    share.is_enabled = request.POST.get('is_enabled') == 'true'
    share.share_with_cod = request.POST.get('share_with_cod') == 'true'
    share.share_with_cod_admin = request.POST.get('share_with_cod_admin') == 'true'
    share.apply_to_all_departments = request.POST.get('apply_to_all_departments') == 'true'
    share.updated_by = request.user
    share.save()

    if not share.apply_to_all_departments:
        dept_ids = [d for d in request.POST.getlist('department_ids') if d.strip().isdigit()]
        share.departments.set(Department.objects.filter(pk__in=dept_ids))
    else:
        share.departments.clear()

    logger.info(
        "Analysis dashboard sharing updated | by=%s | %s",
        request.user.username, str(share),
    )
    return JsonResponse({'status': 'success', 'message': 'Sharing settings saved.'})


# ═══════════════════════════════════════════════════════════════════════════
# Allocation Set switcher — lets the analysis dashboard pick which
# AllocationSet(s) every report and search on this page scopes to. This is
# the SAME session-wide "active allocation set" selection as
# /timetable/dashboard/ (course_allocation.allocation_scope,
# SESSION_KEY_TT_ACTIVE_SETS) — switching it here also changes what the
# main/exam/lab timetable panels and autoschedulers use for the rest of
# the session, and switching it on the dashboard is reflected back here.
# There's one shared selection, not a separate one per page.
# ═══════════════════════════════════════════════════════════════════════════

@analysis_access_required
def analysis_allocation_sets_api(request):
    """
    GET: every non-archived AllocationSet (grouped by department) the
    switcher can offer, plus which one(s) are currently active for this
    session — for the Allocation Set panel on this page to render itself.
    """
    sets_qs = (
    ...
    for s in sets_qs:
        by_department[s.department.name].append({
            'id': s.id,
            'name': s.name,
            'status': s.status,
    ...
    active_ids = get_tt_active_allocation_set_ids(request)
    return JsonResponse({
        'status': 'success',
        'active_allocation_set_ids': active_ids,
        'allocation_sets_by_department': [
            {'department': dept, 'sets': items}
            for dept, items in sorted(by_department.items())
        ],
    })
...
@require_POST
@analysis_access_required
def apply_analysis_allocation_sets(request):
    """
    POST allocation_set_ids[]=... — switches the session-wide active
    AllocationSet(s) from this page's Allocation Set panel. Invalid or
    archived ids are silently dropped (see
    course_allocation.allocation_scope.set_tt_active_allocation_set_ids);
    posting none at all clears the selection, which falls every report on
    this page (and every other TT panel/autoscheduler) back to the
    default "eligible" gate (no set yet / legacy / submitted-to-TT), same
    as never having picked anything this session.
    """
    ...
    logger.info(
        "Analysis dashboard allocation-set scope changed | by=%s | ids=%s",
        request.user.username, stored_ids,
    )

    return JsonResponse({
        'status': 'success',
        'active_allocation_set_ids': stored_ids,
        'active_allocation_sets': [
            {'id': s.id, 'name': s.name, 'department': s.department.name}
            for s in sets
        ],
        'message': (
            f"Switched to {len(stored_ids)} allocation set(s). This applies to every "
            "report/search on this page, the timetable dashboard, and every other "
            "timetable panel and autoscheduler for the rest of your session."
        ) if stored_ids else (
            "No allocation sets selected — every report/search on this page (and every "
            "other timetable panel and autoscheduler) will fall back to the default "
            "(not-yet-set / legacy / submitted-to-TT) allocations for the rest of your session."
        ),
    })


@analysis_access_required
def analysis_program_partitions_api(request):
    """
    For the Scope card: given ?program_id= (and optionally ?year=), list the
    Specialization Stems ("combinations") and Student Groups of that program
    so the person can narrow the scope further. Stems are limited to the
    session's active AllocationSet(s); with ?year= given, only stems whose
    category applies to that year (or to no particular year) and groups of
    that year are returned.
    """
    program_id = _int_or_none(request.GET.get('program_id'))
    year = _int_or_none(request.GET.get('year'))
    empty = {'status': 'success', 'program_id': program_id, 'stems': [], 'student_groups': []}
    if not program_id:
        return JsonResponse(empty)

    program = Program.objects.filter(id=program_id).first()
    locked = getattr(request, 'analysis_locked_department_id', None)
    if program is None or (locked and str(program.department_id) != str(locked)):
        return JsonResponse(empty)

    stems_qs = SpecializationStem.objects.filter(category__program_id=program_id).select_related('category')
    stems_qs = apply_tt_scope(stems_qs, request=request, prefix='category__allocation_set')
    if year:
        stems_qs = stems_qs.filter(Q(category__year__isnull=True) | Q(category__year=year))
    stems = [{
        'id': st.id,
        'name': st.name,
        'category': st.category.name,
        'year': st.category.year,
        'semester': st.category.semester,
    } for st in stems_qs.order_by('category__year', 'category__semester', 'category__name', 'name')]

    groups_qs = StudentGroup.objects.filter(program_id=program_id)
    if year:
        groups_qs = groups_qs.filter(year=year)
    groups = [{
        'id': g.id,
        'name': g.name,
        'letter': g.letter,
        ...
        'semester': g.semester,
        'intake': g.intake,
    } for g in groups_qs.order_by('year', 'semester', 'intake', 'letter')]

    return JsonResponse({'status': 'success', 'program_id': program_id, 'stems': stems, 'student_groups': groups})
...
def _build_course_lookup_results(codes, model=Timetable, request=None):
    """
    ...
    `request`, when given, narrows every term's match to the session's
    active AllocationSet(s) — see _apply_allocation_scope.
    ...
    for raw_code in codes:
        allocs = _expand_term_to_allocations(raw_code, request=request)
        matches = []
        for a in allocs:
...
def query_course_schedule_api(request):
    ...
    scope = _resolve_scope(request)
    results = _build_course_lookup_results(codes, model=_scoped_model(scope), request=request)
...
def export_course_schedule_pdf(request):
    ...
    results = _build_course_lookup_results(codes, model=_scoped_model(_resolve_scope(request)), request=request)
...
def _universal_search_qs(query, scope=SCOPE_TIMETABLE, request=None):
    """
    ...
    `request`, when given, also narrows the search to the session's active
    AllocationSet(s) (see _apply_allocation_scope) — so searching only
    ever surfaces courses belonging to whichever allocation set(s) are
    currently switched on for this session, the same as every other
    report on this dashboard.
    """
    q = (query or '').strip()
    if not q:
    ...
    qs = _apply_allocation_scope(qs, request)
    return qs.distinct()
...
def universal_search_api(request):
    ...
    scope = _resolve_scope(request)
    Model = _scoped_model(scope)
    qs = _universal_search_qs(query, scope=scope, request=request).order_by('department__name', 'program__name', 'course_code')
    tt_cache = {
        tt.course_allocation_id: tt
        for tt in Model.objects.filter(
            venue__isnull=False, start_time__isnull=False, end_time__isnull=False,
...
def lecturer_workload_api(request):
    ...
    try:
        ...
        qs = _apply_allocation_scope(qs, request, prefix='course_allocation__allocation_set')
...
def program_year_weekly_schedule_api(request):
    ...
    try:
        ...
        allocations = list(_apply_allocation_scope(
...
# Recipient resolution (COD / COD Admin lookup, audience filtering, the
# free-text override parser) now lives in core/department_recipients.py so
# this module and course_management's Course Allocation Template sharing
# stay in sync. Thin aliases kept here so the rest of this file (and any
# other code importing these by their old names from this module) doesn't
# need to change.
from core.department_recipients import (
    resolve_department_email_recipients as _resolve_department_email_recipients,
    filter_recipients_by_audience as _filter_recipients_by_audience,
    parse_custom_emails as _parse_custom_emails,
)
...
def _attachment_as_word(filename, content, _mimetype):
    """Attachment tuple → Word: natively built docx bytes are kept, stored PDFs
    are converted, and the file name / mimetype are switched to .docx."""
    if content[:4] == b"%PDF":
        content = pdf_to_docx_bytes(content)
    return (swap_extension(filename, "docx"), content, DOCX_CONTENT_TYPE)


def _build_department_email_attachments(request, department, include_unscheduled, include_scheduled,
                                          include_allocation, include_department_timetable, scope=SCOPE_TIMETABLE):
    """
    ...
    if include_scheduled:
        # Attach the actual latest PUBLISHED Timetable PDF — the same
        # record/file the public Published Timetables registry shows at
        # /published/timetables/ — rather than regenerating a live PDF from
        # current Timetable rows. This is a single university-wide document
        # (not department-scoped), so every department that gets this
        # attachment receives the identical file. Scope-aware: 'exam' pulls
        # the latest Consolidated Exam Timetable, 'timetable' pulls the
        # latest Consolidated Regular Timetable.
        from export_import.models import PDFDocument
        doc_type = 'EXAM' if scope == SCOPE_EXAM else 'REGULAR'
        doc_label = 'Exam' if scope == SCOPE_EXAM else 'Regular'
        latest_doc = (
            PDFDocument.objects
            .filter(document_type=doc_type, status='PUBLISHED', is_latest=True)
        ...
        if latest_doc and latest_doc.pdf_file:
            ...
            attachments.append((
                f"Consolidated {doc_label} Timetable - {latest_doc.academic_year} Sem {latest_doc.semester} (v{latest_doc.version}).pdf",
                file_bytes, 'application/pdf',
            ))
        else:
            notes.append(
                f"No published Consolidated {doc_label} Timetable is on file yet — publish one via "
                f"the Published Timetables page ({doc_label} section) before sending."
            )
    ...
    if include_allocation:
        ...
        if run and run.file and wants_docx():
            try:
                from allocation_reports.services import build_allocation_docx
                attachments.append((f"{department.name} - Course Allocation.pdf",
                                    build_allocation_docx(run.scope, department, campus=run.campus, user=request.user),
                                    'application/pdf'))
            except Exception as exc:
                logger.exception("send_department_timetable_email: failed building allocation Word file")
                notes.append(f"Could not attach the course allocation ({exc}).")
        elif run and run.file:
            try:
                run.file.open('rb')
    ...
    if wants_docx():
        attachments = [_attachment_as_word(*a) for a in attachments]
...
def _send_one_department_email(request, department, subject, body, sender_name,
                                 ...
                                 audience='both', custom_emails=None, scope=SCOPE_TIMETABLE):
    """
    ...
    Recipients: if `custom_emails` is given, it REPLACES the auto-resolved
    COD/COD Admin addresses entirely (the operator typed exactly who this
    should go to, for every department in a bulk send too). Otherwise the
    normal auto-resolution runs, narrowed by `audience` ('cod' / 'cod_admin'
    / 'both').
    """
    if custom_emails:
        ...
        recipients = _resolve_department_email_recipients(department)
        to_emails = _filter_recipients_by_audience(recipients, audience)
    if not to_emails:
        return {
            'department': department.name,
            'status': 'skipped',
    ...
    attachments, notes = _build_department_email_attachments(
        ...
        include_allocation, include_department_timetable, scope=scope,
    )
    if not attachments:
        return {
            'department': department.name,
...
def send_department_timetable_email(request):
    """Wrapper: the sender picks the attachment format (PDF or Word) in the
    form's `export_format` field; every attachment is produced in that format."""
    from core.doc_export import force_format, requested_format
    with force_format(requested_format(request)):
        return _send_department_timetable_email_impl(request)


def _send_department_timetable_email_impl(request):
    """
    ...
    attaching any combination of:
      ...
      - the latest published Consolidated Regular/Exam Timetable PDF
        (whichever matches the page's current scope)
      - the department's own "Departmental Timetable" letterhead PDF
        (regular scope only — no exam equivalent yet)
    ...
    resolution entirely — the message goes to exactly the address(es)
    typed, for every department in a bulk send too.
    ...
    # Who to send to. A typed custom_email always overrides auto-resolution
    # (for every department, including in a bulk "all" send) — lets the
    # sender redirect a department's report to themselves or anyone else
    # instead of that department's registered COD/COD Admin. Otherwise
    # `audience` narrows the normal auto-resolution to just COD, just COD
    # Admin, or both (the previous, only, behaviour).
    audience = (request.POST.get('audience') or 'both').strip().lower()
    if audience not in ('both', 'cod', 'cod_admin'):
        audience = 'both'
    custom_emails = _parse_custom_emails((request.POST.get('custom_email') or '').strip())
    ...
    # The "Departmental Timetable" letterhead PDF (course_management's own
    # module) has no exam equivalent yet — refuse rather than silently
    # attaching the regular-timetable version to an exam-scope email. The
    # published-timetable attachment ("Consolidated Regular/Exam Timetable")
    # IS scope-aware now (see _build_department_email_attachments) so it's
    # no longer blocked here.
    exam_or_tt_scope = _resolve_scope(request)
    if exam_or_tt_scope == SCOPE_EXAM and include_department_timetable:
        return JsonResponse({
            'status': 'error',
            'message': (
                "The Departmental Timetable attachment is only available for the regular "
                "timetable scope — it has no exam equivalent yet. Deselect it, or use the "
                "Unscheduled/Published-Timetable/Course-Allocation attachments for exams."
            ),
        }, status=400)
    ...
    if department_id == 'all':
        ...
        results = [
            _send_one_department_email(
                ...
                include_unscheduled, include_scheduled,
                include_allocation, include_department_timetable,
                audience=audience, custom_emails=custom_emails, scope=exam_or_tt_scope,
            )
