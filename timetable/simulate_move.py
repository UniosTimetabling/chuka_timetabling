...
def execute_move_api(request):
    ...
    # These rows just landed in a new slot: clear whatever issue flags
    # they carried from their OLD slot and record fresh ones for the new
    # one (only if this move actually needed `force` through a warning/
    # error — a clean move is a pure clear). Runs in the background so it
    # never slows this response down. See timetable/issue_tracking.py.
    from .issue_tracking import sync_issues_for_bundle
    sync_issues_for_bundle(
        exclude_ids, messages_list, source="manual_override",
        user=getattr(request, "user", None), replace=True,
    )
...
def execute_swap_api(request):
    ...
    from .issue_tracking import sync_issues_for_bundle
    sync_issues_for_bundle(ids_a, messages_a, source="manual_override",
                           user=getattr(request, "user", None), replace=True)
    sync_issues_for_bundle(ids_b, messages_b, source="manual_override",
                           user=getattr(request, "user", None), replace=True)
...
def execute_combine_api(request):
    ...
    from .issue_tracking import sync_issues_for_bundle
    sync_issues_for_bundle(ids_a, messages_list, source="manual_override",
                           user=getattr(request, "user", None), replace=True)
...
def bulk_move_execute_api(request):
    ...
    for mv in moves:
        ...
        with transaction.atomic():
            ...
            from .issue_tracking import sync_issues_for_bundle
            sync_issues_for_bundle(exclude_ids, messages_list, source="manual_override",
                                   user=getattr(request, "user", None), replace=True)
            applied.append({
...
def execute_copy_api(request):
    ...
    from .issue_tracking import sync_issues_for_bundle
    sync_issues_for_bundle([r.id for r in new_rows], messages_list, source="manual_override",
                           user=getattr(request, "user", None), replace=False)
