...
def _program_hits_main(cohorts, day, st, et):
    """
    Regular lecture rows that genuinely collide with these student cohorts.

    A cohort is (program_id, year_of_study) — NOT a bare program. Matching
    on program alone reported, for example, "students due in lab BCHM 221
    also have a lecture (BCHM 351)": same Biochemistry programme, but Year 2
    vs Year 3 — different students entirely, no collision. Candidates are
    narrowed in SQL by program, then filtered by cohort in Python.
    """
    cohorts = set(cohorts or ())
    if not cohorts:
        return []
    pids = {p for p, _ in (c if isinstance(c, tuple) else (c, None) for c in cohorts) if p}
    if not pids:
        return []
    rows = Timetable.objects.filter(
        course_allocation__program_id__in=pids,
        day=day, start_time__lt=et, end_time__gt=st,
    ).select_related("course_allocation", "course_allocation__program_course", "venue")
    return [
        tt for tt in rows
...
def check_lab_slot_conflicts(alloc, lab_venue, day, st, et, exclude_lab_ids=None):
    ...
    # Cohort = (program, year of study). Two different years of the same
    # programme are different students, so they must not clash-check.
    cohorts = _involved_cohorts(alloc)
    program_ids = {p for p, _ in cohorts if p}

    if lecturer:
        # Another row of the SAME course (same allocation in a second venue,
        # or a sibling section with the same base course code) is a split,
        # not a lecturer clash — skip those and only flag genuinely
        # different courses.
        lecturer_keys = _lab_base_keys(alloc)
        lecturer_lab_rows = LabTimetable.objects.filter(
            lab_allocation__lecturer_id=lecturer.id, day=day,
            start_time__lt=et, end_time__gt=st,
        ).exclude(id__in=exclude_lab_ids).select_related(
            "lab_allocation", "lab_allocation__program_course"
        ).prefetch_related("lab_allocation__additional_courses")
        lab_clash = next(
            (tt for tt in lecturer_lab_rows
             if tt.lab_allocation_id != alloc.id
             and not _is_same_course_split(lecturer_keys, _lab_base_keys(tt.lab_allocation))),
            None,
        )
        if lab_clash:
            messages.append(
    ...
    if program_ids:
        lab_candidates = LabTimetable.objects.filter(
            ...
            "lab_allocation", "lab_allocation__program_course"
        ).prefetch_related("lab_allocation__additional_courses").distinct()
        # A candidate carrying exactly the same base course code is a split
        # SECTION of this same course (cohort too large for one lab room),
        # not a double-booking — skip it.
        my_keys = _lab_base_keys(alloc)
        prog_lab_clash = next(
            (tt for tt in lab_candidates
             if not _is_same_course_split(my_keys, _lab_base_keys(tt.lab_allocation))
             and _cohorts_overlap(cohorts, _involved_cohorts(tt.lab_allocation))),
            None,
        )
        if prog_lab_clash:
            messages.append(
                f"Students in this program/year already have a lab — "
                f"{prog_lab_clash.lab_allocation.all_course_codes()} — on {day} at that time."
            )
        for tt in _program_hits_main(cohorts, day, st, et):
            messages.append(
                f"Students in this program/year already have a lecture — "
                f"{tt.course_allocation.course_code} in {tt.venue.code} — on {day} at that time."
            )
...
def _shared_cohorts(cohorts_a, cohorts_b):
    """The (program_id, year) pairs the two sides actually have in common.
    A year of None on either side is treated as "unknown year" and matches
    any year of the same programme (fail safe — see _cohorts_overlap)."""
    shared = set()
    for pid_a, year_a in cohorts_a or ():
        for pid_b, year_b in cohorts_b or ():
            if pid_a != pid_b or pid_a is None:
                continue
            if year_a is None or year_b is None or year_a == year_b:
                shared.add((pid_a, year_a if year_a is not None else year_b))
    return shared
...
def _program_name(program_id):
    if program_id not in _PROGRAM_NAME_CACHE:
        prog = Program.objects.filter(id=program_id).only("name").first()
        _PROGRAM_NAME_CACHE[program_id] = prog.name if prog else f"Program {program_id}"
    return _PROGRAM_NAME_CACHE[program_id]
...
def compute_lab_conflicts():
    ...
    _PROGRAM_NAME_CACHE.clear()   # names can change between requests
    lecturer_conflicts, student_conflicts, flagged = [], [], set()

    def _cohort_label(cohorts):
        """'BSc Biochemistry Y2' style tag, for the conflict message."""
        parts = []
        for pid, year in sorted(cohorts, key=lambda c: (str(c[0]), str(c[1]))):
            name = _program_name(pid)
            parts.append(f"{name} Year {year}" if year else name)
        return ", ".join(parts)
    ...
    def _entry_label(tt):
        return {
            ...
            "cohort": _cohort_label(_involved_cohorts(tt.lab_allocation)),
        }

    for i, a in enumerate(labs):
        a_alloc = a.lab_allocation
        # Cohorts, not bare programs: a Year-2 lab and a Year-3 lecture of
        # the same programme have no students in common.
        a_cohorts = _involved_cohorts(a_alloc)
        a_base_keys = _lab_base_keys(a_alloc)
        ...
        for b in labs[i + 1:]:
            ...
            # One course run in two labs at once (same allocation across two
            # venues, or sibling sections sharing a base course code) is a
            # deliberate SPLIT of the cohort, taught by the same lecturer —
            # not a lecturer double-booking.
            is_split = (
                a.lab_allocation_id == b.lab_allocation_id
                or _is_same_course_split(a_base_keys, _lab_base_keys(b_alloc))
            )
            if a_lecturer and b_alloc.lecturer_id == a_lecturer.id and not is_split:
            ...
            b_cohorts = _involved_cohorts(b_alloc)
            # Same base course code on both rows = one course split into
            # sections (e.g. EENG 243 in two labs at once). Each section
            # takes a disjoint slice of the cohort, so this is a deliberate
            # split, not students booked into two labs at once.
            if _is_same_course_split(a_base_keys, _lab_base_keys(b_alloc)):
                pass
            elif _cohorts_overlap(a_cohorts, b_cohorts):
                shared_label = _cohort_label(_shared_cohorts(a_cohorts, b_cohorts))
                student_conflicts.append({
                    "type": "lab_vs_lab", "a": _entry_label(a), "b": _entry_label(b),
                    "cohort": shared_label,
                    "message": (
                        f"Students in the same programme year"
                        f"{' (' + shared_label + ')' if shared_label else ''} are booked into "
                        f"two labs at once on {a.day} "
                        f"({a_alloc.all_course_codes()} vs {b_alloc.all_course_codes()})."
                    ),
                })
                flagged.update([a.id, b.id])
        ...
        if a_cohorts:
            for tt in _program_hits_main(a_cohorts, a.day, a.start_time, a.end_time):
                student_conflicts.append({
                    ...
                    "cohort": _cohort_label(
                        _shared_cohorts(a_cohorts, {_main_alloc_cohort(tt.course_allocation)})
                    ),
                    "message": (
