...
class TimetableIssue(models.Model):
    """
    Persistent record of *why* a Timetable entry was placed somewhere even
    though it wasn't a clean fit — e.g. the autoscheduler had to force a
    collision because no free slot existed, or an admin used "Force
    Save/Move (Override)" to push a course into a venue that's too small
    (or absurdly oversized) for it.

    This is deliberately separate from the live conflict checker
    (`_check_conflicts` / `timetable_conflicts_api`), which only tells you
    about problems that are STILL true right now. This model remembers the
    *history* of a forced placement even if, later, the thing it originally
    collided with gets moved away — the placement was still a compromise
    the scheduler/admin made, and that's worth keeping visible until the
    entry itself is moved (at which point the row is re-evaluated and the
    record is cleared or replaced for its new slot).

    Rows are keyed on (timetable_entry, issue_type) so a single cell can
    carry more than one distinct problem (e.g. both an oversized-venue
    mismatch AND a forced lecturer collision) without them overwriting each
    other, while re-recording the same issue_type for the same entry is a
    plain upsert.

    Writes to this table are always done off the request/response path
    (see `timetable/issue_tracking.py`) so recording or clearing an issue
    never adds latency to a save/move/autoschedule API call.
    """

    class IssueType(models.TextChoices):
        CAPACITY_OVER = "capacity_over", "Over venue capacity"
        CAPACITY_UNDER = "capacity_under", "Venue much bigger than class"
        VENUE_COLLISION = "venue_collision", "Venue double-booked"
        LECTURER_COLLISION = "lecturer_collision", "Lecturer double-booked"
        PROGRAM_COLLISION = "program_collision", "Program/year overlap"
        OTHER = "other", "Other override"

    class Severity(models.TextChoices):
        WARNING = "warning", "Warning"
        ERROR = "error", "Error (forced through)"

    class Source(models.TextChoices):
        AUTOSCHEDULER = "autoscheduler", "Autoscheduler"
        MANUAL_OVERRIDE = "manual_override", "Manual override"
        SYSTEM = "system", "System"

    timetable_entry = models.ForeignKey(
        "timetable.Timetable", on_delete=models.CASCADE, related_name="issues"
    )
    ...
    class Meta:
        ...
        indexes = [
            models.Index(fields=["timetable_entry"]),
        ]

    def __str__(self):
        return f"[{self.issue_type}] tt#{self.timetable_entry_id}: {self.message[:60]}"
...
class ExamTimetable(models.Model):
    ...
    start_time = models.TimeField()
    end_time = models.TimeField()
    allocated_students = models.PositiveIntegerField(
        null=True, blank=True,
        help_text="How many of course_allocation's students sit THIS venue "
...
# ===========================================================================
#  ANALYSIS DASHBOARD — VIEW-ACCESS SHARING
# ===========================================================================
# /timetable/analysis/ (timetable/analysis_reports.py) is normally
# SUDO/Director/Timetable-Admin only. This is a single, global on/off
# switch that lets one of those roles "share" read-only access to the
# dashboard with COD and/or COD Admin accounts — either every department's
# (apply_to_all_departments=True), or a chosen subset (the `departments`
# list). A shared COD/COD Admin never gets the free "any department" choice
# a real timetable admin has: their view is always pinned to their OWN
# department (see analysis_reports._can_access_analysis /
# _effective_department_id), even when the share covers every department.
class AnalysisAccessShare(models.Model):
    ...
    def __str__(self):
        if not self.is_enabled:
            return "Analysis dashboard sharing: OFF"
        who = []
        if self.share_with_cod:
            who.append("COD")
        if self.share_with_cod_admin:
            who.append("COD Admin")
        scope = "all departments" if self.apply_to_all_departments else "selected departments"
        return f"Analysis dashboard shared with {', '.join(who) or 'nobody'} ({scope})"

    @classmethod
    def get_solo(cls):
        """There is exactly one row (pk=1). Created on first access with
        every switch off, i.e. today's behaviour, so simply having this
        feature deployed changes nothing until an admin opts in."""
