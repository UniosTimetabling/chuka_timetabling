...
@admin.register(AnalysisAccessShare)
class AnalysisAccessShareAdmin(admin.ModelAdmin):
    """Fallback editor for the /timetable/analysis/ sharing config — the
    normal editing surface is the Sharing panel on the dashboard itself
    (analysis_reports.analysis_sharing_settings_api / update_...), this is
    just here for direct DB-admin access if that panel is ever unreachable."""
    list_display = ('__str__', 'is_enabled', 'share_with_cod', 'share_with_cod_admin',
                     'apply_to_all_departments', 'updated_by', 'updated_at')
    filter_horizontal = ('departments',)
    readonly_fields = ('updated_at',)

    def has_add_permission(self, request):
        # Singleton — only ever one row (AnalysisAccessShare.get_solo()).
        return not AnalysisAccessShare.objects.exists()
