...
def is_scheduling_exempt(alloc_a, alloc_b) -> bool:
    ...
    1. SpecializationStem takes priority over everything else, checked
       ...
       "shares at least one stem" / "shares no stem"):
       - shares a stem          → NEVER exempt (every pair within a stem must
         still clash-check, even if one course is also flagged elective).
       - no shared stem         → exempt. A student only ever follows ONE
         combination/specialization stem, so two courses that each belong
         to a stem but not the SAME stem can never both be taken by one
         student — this holds regardless of which SpecializationCategory
         each stem sits under (different combinations are sometimes set up
         as separate categories rather than alternatives within a single
         category, and the exemption must not depend on that data-entry
         detail). Only an actual shared stem is ever flagged.
    ...
    # Rule 0.9: NESTED ELECTIVE GROUP — two alternatives of the same pick-one
    # SelectionGroup are never both taken by one student, so they may share a
    # slot even when both are members of the same stem. Checked BEFORE the
    # shared-stem "never exempt" rule; alternative vs core unit of the stem
    # shares no pool and still clash-checks.
    ...
    if stems_a and stems_b:
        ...
        return True  # both stem-bound, different stem(s) -> exempt
        # (no category check: a student only ever follows one combination/
        # stem, whether or not that stem's alternatives were entered under
        # the same SpecializationCategory record.)

    # Rule 2: StudentGroup — different, explicitly-set groups only
    # Full mapped-group sets (primary + additional_student_groups + groups a
    # restricted elective pool is mapped to), not just the primary FK.
    ...
    if (sgrp_a is not None and sgrp_b is not None and sgrp_a != sgrp_b
            and not (_exh.effective_group_ids(alloc_a) & _exh.effective_group_ids(alloc_b))):
        return True
        ...
        for timeslot_key, entries in schedule_map.items():
            ...
            for prog_key, prog_entries in prog_groups.items():
                if len(prog_entries) > 1:
                    try:
                        real_ids = set()
                        guaranteed_stem_collision_ids = set()
                        for i, ei in enumerate(prog_entries):
                            for j, ej in enumerate(prog_entries):
                                ...
                                if not is_scheduling_exempt(ei.course_allocation, ej.course_allocation):
                                    real_ids.add(ei.id)
                                    real_ids.add(ej.id)
                                    # Rule 1 of is_scheduling_exempt (see above) only
                                    # returns "not exempt" for a stem-bound pair when
                                    # they actually SHARE a stem id — different stems
                                    # are always exempt now, with no fallthrough. So a
                                    # flagged pair that both belong to stems and share
                                    # one is a GUARANTEED collision (same requirement,
                                    # same cohort), never something to "verify".
                                    stems_i = _specialization_stem_ids_of(ei.course_allocation)
                                    stems_j = _specialization_stem_ids_of(ej.course_allocation)
                                    if stems_i & stems_j:
                                        guaranteed_stem_collision_ids.add(ei.id)
                                        guaranteed_stem_collision_ids.add(ej.id)
                        ...
                        # Severity: 'stem' (softer, "verify" yellow wording) is ONLY
                        # for a case where every colliding course is stem-bound but
                        # NONE of the flagged pairs actually share a stem id (a
                        # fallthrough via some other, non-stem exemption gap) — that's
                        # genuinely worth a human double-check. The moment any flagged
                        # pair shares an ACTUAL stem id, that pair is a guaranteed
                        # collision by construction (see is_scheduling_exempt Rule 1)
                        # and must stay 'hard' (red), never "verify: students only
                        # pick one stem" — there's nothing to verify, they share the
                        # exact same required stem. A plain shared/mandatory course
                        # (no stem membership at all) among the colliders is likewise
                        # always 'hard'.
                        has_guaranteed_stem_collision = bool(guaranteed_stem_collision_ids)
                        is_stem_conflict = (
                            not has_guaranteed_stem_collision
                            and all(_alloc_in_any_stem(e.course_allocation) for e in real_conflicts)
                        )
                        ...
                        if has_guaranteed_stem_collision:
                            stem_note = f' — specialization(s): {", ".join(stem_names)}' if stem_names else ''
                            message = (
                                f'{subject} has courses sharing the same specialization stem '
                                f'scheduled at the same time{stem_note}'
                            )
                        elif is_stem_conflict:
                            stem_note = f' — specialization(s): {", ".join(stem_names)}' if stem_names else ''
                            message = (
...
def _check_conflicts(allocation, venue_input, day, start_t, end_t, force_override=False, request=None):
    """
    Optimized conflict checking with batched queries and smart exemption rules.

    Concurrent Allocation Sets: every overlap query below (program, lecturer,
    venue) is scoped via apply_tt_scope() to whichever AllocationSet(s) are
    active for this session — the same helper/prefix used by
    timetable_conflicts_api and the panel's display queries. Without this,
    placing a course in one department's/set's active timetable could be
    wrongly blocked (or wrongly allowed) by rows that belong to a
    completely different, currently-inactive allocation set. `request=None`
    (e.g. a caller that hasn't been updated) falls back to the same
    "eligible" gate apply_tt_scope always uses when nothing's been ticked.
    ...
    if allocation.program and alloc_year is not None:
        program_overlaps = apply_tt_scope(
        ...
        for existing in program_overlaps:
            ...
            if is_scheduling_exempt(allocation, existing.course_allocation):
                ...
                if time_overlaps(start_t, end_t, existing.start_time, existing.end_time):
                    ...
                    elif _exh.share_pool(allocation, existing.course_allocation):
                        reason = "alternatives of the same elective (pick-one) group — a student takes only one"
                    elif stems_a and stems_b and not (stems_a & stems_b):
                        reason = "different specialization stems (a student only ever follows one)"
    ...
    if allocation.lecturer:
        lecturer_overlaps = apply_tt_scope(
            Timetable.objects.filter(
...
def _check_conflicts_for_members(members, venue_input, day, start_t, end_t, force_override=False, request=None):
    """
    ...
    for member in members:
        msgs, blocked = _check_conflicts(member, venue_input, day, start_t, end_t, force_override=force_override, request=request)
...
def _finalize_timetable_save(allocation, merged_allocations, venue_input, day, start_t, end_t, messages_list):
    ...
    if entries_to_create:
        ...
        # Persist *why* this placement needed a force-override, if it did —
        # off the request path so the save response isn't slowed down.
        # See timetable/issue_tracking.py. A clean save (no ⚠️/❌ in
        # messages_list) is a no-op clear, which is correct: these are
        # brand-new rows so there's nothing stale to clear anyway.
        from .issue_tracking import sync_issues_for_bundle
        sync_issues_for_bundle(
            [e.id for e in created_entries], messages_list,
            source="manual_override", replace=False,
        )
...
def _get_unscheduled_allocations(request=None):
    """
    ...
    Also scoped by the TT AllocationSet cohort/mode chosen on
    /timetable/dashboard/ (see course_allocation.allocation_scope) — pass
    the current `request` so this only returns allocations belonging to a
    set that's in-scope for this session (defaults to the "eligible" gate
    if no request is given, e.g. from a background thread).
    """
    ...
    qs = apply_tt_scope(qs, request=request)
    return qs.order_by('department__faculty__name', 'department__name', 'course_code')


def _get_unscheduled_evening_weekend_allocations(request=None):
    """
    ...
    Also scoped by the TT AllocationSet cohort/mode — see
    _get_unscheduled_allocations() above.
    """
    timetable_ids = set(
        Timetable.objects.filter(
            venue__isnull=False,
...
def _get_zero_student_allocations(request=None):
    """
    ...
    qs = apply_tt_scope(qs, request=request)
    return qs.order_by('department__name', 'course_code')
...
def resolve_unscheduled_courses(request):
    ...
    unscheduled = list(
        _get_unscheduled_allocations(request).select_related(
...
def _build_full_state(exclude_ids=None, request=None):
    """Fresh _ResolveState built from every current Timetable row (scoped to
    the active allocation set(s) when `request` is given — see
    apply_tt_scope), skipping any row id in `exclude_ids` (used to “lift” an
    entry out of the grid before searching for somewhere new to put it)."""
...
def _find_alt_slot_for_row(cand, day_pools, all_venues, specialized_codes, designated_keys_by_venue, request=None):
    ...
    tmp_state = _build_full_state(exclude_ids=sibling_ids, request=request)
...
def _try_swap_for_row(cand, partner_pool, request=None):
    ...
    for partner in partner_pool:
        ...
        if tried > 150:
            break
        tmp_state = _build_full_state(exclude_ids={cand.id, partner.id}, request=request)
    ...
    for conflict_type, rows in conflict_groups:
        ...
        for cand in candidates:
            if not cand.course_allocation or cand.course_allocation.id in already_targeted_alloc_ids:
                continue
            changes = _find_alt_slot_for_row(cand, day_pools, all_venues, specialized_codes, designated_keys_by_venue, request=request)
            if not changes:
                continue
        ...
        for cand in candidates:
            ...
            partner_pool = []
            if getattr(alloc, "lecturer_id", None):
                partner_pool += list(apply_tt_scope(
                    Timetable.objects.select_related(
                        "venue", "course_allocation", "course_allocation__lecturer", "course_allocation__program"
            ...
            changes = _try_swap_for_row(cand, partner_pool, request=request)
            if not changes:
                continue
    ...
    for p in proposals:
        ...
        try:
            with transaction.atomic():
                ...
                fresh_state = _build_full_state(exclude_ids=set(row_ids), request=request)
                venue_cache = {}
...
def timetable_panel(request):
    ...
    unschedule_allocations    = _get_unscheduled_allocations(request)
    ew_unschedule_allocations = _get_unscheduled_evening_weekend_allocations(request)
    ...
    zero_student_count        = _get_zero_student_allocations(request).count()
    ...
    # ── Lab Timetable (embedded table below the regular timetable) ─────────
    from timetable.lab_panel_ops import get_lab_panel_context
    context.update(get_lab_panel_context(request))
...
def _load_full_timetable_data(request):
    ...
    # Optimized timetables query — Concurrent Allocation Sets: scoped to
    # whichever AllocationSet(s) are active for this session, same helper
    # and prefix as main_timetable_view.py / timetable_conflicts_api, so
    # the panel grid never mixes another department's/set's rows in.
    timetables = apply_tt_scope(
        Timetable.objects.select_related(
            'course_allocation__lecturer',
            'course_allocation__program',
...
def load_timetable_data(request):
    ...
    try:
        ...
        if stage == 'initial':
            ...
            # CRITICAL FIX: Get ALL venues for timetable rendering
            # Concurrent Allocation Sets: scoped to the active set(s), same
            # as the 'timetables' stage below, so venue derivation and the
            # timetable_count stat agree with what the grid actually shows.
            venue_ids_from_timetables = apply_tt_scope(
                Timetable.objects.filter(venue__isnull=False),
                request=request,
                prefix='course_allocation__allocation_set',
            ).values_list('venue_id', flat=True).distinct()
            ...
            unscheduled_count     = _get_unscheduled_allocations(request).count()
            ew_unscheduled_count  = _get_unscheduled_evening_weekend_allocations(request).count()
            zero_student_count    = _get_zero_student_allocations(request).count()
            ...
            # Load timetable data (40%) — Concurrent Allocation Sets: scoped
            # to whichever AllocationSet(s) are active for this session, same
            # as the other display/conflict query sites in this file. This is
            # the actual query that populates the visible grid, so an
            # unscoped `.all()` here is exactly what let another department's/
            # set's rows show up mixed into the panel.
            timetables = apply_tt_scope(
                Timetable.objects.select_related(
                    'course_allocation__lecturer',
                    'course_allocation__program',
            ...
            unscheduled = _get_unscheduled_allocations(request)
            # Evening/weekend flagged courses
            ew_unscheduled = _get_unscheduled_evening_weekend_allocations(request)
            ...
            venue_data = []
            for venue in venues:
                # Get all timetable entries for this venue — scoped to the
                # active allocation set(s), so utilization stats match what
                # the grid actually shows for this session.
                venue_timetables = apply_tt_scope(
                    Timetable.objects.filter(venue=venue),
                    request=request,
                    prefix='course_allocation__allocation_set',
                ).count()
                venue_data.append({
...
def optimize_venues_per_timeslot(request):
    ...
    try:
        ...
        # ── 1b. Collect CombinedCourseGroup membership ─────────────────────
        # These are DIFFERENT from AutoMergedExamGroup above (that's the exam
        # side). A CombinedCourseGroup is several courses taught together in
        # ONE shared room on the regular timetable — the scheduler already
        # gives every member allocation the same day/time/venue, so multiple
        # Timetable rows legitimately sharing one venue is not a real
        # double-booking when they're all members of the same group. The
        # optimiser must treat the whole group as a single movable unit:
        # sized by the group's TOTAL student count, and if it moves, every
        # member row moves together — never splitting the group across venues.
        combined_group_of_alloc = {}   # course_allocation_id -> group id
        combined_group_label    = {}   # group id -> group_code (for reporting)

        for g in CombinedCourseGroup.objects.prefetch_related('allocations').only(
            'id', 'group_code'
        ):
            for alloc_id in g.allocations.values_list('id', flat=True):
                combined_group_of_alloc[alloc_id] = g.id
            combined_group_label[g.id] = g.group_code

        print(f"DEBUG optimize: {len(combined_group_of_alloc)} allocations belong to "
              f"{len(combined_group_label)} CombinedCourseGroups")
        ...
        for (day, start_str, end_str, vcode), rows in venue_slot_occupants.items():
            ...
            # Not a real conflict if every distinct allocation sharing this
            # venue/slot belongs to the SAME CombinedCourseGroup — that's a
            # legitimate shared room, not a double-booking.
            group_ids = {combined_group_of_alloc.get(aid) for aid in distinct_allocs}
        ...
        #
        # The optimiser works on "units", not raw rows. A unit is either:
        #   - a single course's Timetable row, or
        #   - every row belonging to the same CombinedCourseGroup in this
        #     slot, bundled together — sized by their COMBINED student
        #     total, and always moved/swapped as one block so the group
        #     never gets split across two venues (which is itself a form
        #     of double-booking / broken shared-room arrangement).
        #
        # Swaps are preferred over moving into some other free venue — a
        # 150-student course/group in a small room and a 93-student
        # course/group in a bigger room simply trade places. A genuinely
        # EMPTY venue (nobody using it at that day/time at all) is only
        # used when no workable swap partner exists in the slot.
        swaps                   = []
        updates                 = []
        skipped_specialization  = []   # informational list of pinned cells
        unresolved_conflicts    = []   # units that couldn't be placed at all
        group_venue_mismatches  = []   # combined-group members found split across venues (data anomaly)

        # Every venue physically occupied at a given (day,start,end) slot —
        # by ANYONE (locked, merged, everyone) — used only to verify a venue
        # is truly empty before handing it out as a fallback below.
        all_slot_occupied = defaultdict(set)
        for r in all_rows:
            if r.venue:
                all_slot_occupied[(r.day or '', r.start_time.strftime('%H:%M'),
                                    r.end_time.strftime('%H:%M'))].add(r.venue.code)

        all_venues_list = list(VenueModel.objects.all().only('id', 'code', 'capacity'))

        class _Unit:
            """One movable thing in a slot: a lone course, or a whole
            CombinedCourseGroup's rows bundled together."""
            __slots__ = ('rows', 'group_id', 'venue', 'students', 'ccodes', 'label')

            def __init__(self, rows, group_id):
                self.rows     = rows
                self.group_id = group_id
                self.venue    = rows[0].venue
                self.students = sum((r.course_allocation.number_of_students or 0) for r in rows)
                self.ccodes   = sorted({
                    (r.course_allocation.course_code or '?').strip().upper() for r in rows
                })
                self.label = combined_group_label.get(group_id) if group_id else None
                if not self.label:
                    self.label = '+'.join(self.ccodes) if self.ccodes else '?'

        for (day, start_str, end_str), slot_rows in slot_groups.items():
            if len(slot_rows) < 2:
                continue
            slot3 = (day, start_str, end_str)

            # ── Exclude per-cell locked rows and merged-group-venue rows. ──
            excluded_row_ids          = set()
            locked_group_ids_this_slot = set()
            for r in slot_rows:
                if not r.venue:
                    excluded_row_ids.add(r.id)
                    continue
                ccode = (r.course_allocation.course_code or '?').strip().upper() if r.course_allocation else '?'
                if r.id in locked_entry_ids:
                    excluded_row_ids.add(r.id)
                    ...
                    gid = combined_group_of_alloc.get(r.course_allocation_id)
                    if gid:
                        locked_group_ids_this_slot.add(gid)
                elif r.venue.code in merged_venues:
                    excluded_row_ids.add(r.id)

            # If ANY member of a combined group is locked/frozen this slot,
            # the WHOLE group must stay put — otherwise we'd split it across
            # two venues, which is exactly the kind of conflict we're trying
            # to eliminate.
            for r in slot_rows:
                if r.id in excluded_row_ids or not r.venue:
                    continue
                gid = combined_group_of_alloc.get(r.course_allocation_id)
                if gid in locked_group_ids_this_slot:
                    excluded_row_ids.add(r.id)
                    skipped_specialization.append({
                        'course_code': (r.course_allocation.course_code or '?').strip().upper(),
                        'venue':       r.venue.code,
                        'day':         day,
                        'slot':        f'{start_str}–{end_str}',
                        'reason':      'part of a combined group with a specialization-locked member — whole group left in place',
                    })

            usable_rows = [r for r in slot_rows if r.venue and r.id not in excluded_row_ids]

            # ── Bundle combined-group rows into single units. ──────────────
            ...
            for r in usable_rows:
                ...
                if gid:
                    ...
                    candidates.append(_Unit([r], None))

            for gid, rows in rows_by_group.items():
                vcodes = {r.venue.code for r in rows}
                if len(vcodes) > 1:
                    # Group members are inexplicably in different venues
                    # this slot already — a pre-existing data issue, not
                    # something this pass caused. Leave them untouched
                    # rather than guess, and report it for manual review.
                    group_venue_mismatches.append({
                        'group':  combined_group_label.get(gid, str(gid)),
                        'day':    day,
                        'slot':   f'{start_str}–{end_str}',
                        'venues': sorted(vcodes),
                    })
                    continue
                candidates.append(_Unit(rows, gid))

            if len(candidates) < 2:
                continue

            resolved             = set()   # id(unit) already handled this slot
            claimed_free_venues  = set()   # empty venues claimed this run, this slot

            def _forced_out(unit):
                # Sitting in a reserved-only (exclusive) venue it has no
                # right to — must be relocated regardless of size fit.
                return unit.venue.code in exclusive_venue_codes

            def _urgency(unit):
                forced  = 0 if _forced_out(unit) else 1
                deficit = unit.students - (unit.venue.capacity or 0)
                return (forced, -deficit)

            ordered = sorted(candidates, key=_urgency)

            for unit in ordered:
                if id(unit) in resolved:
                    continue

                students     = unit.students
                cur_vcode    = unit.venue.code
                cur_capacity = unit.venue.capacity or 0
                forced       = _forced_out(unit)

                # Nothing to do — unit already fits its current venue and
                # isn't sitting somewhere it's forbidden to be.
                if not forced and students <= cur_capacity:
                    resolved.add(id(unit))
                    continue

                # ── 1) Prefer a direct SWAP with another unit in this same
                #        slot over touching any other venue. ─────────────
                # Two passes: first look only for a partner sitting in a
                # venue DESIGNATED to one of this unit's courses (reunites
                # it with its soft-reservation home, e.g. Nursing); only
                # if none exists do we accept any size-compatible partner.
                partner = None
                for prefer_designated in (True, False):
                    for other in candidates:
                        if other is unit or id(other) in resolved:
                            continue
                        other_vcode = other.venue.code
                        if other_vcode in exclusive_venue_codes:
                            continue   # never hand out a reserved-only venue
                        if prefer_designated and not (
                            set(unit.ccodes) & venue_designated_codes.get(other_vcode, set())
                        ):
                            continue
                        if (other.venue.capacity or 0) < students:
                            continue   # other's room can't fit this unit either
                        if (not forced) and other.students > cur_capacity:
                            continue   # swapping would just break the partner too
                        partner = other
                        break
                ...
                if partner is not None:
                    old_venue, new_venue = unit.venue, partner.venue
                    swaps.append({
                        'day': day, 'slot': f'{start_str}–{end_str}',
                        'courses': unit.label, 'from_venue': old_venue.code,
                        'to_venue': new_venue.code, 'students': students,
                        'venue_capacity': new_venue.capacity or '—',
                        'swapped_with': partner.label,
                        'group_members': unit.ccodes if unit.group_id else None,
                    })
                    for r in unit.rows:
                        r.venue = new_venue
                        updates.append(r)
                    for r in partner.rows:
                        r.venue = old_venue
                        updates.append(r)
                    resolved.add(id(unit))
                    resolved.add(id(partner))
                    continue

                # ── 2) No swap partner — fall back to a venue that is
                #        genuinely EMPTY for this exact slot. ────────────
                occupied_now = (
                    all_slot_occupied[slot3] | claimed_free_venues
                    | merged_venues | exclusive_venue_codes
                )
                free_options = [
                    v for v in all_venues_list
                    if v.code not in occupied_now and (v.capacity or 0) >= students
                ]
                # Designated-home venues bubble to the top; ties broken by
                # smallest adequate capacity (best fit, least wasted space).
                free_options.sort(
                    key=lambda v: (
                        0 if (set(unit.ccodes) & venue_designated_codes.get(v.code, set())) else 1,
                        v.capacity or 0,
                    )
                )

                if free_options:
                    new_venue = free_options[0]
                    swaps.append({
                        'day': day, 'slot': f'{start_str}–{end_str}',
                        'courses': unit.label, 'from_venue': cur_vcode,
                        'to_venue': new_venue.code, 'students': students,
                        'venue_capacity': new_venue.capacity or '—',
                        'group_members': unit.ccodes if unit.group_id else None,
                    })
                    for r in unit.rows:
                        ...
                        updates.append(r)
                    resolved.add(id(unit))
                    claimed_free_venues.add(new_venue.code)
                    continue

                # ── 3) Nothing available at all — leave it and report it. ──
                resolved.add(id(unit))
                unresolved_conflicts.append({
                    'course_code':   unit.label,
                    'group_members': unit.ccodes if unit.group_id else None,
                    'venue':         cur_vcode,
                    'day':           day,
                    'slot':          f'{start_str}–{end_str}',
                    'students':      students,
                    'reason':        (
                        'sitting in a reserved-only venue with no swap partner or free '
                        'venue available' if forced else
                        'oversized for its venue but no swap partner or free venue available'
                    ),
                })
                print(f"DEBUG optimize: UNRESOLVED — {unit.label} left in {cur_vcode} "
                      f"({day} {start_str}-{end_str}) — no swap/free venue found")
        ...
        if unresolved_conflicts:
            extra_notes.append(
                f'{len(unresolved_conflicts)} course(s)/group(s) could not be given a free venue '
                f'in their slot (not enough eligible venues) — see unresolved_conflicts.'
            )
        if group_venue_mismatches:
            extra_notes.append(
                f'{len(group_venue_mismatches)} combined group(s) had members already split '
                f'across different venues in the same slot — left untouched, see '
                f'group_venue_mismatches.'
            )
        ...
        return JsonResponse({
            ...
            'unresolved_conflicts':   unresolved_conflicts,
            'group_venue_mismatches': group_venue_mismatches,
        })
