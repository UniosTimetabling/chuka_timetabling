...
class LocalSearchHealer:
    ...
    def heal_all(days, slots, all_venues, cache, lecturer_blocked=None):
        ...
        # 1. Heal Capacity Overflows via venue swaps
    ...
    def _heal_capacity_overflows(days, slots, all_venues, cache):
        ...
        for slot_key, slot_list in slot_entries.items():
            # Find overflows in this slot
            overflows = [e for e in slot_list if e.course_allocation and e.venue and 
                         (e.course_allocation.number_of_students or 0) > (e.venue.capacity or 0)]
            if not overflows:
                continue
                
            # Find underflows (classes in large rooms with few students)
            underflows = [e for e in slot_list if e.course_allocation and e.venue and 
                          (e.course_allocation.number_of_students or 0) <= (e.venue.capacity or 0)]
            
            for overflow_entry in overflows:
                needed = overflow_entry.course_allocation.number_of_students or 0
                current_venue = overflow_entry.venue
                
                # Try to swap with an underflow entry
                for underflow_entry in underflows:
                    if underflow_entry.id == overflow_entry.id:
                        continue
                    other_needed = underflow_entry.course_allocation.number_of_students or 0
                    other_venue = underflow_entry.venue
                    
                    # Check if swap is valid (both fit in each other's rooms)
                    if (other_venue.capacity or 0) >= needed and (current_venue.capacity or 0) >= other_needed:
                        try:
                            with transaction.atomic():
                                overflow_entry.venue, underflow_entry.venue = underflow_entry.venue, overflow_entry.venue
                                overflow_entry.venue_id, underflow_entry.venue_id = underflow_entry.venue_id, overflow_entry.venue_id
                                overflow_entry.save()
                                underflow_entry.save()
                            healed += 1
                            safe_print(f"[Healer/Cap] Swapped {overflow_entry.course_allocation.course_code} "
                                       f"and {underflow_entry.course_allocation.course_code} venues to fix capacity.")
                            break
                        except Exception as e:
                            safe_print(f"[Healer/Cap] Error swapping: {e}")
        return healed
...
def run_optimized_autoscheduler_thread(disabled_constraints: Optional[Set[str]] = None):
    ...
    try:
        ...
        all_courses = list(CourseAllocation.objects.select_related(
            ...
            # Concurrent Allocation Sets: not wired into any live view yet
            # (kept here for whenever this "final version" replaces
            # regular_timetable_autosheduler_algorithm.py), but patched
            # with the same input-side gate as that file now has: only
            # schedule allocations whose own AllocationSet has been
            # submitted to the Timetabling Office (or has no set yet /
            # is a legacy set). See that file's history for why.
            Q(allocation_set__isnull=True)
            | Q(allocation_set__is_legacy=True)
            | Q(allocation_set__status=AllocationSet.STATUS_SUBMITTED_TO_TT)
        ))
