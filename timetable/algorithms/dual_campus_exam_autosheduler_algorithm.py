...
from course_allocation.allocation_scope import resolve_tt_scope, tt_scope_q
...
from course_allocation.exemption_helpers import share_pool as _exh_share_pool
...
def exam_is_collision_exempt(c1, c2) -> bool:
    ...
    # NESTED ELECTIVE GROUP: alternatives of the same pick-one pool are never
    # both sat by one student — exempt even when both are in the same stem.
...
# Concurrent Allocation Sets: same mechanism as the single-campus exam
# scheduler (see exam_timetable_autosheduler_algorith.py). Set once at
# the top of run_dual_campus_exam_scheduler(); every DB read below that
# rebuilds in-memory occupancy/conflict state goes through _temp_qs()
# instead of `ExamTempTimetable.objects` directly, so a leftover draft
# row from a DIFFERENT AllocationSet — never wiped by
# clear_exam_temp_tables() because that function deliberately only
# clears the CURRENT set's own rows — can't be mistaken for real
# occupancy and force an unnecessary split.
...
def sync_scheduled_ids_from_db(scheduled_ids: Set[int]):
    db_ids = set(
        _temp_qs().values_list("course_allocation_id", flat=True).distinct()
    )
...
def rebuild_state_from_db(state: SchedulerState, all_courses: List):
    ...
    course_by_id = {c.id: c for c in all_courses}
    entries = list(
        _temp_qs().values("course_allocation_id", "venue_id", "date", "start_time")
    )
...
def _build_shared_venue_exam_groups() -> int:
    ...
    entries = list(
        _temp_qs().select_related("course_allocation", "venue")
...
def clear_exam_temp_tables(tt_scope: Optional[dict] = None):
    """
    Wipes the dual-campus scheduler's draft tables before a rebuild.

    Concurrent Allocation Sets: `CampusCourseAllocation` (the satellite-
    campus model) has no `allocation_set` FK at all — it isn't part of the
    AllocationSet system — so `CampusExamTempTimetable` is always cleared
    in full, same as before. `ExamTempTimetable` and `MergedCourseGroup`
    DO reach a main-campus `CourseAllocation` (via `course_allocation` /
    `base_course`), so — mirroring `clear_exam_tables_safely()` in
    exam_timetable_autosheduler_algorith.py — when `tt_scope` identifies
    specific AllocationSet(s), only rows belonging to those sets are
    wiped; every other department's/set's draft rows are left untouched.
    `tt_scope=None` (an un-updated caller) falls back to the old full-wipe
    behaviour so nothing relying on it changes.
    """
    with transaction.atomic():
        exam_temp_qs = ExamTempTimetable.objects.all()
        merged_qs = MergedCourseGroup.objects.filter(published=False)
        shared_qs = SharedVenueExamGroup.objects.all()
        if tt_scope is not None:
            exam_temp_qs = exam_temp_qs.filter(
                tt_scope_q(tt_scope, prefix="course_allocation__allocation_set"))
            merged_qs = merged_qs.filter(
                tt_scope_q(tt_scope, prefix="base_course__allocation_set"))
            shared_ids = list(
        ...
        count1 = exam_temp_qs.count()
        count2 = CampusExamTempTimetable.objects.count()
        count3 = merged_qs.count()
        count4 = shared_qs.count()
    ...
    scoped_note = " (scoped to active allocation set(s))" if tt_scope else ""
    safe_print(f"Exam temp tables cleared{scoped_note}: ExamTemp({count1}), CampusExamTemp({count2}), "
               f"Merged({count3}), SharedVenue({count4})")
    log_info(f"Cleared temp tables{scoped_note}: ExamTemp({count1}), CampusExamTemp({count2}), "
             f"Merged({count3}), SharedVenue({count4})")
...
# ─────────────────────────────────────────────────────────
def run_dual_campus_exam_scheduler(tt_scope: Optional[dict] = None):
    """
    `tt_scope`: a JSON-serializable snapshot from
    course_allocation.allocation_scope.resolve_tt_scope(request) — the
    AllocationSet(s) the TT ticked on /timetable/dashboard/, resolved on
    the request thread BEFORE this is handed off to a background thread
    (which has no request/session of its own). Defaults to the "eligible"
    fallback gate when not given. Only `main_allocs` (main-campus
    CourseAllocation rows) are scoped by it — CampusCourseAllocation has
    no allocation_set concept, so campus allocations are unaffected.
    """
    global _log_file_handle
    ...
    try:
        ...
        clear_exam_temp_tables(tt_scope)
        ...
        main_config = ExamSchedulerConfig.objects.filter(pk=1).first()
        if not main_config:
            with _progress_lock:
        ...
        # Concurrent Allocation Sets: narrow to whichever AllocationSet(s)
        # the TT ticked on /timetable/dashboard/ (falls back to the
        # "eligible" gate — no set yet, legacy, or submitted to TT — if
        # nothing's been picked this session), same as the main-campus
        # exam autoscheduler and the regular timetable autoscheduler.
        ...
        ).filter(tt_scope_q(tt_scope), submitted_to_tt=True))
        if not main_allocs:
            main_allocs = list(CourseAllocation.objects.select_related(
                "program", "lecturer", "department", "program_course", "selection_group",
                "specialization_stem", "specialization_stem__category"
            ).filter(tt_scope_q(tt_scope), approved_by_dvc=True))
...
class StartDualExamSchedulerView(View):
    def post(self, request):
        ...
        if tt_scope and tt_scope.get("type") == "sets":
            log_info(f"Scoped to AllocationSet id(s) {tt_scope['ids']} "
                     f"(ticked on the timetable dashboard).")
        else:
            log_warning(
                "No specific AllocationSet is ticked on the timetable "
                "dashboard — running against the default 'eligible' pool "
                "(no allocation_set, legacy sets, and sets already "
                "submitted to TT, across ALL departments). To schedule "
                "only one department's/set's exams, tick that set on "
                "/timetable/dashboard/ first, then re-run."
            )
        threading.Thread(
            target=run_dual_campus_exam_scheduler, args=(tt_scope,), daemon=True
        ).start()
