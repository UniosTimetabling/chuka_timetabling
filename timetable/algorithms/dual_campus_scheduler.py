...
_SEC_BASE_RE = re.compile(r"^\s*([A-Za-z]+)\s*(\d+)")


def _same_base_course(alloc_a, alloc_b) -> bool:
    """Sections of ONE course (COSC 103-A / -A2 / -B ...) partition the
    cohort's students, so they may share a slot. This scheduler used to lack
    the rule the regular/stable schedulers and the panel already have."""
    ca = getattr(alloc_a, 'course_code', None) or ''
    cb = getattr(alloc_b, 'course_code', None) or ''
    ma, mb = _SEC_BASE_RE.match(ca), _SEC_BASE_RE.match(cb)
    if not ma or not mb:
        return False
    return (ma.group(1).upper(), ma.group(2)) == (mb.group(1).upper(), mb.group(2))


def is_program_year_collision_exempt(alloc_a, alloc_b) -> bool:
    if _same_base_course(alloc_a, alloc_b):
        return True
    # NESTED ELECTIVE GROUP: same pick-one pool => never both taken, even when
    # both are also in the same stem (checked before the shared-stem rule).
    ...
    if sgrp_a is not None and sgrp_b is not None and sgrp_a != sgrp_b and not (_exh_group_ids(alloc_a) & _exh_group_ids(alloc_b)):
        return True
