...
class SchedulerState:
    ...
    def designated_venues_for_course(self, course) -> List[int]:
        ...
        for venue_ids, program_ids, course_ids in scopes:
            # A rule with NO program/course scope at all means "every
            # offering of this code" — not "no offering". Without this,
            # eligible was unconditionally False whenever program_ids and
            # course_ids were both empty (a code-only rule, e.g. a single
            # designated-lab course with no program restriction), so that
            # course silently got zero venues here, was skipped by
            # designated_venue_priority_pass (`if not venues: continue`),
            # never got strict_locked_ids set, and fell through to every
            # later general-placement phase with NO venue restriction at
            # all — the exact bug behind a "designated/blocked" course
            # (e.g. BLAW 412) turning up outside its locked venue.
            unscoped = not program_ids and not course_ids
            eligible = (
                unscoped
                or (pc_id is not None and pc_id in course_ids)
...
def schedule_small_courses_in_small_venues(all_courses, state, scheduled_ids):
    ...
    eligible = [
        ...
        # A designated-venue course (e.g. a small BLAW section under 100
        # students) must go through designated_venue_priority_pass first,
        # same as a family course. Without this exclusion, this phase runs
        # BEFORE Phase0-Designated, sees no reason to leave a "small"
        # course alone, and seats it in an arbitrary small venue — Phase0
        # then skips it (`c.id not in scheduled_ids` is already False) and
        # its designated-room rule never gets applied at all. This is why
        # the >=100-student BLAW sections correctly landed in their
        # designated room while the <100-student ones (Year 3, some Year
        # 4 sections) scattered into unrelated small rooms.
        and normalize_course_code(getattr(c, "course_code", "") or "") not in state.designated_venues_by_norm_code
    ]
