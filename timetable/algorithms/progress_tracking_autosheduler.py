...
from course_allocation.allocation_scope import resolve_tt_scope
...
def run_scheduler_in_thread(disabled_constraints=None, tt_scope=None):
    try:
        from .exam_timetable_autosheduler_algorith import run_optimized_autoscheduler_thread

        result       = run_optimized_autoscheduler_thread(disabled_constraints, tt_scope)
...
class StartSchedulingView(View):
    ...
    def post(self, request):
        ...
        if tt_scope and tt_scope.get("type") == "sets":
            _append_log(
                "info",
                f"Scoped to AllocationSet id(s) {tt_scope['ids']} "
            ...
            _append_log(
                ...
                "(every set with no allocation_set, every legacy set, and "
                "every set already submitted to TT, across ALL "
                "departments). If you only want ONE department's/set's "
                "exams scheduled in this run, go tick that set on "
        ...
        try:
            scheduler_thread = threading.Thread(
                target=run_scheduler_in_thread, args=(disabled_constraints, tt_scope),
