...
def is_program_year_collision_exempt(alloc_a, alloc_b) -> bool:
    ...
    # Rule 0.9: NESTED ELECTIVE GROUP — alternatives of the same pick-one pool
    # never share students, even when both sit in the same stem.
...
def run_basic_stable_scheduler_thread():
    ...
    try:
        ...
        all_courses = list(CourseAllocation.objects.select_related(
            ...
            # Concurrent Allocation Sets: only schedule allocations whose own
            # set has been submitted to the Timetabling Office (rows with no
            # set yet / legacy sets behave exactly as before this upgrade).
