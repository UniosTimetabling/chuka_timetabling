...
def _find_safe_relocation(
    ...
    `preferred_day_slot_pairs` / `preferred_venue_ids`, if given (the
    lecturer's stated LecturerTimePreference / LecturerVenuePreference),
    are tried FIRST within each safety tier — being forced to relocate a
    class is a chance to land it somewhere the lecturer actually wants,
    instead of anywhere merely safe. This never relaxes a safety tier just
    to chase a preference: if no preferred-and-safe slot exists at the
    current relaxation level, the search falls back to any safe slot at
    that SAME level before moving to a laxer one, exactly as before.
...
def resolve_venue_double_bookings(
    ...
    # Lecturer soft preferences (day/time + venue) — read once here so any
    # forced relocation this safety net makes can land on a preferred
    # slot/venue where possible, instead of undoing the work Phase 6.5
    # (apply_lecturer_soft_preferences_pass) already did earlier in the
    # pipeline. Phase 7B runs LAST — with no preference-aware pass after
    # it, any relocation it made blind to preferences would silently stick.
    try:
...
def _get_specialization_stem_ids(alloc) -> frozenset:
    """
    ALL SpecializationStem ids this allocation is a member of.

    IMPORTANT: reads the `specialization_stems` M2M (the real membership
    list — "the set of courses a student takes if they choose this
    stem"), NOT the singular `specialization_stem` FK. The FK is a
    "primary stem" pointer that the COD-panel views deliberately NULL OUT
    the moment a course is added to a SECOND stem (see
    course_allocation/specialization_stem_views.py) — a course shared
    between two stems of the same category (e.g. a MATH course required
    by both a 'Statistics' and an 'Accounting' stem) is a completely
    normal, common case, not an edge case. Reading only the FK for
    exemption purposes made every such course fall all the way through
    to "ordinary mandatory course", clashing with EVERY other stem's
    courses in that category — including stems it has nothing to do
    with — not just the two stems it's actually shared between. Reading
    the full M2M set instead lets the exemption logic below reason about
    exactly which stems a course belongs to, and exempt it from stems it
    is NOT a member of while still correctly clashing against the stems
    ...
    try:
        ...
        if stem_ids:
            return stem_ids
        # Defensive fallback: a row somehow has the legacy singular FK set
        # but no M2M membership (shouldn't happen once synced, but don't
        # silently lose the signal if it does).
...
def _get_specialization_category_ids(alloc, stem_ids: frozenset) -> frozenset:
    """SpecializationCategory ids covering every stem `alloc` belongs to."""
    if not stem_ids:
        return frozenset()
    try:
        cats = set()
        ...
        pass
    # Fallback path (legacy-FK case in _get_specialization_stem_ids above).
    try:
...
def _get_specialization_stem_id(alloc) -> Optional[int]:
    """Single-value compatibility shim over `_get_specialization_stem_ids`
    for the capacity-estimation heuristic below (`_effective_window_
    demand`), which just needs SOME representative stem id to bucket a
    course by for counting purposes — it doesn't decide real clash
    exemption, so collapsing a multi-stem course to one id here is a
    reasonable estimate, not the correctness-critical path. Actual
    clash-exemption uses the full set via `_get_specialization_stem_ids` /
    `_get_specialization_category_ids` in `is_program_year_collision_exempt`.
    """
...
def _get_specialization_category_id(alloc) -> Optional[int]:
    """Single-value compatibility shim — see `_get_specialization_stem_id`."""
    stem_ids = _get_specialization_stem_ids(alloc)
...
def _is_same_base_course_pair(alloc_a, alloc_b) -> bool:
    ...
    both simply coded 'COMS 101'. This is one course SPLIT into multiple
    sections/streams (typically because the cohort is too large for one
    venue/slot), not two different courses — every section teaches a
    disjoint subset of the same program-year's students, so sections
    running concurrently (same venue or different ones) is never a
    program-year collision, whether or not a StudentGroup was formally
    assigned to each section.

    Uses normalize_course_code_base(), which already strips both dash/
    underscore-style ('-A', '_C') and word-style ('GROUP C', 'GRP B',
    'SECTION A', bare trailing letter) section/stream tags, so it
    recognises a split regardless of how the section was labelled.

    Kept IN SYNC with `_is_same_base_course_pair` in timetable/timetable_panel.py.
    """
...
def is_program_year_collision_exempt(alloc_a, alloc_b) -> bool:
    # ── Same base course code — a split section of ONE course, not a
    # collision between two different courses. Checked first and wins
    # outright, overriding every other rule below (e.g. a specialization-
    # stem course that also got split into sections must still be exempt
    # against its own other sections). ─────────────────────────────────
    ...
    # cohort of the same year, so special-Sem2 vs normal-Sem1 genuinely
    # runs at the same real time). Same intake + different semester is
    # NOT exempt. Kept in sync with timetable.timetable_panel.is_scheduling_exempt.
    ...
    # A student picks ONE stem per category and takes EVERY course listed
    # under that stem. Using the FULL stem-MEMBERSHIP SET (not just a
    # single "primary" stem) matters because a course can legitimately
    # belong to more than one stem at once (e.g. a course required by
    # both a 'Statistics' stem and an 'Accounting' stem of the same
    # category) — that course must still clash-check against those TWO
    # stems' own courses (a student in either stem takes it), but has no
    # reason to clash against a THIRD, unrelated stem's courses just
    # because it couldn't be reduced to one "primary" stem.
    #   * shares ANY stem with the other side → NEVER exempt — some
    #     student takes both (same rule as "same stem" before, generalised
    #     to sets so it still catches a shared/multi-stem course).
    #   * no shared stem, but every stem either side belongs to sits in
    #     the SAME category → exempt (a student who picks one of those
    #     stems never ends up in another stem of that same category, so
    #     they never take both courses).
    #   * either side has no stem at all, or the stem-sets' categories
    #     don't overlap → falls through to the checks below.
    stems_a = _get_specialization_stem_ids(alloc_a)
    stems_b = _get_specialization_stem_ids(alloc_b)
    if stems_a and stems_b:
        if stems_a & stems_b:
            return False
        cats_a = _get_specialization_category_ids(alloc_a, stems_a)
        cats_b = _get_specialization_category_ids(alloc_b, stems_b)
    ...
    # AND only against the OTHER courses in that SAME SelectionGroup — a
    # student picks exactly one course from a given group, so two courses
    # in the SAME group can never both be taken by one student and are
    # safe to schedule concurrently.
    #
    # Courses from TWO DIFFERENT SelectionGroups are NOT automatically
    # exempt: a student can (and typically must) pick one course from
    # group A *and* one course from group B, so those two courses must
    # still clash-check normally. Likewise, a course flagged elective but
    # never added to any SelectionGroup is NOT optional for the cohort —
    # every student still takes it — so it must clash-check like an
    # ordinary mandatory course instead of being waved through.
...
# ═══════════════════════════════════════════════════════════════════════════════
# build_global_merged_tasks — CombinedCourseGroups, PLUS automatic
# same-lecturer + same-course-code merging (venue supply is limited)
# ═══════════════════════════════════════════════════════════════════════════════

# Venue supply is limited system-wide, so when the SAME lecturer teaches
# several allocations of the SAME base course code — no matter which
# program(s) each one belongs to — they are auto-merged into one shared
# slot/venue as long as their combined headcount fits in one room. 170 is
# the cap requested for this rule specifically (independent of the
# COD-panel CombinedCourseGroup `merge_limit`, which defaults to 200 and
# governs a different, manually-curated merge path).
AUTO_MERGE_LECTURER_COURSE_STUDENT_CAP = 170

# Populated by build_global_merged_tasks() each time it runs, and read as
# ONE OF THREE sources by get_protected_merged_alloc_ids() — see that
# function's docstring. Kept as a fast-path belt-and-braces for merges
# computed earlier in this exact process, but get_protected_merged_alloc_ids()
# no longer depends on it alone: it also reads MergedCourseGroupTimetable
# straight from the DB, so protection still holds when the collision-swap
# pass runs in a different process/run than the one that built the merges
# (re-run, separate worker, admin action, server restart).
...
def build_global_merged_tasks(
    ...
    Two independent merge mechanisms feed into the SAME task list, in this
    order:

    Pass 1 — CombinedCourseGroup (explicit, COD-panel defined)
    ────────────────────────────────────────────────────────────
    Allocations that belong to a CombinedCourseGroup are always scheduled
    together as one task, regardless of course code, lecturer, or headcount.
    This is a manual, per-group decision made in the COD panel and always
    wins first claim on its allocations.

    Pass 2 — automatic SAME-COURSE-CODE merge (grouped by lecturer, INCLUDING
    unassigned)
    ────────────────────────────────────────────────────────────
    Everything NOT already claimed by a CombinedCourseGroup is grouped by
    (normalize_course_code_base(course_code), lecturer_id) — e.g. 'ECON 111'
    taught by lecturer #7 in the Bachelor of Economics AND 'ECON 111' taught
    by lecturer #7 in the Bachelor of Commerce are the SAME group, even
    though they belong to different programs, because it is physically the
    same lecturer teaching the same course content and can only be in one
    room at a time anyway. Sections with NO lecturer assigned yet group the
    same way under (norm_code, None) — there's no lecturer to conflict on,
    so several small unassigned sections of one course code (e.g. two
    'ENSC 0100' rows) still combine into one slot instead of each wasting a
    separate venue while waiting for allocation. Only a lone section for a
    given (course, lecturer-or-None) pair is left individual — there is
    nothing to merge it with.

    Within each (course, lecturer) group, allocations are bin-packed
    (best-fit-decreasing) into as few sub-groups as possible, each capped at
    `auto_merge_student_cap` (170) total students, so:
      * ECON111-A (60) + ECON111-B (55) + ECON111-C (50) → ONE 165-seat class.
      * If a fourth 60-student section pushed the total over 170, that
        section is bin-packed into its own group (or combined with any
        other still-open group it fits in) instead of blowing the cap.
      * A single section that alone exceeds the cap is left as its own
        individual task — the ordinary overflow/venue-shortage handling
        downstream takes it from there.

    Every merged task — from either pass — is placed at ONE slot in ONE
    venue sized for its combined headcount, freeing up the venues its
    sections would otherwise have occupied separately.
    """
    alloc_by_id = {a.id: a for a in allocations}
    tasks = []
    combined_alloc_ids: Set[int] = set()  # IDs already claimed by any merge (either pass)
    ...
    # ── Pass 2: automatic SAME-LECTURER + SAME-COURSE-CODE merge ───────────
    # Also covers UNASSIGNED-lecturer sections: several small sections of one
    # course code with no lecturer assigned yet (e.g. two 'ENSC 0100'-style
    # rows) still bin-pack together under key (norm_code, None) exactly like
    # a real lecturer's sections do — there's no lecturer to conflict on, so
    # there's nothing stopping them sharing one slot/venue instead of each
    # claiming a separate one while waiting for allocation.
    remaining = [a for a in allocations if a.id not in combined_alloc_ids]

    lecturer_course_groups: Dict[Tuple[str, Optional[int]], List] = defaultdict(list)
    ...
    for alloc in remaining:
        ...
        if not norm_code:
            # No usable course code — nothing to key a merge on.
            individual_leftovers.append(alloc)
            continue
        lecturer_course_groups[(norm_code, lid)].append(alloc)

    auto_merge_group_index = 0
    for (norm_code, lid), group_allocs in lecturer_course_groups.items():
        if len(group_allocs) < 2:
            # Only one section of this course for this lecturer (or,
            # for lid=None, only one unassigned section) — nothing to merge.
            individual_leftovers.extend(group_allocs)
            continue

        for bin_allocs in _pack_into_merge_bins(group_allocs, auto_merge_student_cap):
            if len(bin_allocs) < 2:
                # Didn't combine with anything (e.g. it alone exceeds the cap).
            ...
            lecturer_label = f"lecturer #{lid}" if lid is not None else "unassigned lecturer"
            safe_print(
                f"[AutoMerge] {norm_code} ({lecturer_label}): "
    ...
    # ── Pass 3: everything still unclaimed is an INDIVIDUAL task ───────────
...
def _pack_into_merge_bins(allocs: List, merge_limit: int) -> List[List]:
    """
    Greedily pack `allocs` (expected to all share the same normalized course
    code and lecturer) into bins whose summed `number_of_students` never
    exceeds `merge_limit`, so several small sections of one course taught by
    one lecturer can share a single venue/slot instead of each claiming a
    separate one.

    Sorted largest-first (best-fit-decreasing): each allocation goes into the
    first existing bin it still fits in, or opens a new bin if it fits in
    none. An allocation that alone exceeds `merge_limit` still gets a bin of
    its own — it simply never picks up a partner — so the caller can treat
    single-item bins as "not merged" without any allocation going missing.
    """
    if not allocs:
        return []
...
def get_protected_merged_alloc_ids() -> Set[int]:
    """Every CourseAllocation ID that must never be relocated, swapped, or
    scheduled individually because it shares one slot/venue with other
    allocations — from EITHER merge mechanism:

    1. Explicit CombinedCourseGroup membership (COD-panel defined).
    2. Any runtime auto-merge recorded in MergedCourseGroupTimetable — read
       straight from the DB, NOT from `_LAST_AUTO_MERGED_ALLOC_IDS`.

    `_LAST_AUTO_MERGED_ALLOC_IDS` is a plain in-memory global populated by
    build_global_merged_tasks(); it only reflects merges computed earlier
    in THIS process, during THIS run. process_post_schedule_collision_swap
    (the caller that matters most here) can run on its own — a re-run, a
    different worker/process, an admin "fix collisions" action, or after a
    server restart — with that global empty or stale from a previous run.
    Relying on it alone meant a genuinely merged class (same lecturer, same
    course, sharing one slot on purpose) looked "unprotected" any time this
    ran outside the original scheduling process, so this pass split it
    apart and reported/created real lecturer collisions in the process.

    resolve_venue_double_bookings avoids this by querying
    MergedCourseGroupTimetable directly — mirrored here so both safety
    nets agree regardless of process state. The in-memory global is still
    unioned in (belt-and-braces for merges from this exact run that may
    not have hit the DB yet), but it is no longer the only source.
    """
    try:
        combined_ids = set(
...
def _validate_and_rebuild_merged_group(missing_allocs: List, original_task: Dict,
    ...
    """
    Validate and rebuild a merged group (from either merge mechanism) after
    some of its allocations turned out to be unplaced/missing.

    - CombinedCourseGroup task → keep every remaining allocation together
      (a COD-defined group is never silently split).
    - Auto-merged (same lecturer + same course code) task → keep the
      remaining allocations together too, but re-cap them at
      `auto_merge_student_cap` in case the missing subset changed the total
      enough to matter; bin-packing degrades gracefully back to individual
      tasks for anything that no longer fits with a partner.
    - Anything else (a plain individual allocation) → return as-is,
    ...
    # Not part of any merge — return individual allocations.
...
# ═══════════════════════════════════════════════════════════════════════════════
# SchedulerCache and ConflictTracker classes (FIXED)
# ═══════════════════════════════════════════════════════════════════════════════
...
class ConflictTracker:
    """Tracks all scheduled allocations to prevent conflicts.
    
    FIXED: Properly tracks lecturer, program-year, and venue conflicts
    with slot-level granularity.
    """
    def __init__(self, days, slots, lecturer_blocked: Optional[Dict] = None):
        ...
        # {day: {slot_index: count}} — how many classes (of ANY
        # lecturer/program/venue) currently sit in this day/slot. Distinct
        # from program_year_daily_count (which is per-cohort): this is
        # university-wide, and exists purely so placement passes can ask
        # "which slot is least busy right now" instead of always trying
        # slot 0 (the first/earliest slot) first. See get_slot_order().
    ...
    def has_lecturer_conflict(self, lecturer_id, day, slot_index) -> bool:
        """Check if lecturer is already scheduled at this day/slot OR hard-blocked."""
        if not lecturer_id:
            return False
    ...
    def has_lecturer_hard_block(self, lecturer_id, day, slot_index) -> bool:
        """Hard LecturerBlockedSlot check ONLY."""
        if not lecturer_id:
            return False
        day_block = self.lecturer_blocked.get(lecturer_id, {}).get(day)
        if day_block == 'ALL_DAY':
    ...
    def has_program_conflict(self, program_id, year, day, slot_index,
                              new_alloc=None) -> bool:
        """Check if program-year cohort already has a non-exempt course at this slot."""
        if not program_id:
            return False
    ...
    def has_venue_conflict(self, venue_id, day, slot_index) -> bool:
        """Check if venue is already booked at this day/slot."""
        if not venue_id:
            return False
        return slot_index in self.venue_schedule[venue_id][day]

    def get_program_year_day_load(self, program_id, year, day) -> int:
        """Get number of courses scheduled for this program-year cohort on this day."""
        if not program_id:
            return 0
    ...
    def get_best_day_for_program_year(self, program_id, year, preferred_day) -> str:
        """Get the day with the lowest load for this program-year cohort."""
        if not program_id:
    ...
    def add_schedule(self, lecturer_id, program_id, year, venue_id, day, slot_index,
        ...
        """Add a single allocation to the conflict tracker."""
        if lecturer_id:
    ...
    def add_merged_schedule(self, allocs, venue_id, day, slot_index, cache):
        """Add a merged task (multiple allocations sharing one slot) to the conflict tracker."""
        self.slot_load[day][slot_index] += 1
    ...
    def resolve_program_conflict(self, program_id, year, day, slot_index,
        ...
        """Find an alternative slot that resolves a program-year conflict."""
        self.collisions_detected += 1
        max_attempts = 50
        attempts = 0

        # Try nearby slots first (same day)
        ...
        # Exhaustive search across all days and slots
        day_scores = sorted(
    ...
    def get_slot_order(self, day: str, num_slots: int) -> List[int]:
        """Slot indices for `day`, least-busy first.

        Placement loops used to do `for slot_idx in range(len(slots))`,
        which means slot 0 (the earliest/morning slot) was ALWAYS tried
        first, for every single task, on every pass. Since most tasks find
        a free venue/lecturer/program combo on the first slot they try,
        slot 0 (then 1, then 2...) fills up long before the later slots are
        even considered — courses pile into the morning while afternoon
        slots sit empty, independent of the day-level load balancing that
        already existed.

        Sorting by current occupancy (ties broken by original slot order,
        since Python's sort is stable) fixes this: once slot 0 has more
        classes in it than slot 1, slot 1 gets tried first for the next
        task, and so on — the passes naturally spread load across the
        whole day instead of packing it into the morning.
        """
...
# ═══════════════════════════════════════════════════════════════════════════════
# EVENING & WEEKEND OVERFLOW PASS (FIXED slot index handling)
# ═══════════════════════════════════════════════════════════════════════════════
...
def process_evening_weekend_overflow(
    ...
    FIXED: Evening and weekend slots now use REAL slot indices (not virtual 1000+),
    so lecturer/program conflicts with regular courses are properly detected.
    """
    ...
    # ── Build slot pools with REAL indices ──────────────────────────────────
    overflow_windows: List[tuple] = []   # (day, start, end, slot_index)

    regular_days = getattr(config, 'days', None) or [
        "Monday", "Tuesday", "Wednesday", "Thursday", "Friday"
    ]

    # Get the regular slot count so we can extend with evening/weekend slots
...
class VenueAllocator:
    ...
    def find_efficient_venue(self, total_students: int, available_venues: List[Venue],
                             ...
                             preference: str = "best_fit") -> Optional[Venue]:
        """
        Find the best venue, considering both capacity and lecturer preferences.
        """
        if not available_venues:
            return None
            
        # Filter by capacity (exact fit first, then any free)
        suitable = [
        ...
        if not suitable:
            ...
            if any_free:
                # Prefer the largest free venue if nothing fits exactly
                return max(any_free, key=lambda x: (x.capacity or 0))
            return None
        
        if preference == "best_fit":
            # If we have preferences, score venues by preference + capacity fit
...
# ═══════════════════════════════════════════════════════════════════════════════
# process_ug_batch (FIXED - better conflict checking)
# ═══════════════════════════════════════════════════════════════════════════════
...
def process_ug_batch(batch_tasks, config, faculty_venues_map, all_venues, days, slots,
    ...
    # Track lecturer slot usage per day - this is the KEY fix
    # Prevents a lecturer from being scheduled in multiple slots on the same day
    ...
    for task in sorted_tasks:
        ...
        for day in preferred_days:
            ...
            # Check if lecturer already has a class at this day (any slot)
            if lecturer_id:
                lecturer_slots_this_day = lecturer_slot_usage[lecturer_id].get(day, set())
                # If lecturer already has a class at this day, try to use a different slot
                # The conflict tracker will handle exact slot conflicts
            ...
            for slot_idx in conflict_tracker.get_slot_order(day, len(slots)):
                ...
                # Check lecturer conflict with the conflict tracker
                ...
                # Also check if this lecturer is already using this exact slot (local batch tracking)
                ...
                if program_conflict:
                    ...
                    if resolved:
                        ...
                        # Re-check lecturer conflict after resolution
                        lecturer_conflict = lecturer_id and conflict_tracker.has_lecturer_conflict(lecturer_id, current_day, current_slot_idx)
                        # Also check local batch tracking
                        if lecturer_id and current_slot_idx in lecturer_slot_usage[lecturer_id].get(current_day, set()):
                            lecturer_conflict = True
                ...
                if venue:
                    ...
                    # Track lecturer slot usage in this batch
                    if lecturer_id:
...
def process_ug_fallback(unscheduled_tasks, all_venues, days, slots,
    ...
    batch_entry_keys = set()
    # Local lecturer slot tracking for this pass
    lecturer_slot_usage: Dict[int, Dict[str, Set[int]]] = defaultdict(lambda: defaultdict(set))
...
def _rebuild_sweep_tasks_from_db(
    ...
    for t in all_tasks:
        ...
        if isinstance(t, dict) and t.get('combined_group'):
            ...
            # Keep the auto-merged (same lecturer + same course code) group
            # together where possible, same as a CombinedCourseGroup — see
            # _validate_and_rebuild_merged_group for the re-cap/bin-pack logic.
            rebuilt = _validate_and_rebuild_merged_group(missing, t, merge_limit)
            unscheduled_tasks.extend(rebuilt)
            safe_print(
                f"[Phase5-Sweep] auto-merged group {t.get('norm_code', '?')}: "
                f"{len(missing)}/{len(allocs)} still unscheduled → "
                f"rebuilt into {len(rebuilt)} task(s)"
            )
        else:
            for a in missing:
...
def process_exhaustive_sweep(
    ...
    auto_merged_groups = sum(1 for t in unscheduled_tasks if isinstance(t, dict) and t.get('auto_merged'))
    merged_other = sum(
        1 for t in unscheduled_tasks
        if isinstance(t, dict) and not t.get('combined_group') and not t.get('auto_merged')
    )
    indiv_count = len(unscheduled_tasks) - combined_groups - auto_merged_groups - merged_other

    safe_print(
        f"[Phase5-Sweep] Task composition: {combined_groups} CombinedCourseGroups, "
        f"{auto_merged_groups} auto-merged (same lecturer + same course code) groups, "
            ...
            for day in days:
                for slot_idx, (start, end) in enumerate(slots):
                    if sub_lecturer_id and \
                            (conflict_tracker.has_lecturer_conflict(sub_lecturer_id, day, slot_idx) or
                             slot_idx in lecturer_slot_usage[sub_lecturer_id].get(day, set())):
                        continue
                    ...
                    if sub_is_merged:
                        ...
                        for alloc in sub_allocs:
                            ...
                            try:
                                ...
                                break
                            if a_lid and \
                                    (conflict_tracker.has_lecturer_conflict(a_lid, day, slot_idx) or
                                     slot_idx in lecturer_slot_usage[a_lid].get(day, set())):
                                conflict = True
                                break
                    ...
                    if sub_lecturer_id:
                        lecturer_slot_usage[sub_lecturer_id][day].add(slot_idx)
    ...
    for task_idx, task in enumerate(unscheduled_tasks):
        ...
        is_auto_merged = is_merged and task.get('auto_merged')
        allocs = _task_allocs(task)
        total_students = _task_students(task)
        rep = _representative(task)
        lecturer_id = rep.lecturer.id if rep.lecturer else None
        ...
        safe_print(
            ...
            f"is_combined={is_combined}  is_auto_merged={is_auto_merged}"
        )
        ...
        elif not assigned and is_auto_merged:
            msg = (
                f"{code} (Year {year}, {total_students} students) "
                f"[SWEEP] — Truly Unschedulable after testing "
                f"{tested_triples} (day×slot) combinations × "
                f"{len(all_venues)} venues each (auto-merged: same lecturer + "
                f"same course code — needs a venue with capacity ≥ {total_students}, "
                f"or fewer sections merged together)"
            )
            unschedulable_list.append(msg)
            safe_print(
                f"[Phase5-Sweep] TRULY UNSCHEDULABLE: {code} "
                f"(auto-merged group — dissolve is disabled for this task)"
            )
        ...
        elif assigned and is_auto_merged:
            safe_print(f"[Phase5-Sweep] SUCCESSFULLY PLACED auto-merged group: {code}")
...
def process_universal_safety_net(
    ...
    if combined_remaining:
        ...
        for gid, group_allocs in by_group.items():
            ...
            # first one. A combined group can span several member program-
            # years at once, so blindly taking the first slot that clears
            # the bare hard-block check (as passes A/B do — safe there,
            # since they guarantee zero or program-year-safe collisions)
            # can force MULTIPLE simultaneous program-year clashes when a
            # nearby slot would only have caused one, or none. Scan every
            # day/slot this group could legally go in, score each by how
            # many member collisions it would create, and commit to the
            # lowest-scoring one — a strict improvement over first-fit
            # since every candidate considered already clears the same
            # hard-block bar the old code required.
            if not group_placed:
...
def run_optimized_autoscheduler_thread(disabled_constraints: Optional[Set[str]] = None):
    ...
    try:
        ...
        # STEP 1: BUILD TASKS - CombinedCourseGroups + automatic same-lecturer/
        # same-course-code merge (capped at AUTO_MERGE_LECTURER_COURSE_STUDENT_CAP)
        ...
        update_progress(5, "STEP 1: Building tasks from allocations...",
                        0, total_courses,
                        console_message=f"Processing {total_courses} allocations "
                                        f"(CombinedCourseGroups + same-lecturer/same-course auto-merge)")
        ...
        safe_print(
            ...
            f"{n_auto_merge_groups} auto-merged (same lecturer + same course code) groups "
            f"covering {n_auto_merge_allocs} sections, "
        ...
        update_progress(8, f"STEP 1 done: {n_combined_groups} CombinedCourseGroups, "
                        ...
                        console_message=f"{n_combined_groups} CombinedCourseGroups ({n_combined_allocs} sections merged), "
                                        f"{n_auto_merge_groups} auto-merged groups ({n_auto_merge_allocs} sections merged), "
        ...
        # ── Load preferences for all phases ──────────────────────────────────
        time_prefs = constraint_engine.get_lecturer_time_preferences(disabled_constraints)
        venue_prefs = constraint_engine.get_lecturer_venue_preferences(disabled_constraints)
        blocked_ranges = constraint_engine.get_lecturer_blocked_ranges(disabled_constraints)
        ...
        # ================================================================
        # STEP 2: SCHEDULE ALL UG TASKS TOGETHER (fair interleaving)
        # ================================================================
        safe_print(f"\nSTEP 2: Scheduling ALL UG tasks together (individual + CombinedCourseGroups)...")

        all_ug_tasks = merged_ug_tasks + individual_ug_tasks

        # ── SMALL-COURSE DEFERRAL ────────────────────────────────────────
        # interleave_by_program_year() ranks each task by its size RELATIVE
        # TO ITS OWN COHORT, then round-robins across cohorts. That means a
        # 5-student course from a program with only 2 courses total lands
        # in the very first processing round — the same round as some other
        # cohort's 300-student 2nd-largest course — and both compete for
        # venues/slots in the same early batch. Since batches run
        # sequentially against one shared conflict_tracker, whichever batch
        # goes first claims the best venue/day/slot combos, regardless of
        # actual size. Small classes have almost no placement constraints
        # (they fit in nearly any room, at nearly any time) while large or
        # collision-prone classes have very few venues/slots that actually
        # work for them. So: let every course above the threshold fight for
        # slots FIRST via the untouched pipeline below, and schedule the
        # small ones LAST, as cheap filler into whatever venues/slots the
        # big/hard courses didn't need. This does not change how conflicts
        # are checked — small tasks still go through the same
        # lecturer/program/venue conflict logic in process_ug_fallback,
        # they just don't get first pick anymore.
        SMALL_COURSE_FILLER_THRESHOLD = int(
            getattr(config, 'small_course_filler_threshold', 20)
        )
        small_ug_tasks = [
        ...
        if small_ug_tasks:
            ...
            safe_print(
                f"[SmallCourseDeferral] {len(small_ug_tasks)} task(s) with "
                f"\u2264{SMALL_COURSE_FILLER_THRESHOLD} students deferred to run "
                f"LAST, after every larger/harder course has already claimed "
                f"a slot ({len(all_ug_tasks)} task(s) remain in the main pipeline)."
            )

        n_ug_tasks = len(all_ug_tasks)

        all_ug_tasks = interleave_by_program_year(
            all_ug_tasks, cache, size_desc=True, known_count_first=True
        )

        update_progress(12, f"STEP 2: Scheduling {n_ug_tasks} UG tasks (individual + CombinedCourseGroups)...",
                        0, n_ug_tasks,
                        console_message=f"UG tasks: {n_individual} individual + {n_merged_ug} CombinedCourseGroups")
        ...
        for b_idx, i in enumerate(range(0, n_ug_tasks, UG_BATCH), 1):
            ...
            update_progress(min(42, prog),
                            f"STEP 2: UG batch {b_idx}/{ug_batches}",
                            ug_scheduled,
                            n_ug_tasks - ug_scheduled,
                            batch_info=f"Batch {b_idx}/{ug_batches}",
                            current_batch=b_idx, total_batches=ug_batches,
                            console_message=f"UG batch {b_idx}: {len(batch)} tasks",
                            scheduled_courses=all_scheduled_courses,
                            unscheduled_courses=all_unscheduled_courses)
        ...
        if ug_still_unscheduled:
            update_progress(45, "STEP 2: UG fallback (all venues)...",
                            ug_scheduled,
                            len(ug_still_unscheduled),
                            console_message=f"UG fallback: {len(ug_still_unscheduled)} tasks",
                            scheduled_courses=all_scheduled_courses,
                            unscheduled_courses=all_unscheduled_courses)
        ...
        if ug_unscheduled_after_fallback:
            update_progress(48, "STEP 2b: Phase 3 Compression...",
                            ug_scheduled,
                            len(ug_unscheduled_after_fallback),
                            console_message=f"Compression: {len(ug_unscheduled_after_fallback)} tasks remain",
                            scheduled_courses=all_scheduled_courses,
                            unscheduled_courses=all_unscheduled_courses)
        ...
        # ── STEP 2c: SMALL-COURSE FILLER PASS ────────────────────────────
        # Every larger/harder UG course above the threshold has now had its
        # full shot at batches → fallback → compression, so the
        # conflict_tracker reflects their placements. Small courses go
        # through process_ug_fallback (full venue pool, real
        # lecturer/program conflict checks via resolve_program_conflict —
        # no shortcuts) LAST, so they fill whatever slots/venues remain
        # instead of occupying prime slots ahead of courses that actually
        # needed them.
        small_scheduled = 0
        if small_ug_tasks:
            update_progress(49, f"STEP 2c: Small-course filler ({len(small_ug_tasks)} tasks \u2264{SMALL_COURSE_FILLER_THRESHOLD} students)...",
                            ug_scheduled,
                            len(small_ug_tasks),
                            console_message=f"Small-course filler: {len(small_ug_tasks)} tasks",
                            scheduled_courses=all_scheduled_courses,
                            unscheduled_courses=all_unscheduled_courses)
            ...
            all_scheduled_courses.extend(
                [s.replace("[FALLBACK]", "[SMALL-COURSE-FILLER]") for s in sl_small]
            )
            all_unscheduled_courses.extend(ul_small)
            safe_print(
                f"[SmallCourseDeferral] Filler pass placed {sc_small}/"
                f"{len(small_ug_tasks)}, still unscheduled: {len(still_small)}"
            )
        ...
        safe_print(
            f"STEP 2 done: {ug_scheduled}/{ug_total} UG allocs scheduled ({ug_rate:.1f}%) | "
            f"CombinedCourseGroups interleaved fairly with individual courses"
        )
        update_progress(50, "STEP 2 done: UG scheduling complete (fair interleaving)",
                        ug_scheduled, len(ug_unscheduled),
                        console_message=f"UG: {ug_scheduled}/{ug_total} ({ug_rate:.1f}%) | "
                                        f"CombinedCourseGroups interleaved fairly with individual courses",
                        scheduled_courses=all_scheduled_courses,
                        unscheduled_courses=all_unscheduled_courses)
        ...
        update_progress(55, "STEP 4: Scheduling PG courses (deferred, last)...",
                        ug_scheduled, n_pg,
        ...
        if pg_tasks:
            ...
            update_progress(70, "STEP 4 done: PG scheduling complete",
                            ug_scheduled + pg_scheduled, len(still_pg),
        ...
        update_progress(
            75,
            "STEP 5: Exhaustive Free-Slot Sweep…",
            ug_scheduled + pg_scheduled,
            total_courses - (ug_scheduled + pg_scheduled),
        ...
        if evening_weekend_tasks:
            ...
            update_progress(
                ...
                ug_scheduled + pg_scheduled + sweep_placed + ew_placed,
                len(ew_still),
                console_message=(
        ...
        # STEP 5c: ZERO-STUDENT SAFETY NET
        zero_placed = 0
        ...
        if still_unplaced_zero:
            ...
            update_progress(
                ...
                ug_scheduled + pg_scheduled + sweep_placed + evening_weekend_placed,
            ...
            for alloc in still_unplaced_zero:
                ...
                if not placed:
                    zero_unscheduled_list.append(
                        ...
                        f"conflict-free slot available"
                    )
        ...
        # ── Notifications for zero-student courses ──────────────────────────
        ...
        try:
            ...
            safe_print(
                ...
                f"resolved_by_merge={collision_swap_stats.get('resolved_by_merge', 0)} | "
        ...
        final_message = (
            ...
            f"CombinedCourseGroups interleaved fairly: {n_combined_groups} groups ({n_combined_allocs} sections) | "
            f"Auto-merged (same lecturer + same course code, cap {AUTO_MERGE_LECTURER_COURSE_STUDENT_CAP}): "
            f"{n_auto_merge_groups} groups ({n_auto_merge_allocs} sections) | "
        ...
        safe_print(f"CombinedCourseGroups:    {n_combined_groups} groups covering {n_combined_allocs} sections (interleaved fairly)")
