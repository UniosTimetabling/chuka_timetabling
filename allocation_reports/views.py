...
from allocation_reports.services import (
    build_allocation_docx, dashboard_rows, get_or_generate_pdf, queue_bulk_generation,
)
...
def api_status(request):
    ...
    return JsonResponse({
        ...
        "word_url": (reverse("allocation_word_download", args=[run.pk]) if run and run.file else None),
...
# ─────────────────────────────────────────────────────────────────────────────
# Word (.docx) version of an allocation PDF
# ─────────────────────────────────────────────────────────────────────────────
@login_required
@allowed_roles(Role.COD, Role.COD_ADMIN, *MANAGEMENT_ROLES_FOR_ALLOCATIONS)
def download_allocation_word(request, run_id):
    """
    Word version of an allocation PDF run. The current run is rebuilt natively
    from live data; archived runs are converted from their stored PDF.
    COD / COD Admin can only fetch their own department's runs.
    """
    from core.doc_export import docx_response, pdf_to_docx_bytes

    run = AllocationPdfRun.objects.select_related("department", "campus").filter(pk=run_id).first()
    if run is None:
        raise Http404("Allocation run not found")

    if not request.user.is_superuser:
        own = resolve_user_department(request.user)
        from core.rbac import get_user_roles
        roles = set(get_user_roles(request.user))
        is_cod_only = bool(roles & {Role.COD, Role.COD_ADMIN}) and not (roles & set(MANAGEMENT_ROLES_FOR_ALLOCATIONS))
        if is_cod_only and (own is None or own.pk != run.department_id):
            raise Http404("Allocation run not found")

    base = f"{run.department.name} - {run.scope} allocation v{run.version}"
    if run.is_current:
        data = build_allocation_docx(run.scope, run.department, campus=run.campus, user=request.user)
    elif run.file:
        run.file.open("rb")
        try:
            data = pdf_to_docx_bytes(run.file.read())
        finally:
            run.file.close()
    else:
        raise Http404("This run has no file")
    return docx_response(data, base)
