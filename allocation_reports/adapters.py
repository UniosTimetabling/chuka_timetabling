...
def _dept_name(dept):
    """Short display form of a department for the allocation PDF/rows — its
    code (e.g. "DHUM") when one exists, else the full name. Department
    names (especially merged ones like "Agricultural Economics,
    Agribusiness Management & Agricultural Education") were overflowing
    and getting clipped in the Origin/Allocating Department table columns;
    the code keeps this compact. pdf_builder renders a "Department Codes"
    legend at the end of the report so codes can be read back to full
    names. See department_management/department_codes.py."""
    if not dept:
        return "—"
    from department_management.department_codes import department_initials
    return department_initials(dept) or dept.name
