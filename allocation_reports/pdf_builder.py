...
def _render_department_codes_legend(story, styles, rows, serviced_rows):
    """Origin/Allocating Department columns now show a department's short
    code (e.g. "DHUM") instead of its full name, to stop long department
    names from overflowing those columns (see adapters._dept_name). This
    prints a small "CODE = Full Name" legend at the end of the report for
    every code that actually appears above, so the report is still
    self-explanatory without anyone needing to look the code up elsewhere.
    """
    from department_management.department_codes import name_for_code

    codes_seen = set()
    for r in rows:
        codes_seen.add(r.get("origin_department"))
    for r in serviced_rows or []:
        codes_seen.add(r.get("allocating_department"))

    legend = {}
    for code in codes_seen:
        full_name = name_for_code(code)
        if full_name:
            legend[code] = full_name

    if not legend:
        return

    story.append(Spacer(1, 10))
    story.append(Paragraph("Department Codes", styles["SubHead"]))
    legend_text = "; ".join(f"{code} = {legend[code]}" for code in sorted(legend))
    story.append(Paragraph(legend_text, styles["Small"]))
...
def build_allocation_pdf(scope, department, rows, campus=None, generated_by=None, serviced_rows=None):
    ...
    if not rows:
        ...
        if serviced_rows:
            ...
            _render_department_codes_legend(story, styles, [], serviced_rows)
        doc.build(story)
        return buf.getvalue()
    ...
    _render_department_codes_legend(story, styles, rows, serviced_rows or [])
