...
def get_or_generate_pdf(scope, department, campus=None, user=None, force=False):
    ...
    try:
        ...
        from core.doc_export import force_pdf   # stored runs are ALWAYS real PDFs
        with force_pdf():
...
def build_allocation_docx(scope, department, campus=None, user=None):
    """The same allocation report as the stored PDF, built natively as a Word file."""
    from allocation_reports.adapters import fetch_rows, fetch_serviced_rows
    from core.doc_export import force_format
    rows = fetch_rows(scope, department, campus=campus)
    serviced_rows = fetch_serviced_rows(scope, department, campus=campus)
    with force_format("docx"):
