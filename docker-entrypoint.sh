...
# Delegated to wait_for_db.py. The previous inline loop ran the connection
# attempt with `2>/dev/null`, so a wrong password, an unknown hostname and a
# dead server all produced the same useless "Database not available after
# 30s". wait_for_db.py prints the actual error, tells loopback-inside-a-
# container apart from a real outage, and exits immediately (instead of
# burning 30 retries) when the server is up but rejecting the credentials.
#
# DB_WAIT_TIMEOUT defaults to 120s here — the old hard 30s was shorter than
# MySQL 8 takes to finish InnoDB recovery on a cold start.
python /app/wait_for_db.py || exit 1
