        ...
        for key, rows in dup_groups.items():
            ...
            with transaction.atomic():
                ...
                if keeper.course_code != canonical_form:
                    ...
                    # course_code, so this save can collide with a loser row
                    # that's already stored in that exact normalized form and
                    # hasn't been deleted yet (e.g. keeper 'HIST211' ->
                    # 'HIST 211' on save, colliding with a loser already
                    # stored as 'HIST 211'). That loser is deleted a few
                    # lines below anyway, so bump it out of the way first
                    # with a harmless placeholder code (via .update(), which
                    # bypasses save()'s own normalization) so the keeper's
                    # save never hits the unique_together constraint.
                    for loser in losers:
                        if loser.pk and normalize_code(loser.course_code) == canonical_form:
                            ProgramCourse.objects.filter(pk=loser.pk).update(
                                course_code=f"__MERGING_{loser.pk}__"
                            )
