...
def _dvc_scoped_standard_allocations(request):
    """
    Base queryset for any DVC action that approves/rejects *standard*
    CourseAllocation rows in bulk (all / department / faculty / lecturer).

    Concurrent Allocation Sets: a department can have several AllocationSets
    open at once, and only the one(s) actually submitted to DVC (or legacy /
    set-less rows, for pre-upgrade data) should ever be touched by a DVC bulk
    action — never a draft set the COD hasn't submitted yet, and never a
    *different* submitted set than the one the DVC is currently looking at.
    This mirrors the exact filter the panel itself uses to decide what to
    display, so "approve everything on screen" can never reach further than
    what's actually on screen.

    If the request carries an allocation_set_id (the concurrent-set switcher
    on the panel), results are narrowed to just that set.
    """
    from course_allocation.models import AllocationSet, SubmissionControl

    allowed_depts = SubmissionControl.objects.filter(
        allow_submission_to_dvc=True, department__isnull=False
    ).values_list("department_id", flat=True)

    qs = CourseAllocation.objects.filter(department_id__in=allowed_depts).filter(
        Q(allocation_set__status=AllocationSet.STATUS_SUBMITTED_TO_DVC)
        | Q(allocation_set__is_legacy=True)
        | Q(allocation_set__isnull=True)
    )

    allocation_set_id = request.POST.get("allocation_set_id") or request.GET.get("allocation_set_id")
    if allocation_set_id:
        qs = qs.filter(allocation_set_id=allocation_set_id)
...
def dvc_panel(request):
    ...
    if request.method == "POST" and request.headers.get("x-requested-with") == "XMLHttpRequest":
        ...
        if action in ("approve_all_allocations", "reject_all_allocations"):
            ...
            # Concurrent Allocation Sets: only touch rows actually up for DVC
            # review (or legacy/set-less), never a department's other draft
            # sets — see _dvc_scoped_standard_allocations.
            qs = _dvc_scoped_standard_allocations(request)
        ...
        if action in ("approve_department_allocations", "reject_department_allocations"):
            ...
            # Concurrent Allocation Sets: scope to what's actually submitted
            # to DVC for this department, not every set it has.
            qs = _dvc_scoped_standard_allocations(request).filter(department=dept)
            updated = qs.update(approved_by_dvc=is_approve, rejected_by_dvc=not is_approve) if _has_rejected_field() \
                      else qs.update(approved_by_dvc=is_approve)
            if updated:
        ...
        if action in ("approve_faculty_allocations", "reject_faculty_allocations"):
            ...
            # Concurrent Allocation Sets: same scoping as the department
            # action, applied across every department in the faculty.
            qs = _dvc_scoped_standard_allocations(request).filter(department_id__in=dept_ids)
            affected = list(qs.values_list("department_id", flat=True))
            updated = qs.update(approved_by_dvc=is_approve, rejected_by_dvc=not is_approve) if _has_rejected_field() \
                      else qs.update(approved_by_dvc=is_approve)
            for dept in Department.objects.filter(id__in=set(affected)):
        ...
        if action in ("approve_lecturer_allocations", "reject_lecturer_allocations"):
            ...
            # Concurrent Allocation Sets: same scoping, per lecturer.
            qs = _dvc_scoped_standard_allocations(request).filter(lecturer=lecturer)
            updated = qs.update(approved_by_dvc=is_approve, rejected_by_dvc=not is_approve) if _has_rejected_field() \
                      else qs.update(approved_by_dvc=is_approve)
            if updated:
                _log_dvc_action(request, DVCActionLog.ALLOC_STANDARD, is_approve,
    ...
    # Concurrent Allocation Sets: a department-wide SubmissionControl flag
    # used to mean "everything this department has is up for DVC review".
    # Now that a department can have several AllocationSets open at once,
    # also require that a row's OWN set has actually been submitted to DVC
    # — unless it has no set yet (pre-upgrade data / legacy set), in which
    # case it keeps behaving exactly as before so nothing already in review
    # disappears from this queue.
    ...
    allocation_set_filter_id = request.GET.get('allocation_set_id')
    if allocation_set_filter_id:
        allocations = allocations.filter(allocation_set_id=allocation_set_filter_id)
    ...
    # Concurrent Allocation Sets: sets currently awaiting DVC review, for the
    # switcher dropdown. Scoped to the department filter when one is active,
    # otherwise across every department this DVC can see, so the dropdown
    # doesn't force picking a department first.
    allocation_sets_awaiting_dvc = AllocationSet.objects.filter(
        department_id__in=(
            [department_filter_id] if department_filter_id else allowed_depts
        ),
        status=AllocationSet.STATUS_SUBMITTED_TO_DVC,
    ).select_related("department").order_by("department__name", "-submitted_to_dvc_at")

    return render(request, "faculty/dvc_panel.html", {
        # Concurrent Allocation Sets
        "allocation_sets_awaiting_dvc": allocation_sets_awaiting_dvc,
        "active_allocation_set_id":     int(allocation_set_filter_id) if allocation_set_filter_id else None,
        # Standard
