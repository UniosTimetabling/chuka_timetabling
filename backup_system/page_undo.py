...
class PageUndoConfig:
    ...
    # Optional secondary narrowing, applied AFTER the department-level scope
    # above. Signature: callable(logs: list[AuditLog], request) -> list[AuditLog].
    # Used by pages that need a finer-grained scope than "department" (e.g.
    # the COD panel's Concurrent Allocation Sets — a department can have
    # several AllocationSets open at once, and Safe Undo should only offer
    # to restore/undo changes that belong to whichever set is currently
    # active, not every set the department has ever had). Left as None
    # (no-op) for pages that only need department-level scoping.
    filter_logs: Optional[Callable] = None
...
def build_views(config: PageUndoConfig, get_user_scope, template_name, roles, action_url_name):
    ...
    def _still_applicable_now(scope, request=None):
        logs = get_undoable_logs(config, scope.id if scope else None)
        if config.filter_logs is not None:
            logs = config.filter_logs(logs, request)
        return logs
    ...
    def safe_undo_page(request):
        ...
        logs = _still_applicable_now(scope, request)
    ...
    def safe_undo_action(request):
        ...
        all_logs = {log.id: log for log in _still_applicable_now(scope, request)}
