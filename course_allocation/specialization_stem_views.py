...
def specialization_stems_page(request):
    ...
    # Concurrent Allocation Sets: only show categories from the allocation
    # the COD is currently working in.
    _active_set = get_active_allocation_set(request, dept) or get_or_default_legacy_set(dept)
    if _active_set is not None:
        categories = categories.filter(allocation_set=_active_set)
...
def add_specialization_category(request):
    ...
    try:
        with transaction.atomic():
            ...
            if SpecializationCategory.objects.filter(
                program=program, name__iexact=category_name, allocation_set=_active_set
            ).exists():
                return JsonResponse({"status": "error", "message": "A specialization category with this name already exists for this program in this allocation"}, status=400)
        ...
        with transaction.atomic():
            ...
            for pc in pcs:
                ca, created = CourseAllocation.get_or_create_shared(
                    program_course=pc,
                    department=dept,
                    allocation_set=category.allocation_set,
        ...
        with transaction.atomic():
            ...
            for pc in pcs:
                ca, created = CourseAllocation.get_or_create_shared(
                    program_course=pc,
                    department=dept,
                    allocation_set=stem.category.allocation_set,
                    defaults={'specialization_stem': stem},
                )
...
def quick_create_stem_for_student_group(request):
    ...
    try:
        with transaction.atomic():
            ...
            for pc in pcs:
                ca, created = CourseAllocation.get_or_create_shared(
                    program_course=pc, department=dept, allocation_set=category.allocation_set,
                    defaults={'specialization_stem': stem},
                )
                # A SelectionGroup may be NESTED inside a stem — see the note
                # in add_specialization_stem above. Keeping ca.selection_group_id
