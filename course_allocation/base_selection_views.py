...
from .models import BaseSelection, SelectionGroup, CourseAllocation, StudentGroup, SpecializationStem
...
def base_selections_page(request):
    ...
    selection_groups = SelectionGroup.objects.filter(department=dept)
    if _active_set is not None:
        selection_groups = selection_groups.filter(allocation_set=_active_set)
    selection_groups = selection_groups.prefetch_related("courses").annotate(course_count=Count('courses'))
...
def add_courses_to_group(request, group_id):
    ...
    try:
        ...
        with transaction.atomic():
            ...
            # Pool nested in a stem: keep the new alternatives in that stem too.
...
def _selection_group_map_dict(group, student_group=None, stem=None):
    d = {
        "id": group.id,
        "name": group.name,
        ...
        # Nested-in-stem mapping (pick-one pool living inside a combination stem)
        "mapped_stem_ids": list(group.specialization_stems.values_list("id", flat=True)),
        "mapped_stem_names": [st.name for st in group.specialization_stems.all()],
    }
    if student_group is not None:
        d["mapped"] = group.restricted_to_groups.filter(id=student_group.id).exists()
    if stem is not None:
        d["mapped"] = group.specialization_stems.filter(id=stem.id).exists()
        stem_course_ids = set(stem.courses.values_list("id", flat=True))
        d["in_stem_count"] = group.courses.filter(id__in=stem_course_ids).count()
    return d
...
def quick_create_selection_group_for_student_group(request):
    ...
    try:
        with transaction.atomic():
            _active_set = get_active_allocation_set(request, dept) or get_or_default_legacy_set(dept)
            if SelectionGroup.objects.filter(
                department=dept, program=student_group.program, name__iexact=group_name, allocation_set=_active_set
            ).exists():
                return JsonResponse({"status": "error", "message": "An elective group with this name already exists for this program in this allocation"}, status=400)
            ...
            for pc in pcs:
                ca, created = CourseAllocation.get_or_create_shared(
                    program_course=pc, department=dept, allocation_set=sg.allocation_set,
...
# ─────────────────────────────────────────────────────────────────────────────
# Elective (pick-one) groups NESTED INSIDE a combination stem
#   e.g. a stem of 12 units where 10 are core and the student picks ONE of the
#   last two. The pool's courses are members of the stem (so they clash with the
#   10 core units) but two alternatives never clash with each other.
# ─────────────────────────────────────────────────────────────────────────────
def _stem_for_dept(stem_id, dept):
    return (
        SpecializationStem.objects.select_related("category", "category__program")
        .get(id=stem_id, category__department=dept)
    )


@allowed_roles(Role.COD, Role.COD_ADMIN, Role.SUDO)
def get_selection_groups_for_stem(request):
    """Every elective pool of this stem's program, flagged with whether it is
    already nested in the stem. Powers the "Map Elective Group(s)" modal on the
    stem header right-click menu in /cod/."""
    dept = detect_user_department(request.user)
    if not dept:
        return JsonResponse({"status": "error", "message": "Department not found"}, status=400)
    stem_id = request.GET.get("stem_id")
    if not stem_id:
        return JsonResponse({"status": "error", "message": "stem_id is required"}, status=400)
    try:
        stem = _stem_for_dept(stem_id, dept)
    except SpecializationStem.DoesNotExist:
        return JsonResponse({"status": "error", "message": "Stem not found"}, status=404)

    qs = (
        SelectionGroup.objects.filter(department=dept, program=stem.category.program)
        .prefetch_related("restricted_to_groups", "specialization_stems")
        .order_by("name")
    )
    _active_set = get_active_allocation_set(request, dept) or get_or_default_legacy_set(dept)
    if _active_set is not None:
        qs = qs.filter(allocation_set=_active_set)
    stem_course_ids = set(stem.courses.values_list("id", flat=True))
    elective_ids = stem.elective_course_ids()
    return JsonResponse({
        "status": "success",
        "stem": {
            "id": stem.id, "name": stem.name,
            "program_id": stem.category.program_id,
            "program_name": stem.category.program.name if stem.category.program else "",
            "year": stem.category.year,
            "semester": stem.category.semester,
            "course_count": len(stem_course_ids),
            "core_count": len(stem_course_ids - elective_ids),
        },
        "selection_groups": [_selection_group_map_dict(g, stem=stem) for g in qs],
    })


@allowed_roles(Role.COD, Role.COD_ADMIN, Role.SUDO)
def toggle_selection_group_stem(request, group_id):
    """Nest / un-nest ONE elective pool in ONE stem."""
    if request.method != "POST":
        return JsonResponse({"status": "error", "message": "POST required"}, status=400)
    dept = detect_user_department(request.user)
    if not dept:
    ...
    action = (request.POST.get("action") or "add").strip().lower()
    if not stem_id or action not in ("add", "remove"):
        return JsonResponse({"status": "error", "message": "stem_id and a valid action are required"}, status=400)
    try:
        group = SelectionGroup.objects.select_related("program").get(id=group_id, department=dept)
        stem = _stem_for_dept(stem_id, dept)
    except (SelectionGroup.DoesNotExist, SpecializationStem.DoesNotExist):
        return JsonResponse({"status": "error", "message": "Group or stem not found"}, status=404)
    if stem.category.program_id != group.program_id:
        return JsonResponse({"status": "error", "message": "The stem and the elective group must belong to the same program"}, status=400)

    with transaction.atomic():
        if action == "add":
            group.specialization_stems.add(stem)
            group.sync_courses_into_mapped_stems()
            message = (f"'{group.name}' is now a pick-one group inside stem '{stem.name}' — "
                       f"its courses are members of the stem, alternatives can share a slot, "
                       f"and they still clash with the stem's core units.")
        else:
            group.specialization_stems.remove(stem)
            message = (f"'{group.name}' is no longer nested in '{stem.name}'. Its courses stay in the "
                       f"stem as ordinary units — remove them from the stem if students no longer take them.")
    return JsonResponse({"status": "success", "message": message,
                         "selection_group": _selection_group_map_dict(group, stem=stem)})


@allowed_roles(Role.COD, Role.COD_ADMIN, Role.SUDO)
def quick_create_selection_group_for_stem(request):
    """Create a pick-one pool directly inside one stem: pick the alternatives
    (existing ProgramCourses of the stem's program), name it, done."""
    if request.method != "POST":
        return JsonResponse({"status": "error", "message": "POST required"}, status=400)
    dept = detect_user_department(request.user)
    if not dept:
        return JsonResponse({"status": "error", "message": "Department not found"}, status=400)
    try:
        stem = _stem_for_dept(request.POST.get("stem_id"), dept)
    except (SpecializationStem.DoesNotExist, ValueError, TypeError):
        return JsonResponse({"status": "error", "message": "Stem not found"}, status=404)
    group_name = (request.POST.get("group_name") or "").strip()
    if not group_name:
        return JsonResponse({"status": "error", "message": "Group name is required"}, status=400)
    pc_ids = request.POST.getlist("program_course_ids")
    if len(pc_ids) < 2:
        return JsonResponse({"status": "error", "message": "Pick at least two alternative courses"}, status=400)
    program = stem.category.program
    try:
        with transaction.atomic():
            _active_set = get_active_allocation_set(request, dept) or get_or_default_legacy_set(dept)
            if SelectionGroup.objects.filter(department=dept, program=program, name__iexact=group_name,
                                             allocation_set=_active_set).exists():
                return JsonResponse({"status": "error", "message": "An elective group with this name already exists for this program in this allocation"}, status=400)
            pcs = ProgramCourse.objects.filter(pk__in=pc_ids, program=program)
            if pcs.count() != len(set(pc_ids)):
                return JsonResponse({"status": "error", "message": "All courses must belong to the stem's program"}, status=400)
            sg = SelectionGroup.objects.create(name=group_name, department=dept, program=program,
                                               created_by=request.user, allocation_set=_active_set)
            cas = []
            for pc in pcs:
                ca, _created = CourseAllocation.get_or_create_shared(
                    program_course=pc, department=dept, allocation_set=sg.allocation_set,
                    defaults={"is_elective": True, "selection_group": sg},
                )
                if ca.selection_group_id != sg.id or not ca.is_elective:
                    ca.selection_group = sg
                    ca.is_elective = True
                    ca.save()
                cas.append(ca)
                BaseSelection.objects.get_or_create(program_course=pc, department=dept,
                                                    defaults={"created_by": request.user})
            sg.courses.set(cas)
            sg.specialization_stems.add(stem)
            sg.sync_courses_into_mapped_stems()
        return JsonResponse({"status": "success",
                             "message": f"Pick-one group '{group_name}' created inside stem '{stem.name}'.",
                             "selection_group": _selection_group_map_dict(sg, stem=stem)})
    except Exception as e:
        logger.error(f"Error quick-creating elective group for stem: {str(e)}")
        return JsonResponse({"status": "error", "message": str(e)}, status=500)
