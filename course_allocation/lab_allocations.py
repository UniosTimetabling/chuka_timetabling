...
from course_allocation.models import LabAllocation, AllocationSet
...
# LabAllocation used to be scoped only by department, so a COD working in
# a brand-new AllocationSet ("Semester 2", a Special intake, etc.) would
# still see — and could bulk-affect — every lab/workshop allocation ever
# created for that department across every other concurrent set. This
# mirrors the same fix already applied to CourseAllocation: resolve which
# set the page is currently working in (explicit allocation_set_id param
# first, else the session's active set / department's legacy set), and
# scope every read/write to it.
def _resolve_active_set(request, dept):
    """
    Explicit allocation_set_id (from the page's switcher / an AJAX call)
    takes priority so the value on screen can never silently diverge from
    what gets written; otherwise falls back to the session's active set
    for this department, then its legacy set — same resolution order as
    get_department_allocations uses for the course side.
    """
    if dept is None:
        return None
    explicit_id = request.POST.get("allocation_set_id") or request.GET.get("allocation_set_id")
    if explicit_id:
        aset = AllocationSet.objects.filter(id=explicit_id, department=dept).first()
        if aset:
            return aset
    return resolve_allocation_set_for_request(request, dept)


# ── Lab-allocation queryset scoped to a department (and allocation set) ──────
def _lab_qs_for_dept(dept, allocation_set=None):
    """
    Return LabAllocations whose primary program_course belongs to a program
    in the given department, scoped to one AllocationSet (Semester 1 vs
    Semester 2, a Special allocation, etc.) — no cross-department AND no
    cross-set leakage. If allocation_set is None, returns every set's rows
    unchanged (department-only scoping), same convention as
    course_allocation.allocation_scope.scope_qs.
    """
    qs = (
    ...
    return scope_qs(qs, allocation_set)
...
def lab_allocations(request):
    ...
    active_set = _resolve_active_set(request, dept)
    ...
    if (
        ...
        if action in SR_ACTIONS:
            return handle_sr_action(
                ...
                dept_scoped_qs=lambda d: _lab_qs_for_dept(d, active_set),
        ...
        if action == "create_lab":
            ...
            if lab_id:
                # Edit: verify the allocation belongs to this dept AND this
                # allocation set before touching it — otherwise a COD
                # switched into "Semester 2" could silently overwrite a row
                # that actually belongs to "Semester 1" just because the id
                # happened to be known/guessable client-side.
                allocation = get_object_or_404(
                    LabAllocation,
                    pk=lab_id,
                    program_course__program__department=dept,
                    allocation_set=active_set,
                )
        ...
        # ── Detail (for edit form) ────────────────────────────────────────────
            ...
            lab_id = request.POST.get("id")
            # Scope to this department AND this allocation set so one COD
            # cannot peek at (or open for editing) another set's row.
            allocation = get_object_or_404(
        ...
        # ── Delete ────────────────────────────────────────────────────────────
            ...
            lab_id = request.POST.get("id")
            # Scope delete to the user's own department AND allocation set.
            allocation = get_object_or_404(
                LabAllocation,
                pk=lab_id,
                program_course__program__department=dept,
    ...
    # ── GET ───────────────────────────────────────────────────────────────────
    if dept is not None:
        lab_allocations_qs = _lab_qs_for_dept(dept, active_set)
        ...
        allocation_sets_for_switcher = AllocationSet.objects.none()
    ...
    context = {
        ...
        "active_allocation_set": active_set,
