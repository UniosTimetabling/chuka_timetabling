...
from course_allocation.lab_allocations import detect_user_department, _resolve_active_set
from course_allocation.allocation_scope import scope_qs, resolve_allocation_set_for_request
...
def get_allocation_tree(request):
    ...
    Query params:
      ...
      allocation_set_id  optional — which concurrent AllocationSet (Semester 1
                         vs Semester 2, a Special allocation, etc.) to check
                         "already allocated" status against. Without this,
                         the tree used to check LabAllocation across every
                         set for the department, so a brand-new empty set
                         would still show every course as "already
                         allocated" from some other set. Falls back to the
                         session's active set / the department's legacy set,
                         same as the manual Lab Allocations page.
    """
    ...
    allocation_set = _resolve_active_set(request, dept)
    ...
    #
    # Concurrent Allocation Sets: "already allocated" is scoped to the
    # active set too -- a course allocated only in a different set should
    # show as available here, not falsely locked/marked as done.
    primary_qs = scope_qs(
        LabAllocation.objects.filter(program_course_id__in=pc_ids), allocation_set,
    )
    primary = {
        a.program_course_id: a
        for a in primary_qs.select_related("lecturer")
    }
    secondary_ids = set(
        scope_qs(
            LabAllocation.objects.filter(additional_courses__id__in=pc_ids), allocation_set,
    ...
    return JsonResponse({
        ...
        "allocation_set_name": allocation_set.name if allocation_set else None,
...
def run_allocation(request):
    ...
    POST body (JSON):
      ...
      department_id       : required for admins.
      allocation_set_id   : optional -- which concurrent AllocationSet
                            (Semester 1 vs Semester 2, a Special allocation,
                            etc.) to create the new LabAllocation rows in,
                            and to check "already allocated" against.
                            Without this, a run used to check across every
                            set for the department and could tag a new row
                            with no set at all, so a brand-new empty set
                            looked pre-populated with another set's labs.
                            Falls back to the session's active set / the
                            department's legacy set if omitted.
      ...
      overwrite            : bool, default False. If False, courses that
                            ...
                            course (within the same allocation set) are
                            skipped. If True, that allocation is deleted and
                            re-created. Courses that only appear as a
                            *secondary* course of someone else's shared lab
                            slot are always skipped (see module docstring).
    """
    ok, err, dept, is_admin = _resolve_scope(request)
    if not ok:
        return err
    ...
    # Concurrent Allocation Sets: an explicit allocation_set_id from the
    # request body (the page's switcher) wins; otherwise fall back to the
    # session's active set / the department's legacy set, same resolution
    # order as the manual Lab Allocations page and the course-allocation side.
    allocation_set_id = data.get("allocation_set_id")
    ...
    primary_ids = set(
        scope_qs(LabAllocation.objects.filter(program_course_id__in=pc_ids), allocation_set)
        .values_list("program_course_id", flat=True)
    )
    secondary_ids = set(
        scope_qs(LabAllocation.objects.filter(additional_courses__id__in=pc_ids), allocation_set)
        .values_list("additional_courses__id", flat=True)
    ) - primary_ids
    ...
    with transaction.atomic():
        if overwrite:
            scope_qs(
                LabAllocation.objects.filter(program_course_id__in=[pc.id for pc in pcs]),
                allocation_set,
            ).delete()
