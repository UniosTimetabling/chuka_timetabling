...
run_pre_deploy_backup() {
  ...
  local db_name db_user db_pass db_root_pass db_ok=false
  ...
  db_root_pass=$(get_env_var DB_ROOT_PASSWORD "$PROJECT_ROOT/.env")

  # mysqldump runs *inside* the db container over the local socket, so it
  # authenticates as user@localhost. For DB_USER=root that is the real root
  # account, whose password is MYSQL_ROOT_PASSWORD (DB_ROOT_PASSWORD) — not
  # DB_PASSWORD. Using DB_PASSWORD there is what produced
  #   "Access denied for user 'root'@'localhost' (using password: YES)"
  # and silently left the deploy with no restore point.
  if [[ "$db_user" == "root" && -n "$db_root_pass" ]]; then
  ...
  if [[ "$db_ok" == "true" || "$media_ok" == "true" ]]; then
    ...
    # A totally empty snapshot (both dump and archive failed, e.g. the
    # mysqldump-interrupted case above) isn't a restore point at all — don't
    # let it eat one of the retained slots below, just drop it now.
    warn "Pre-deploy backup did NOT produce anything usable — removing the empty snapshot folder and proceeding anyway (no fresh restore point for this deploy)."
    rm -rf "$backup_dir"
  fi

  # ---- Prune old snapshots ---------------------------------------------------
  # Retention is configurable (DEPLOY_BACKUP_RETENTION) because a full
  # DB dump + media tar per snapshot adds up fast on a box that gets
  # redeployed often — keeping 14 of those can by itself account for
  # most of a disk filling up, especially across repeated retries while
  # chasing down a build failure (each retry re-runs this step). Default
  # is lowered to 7; override per-box if more history is actually needed.
  local keep="${DEPLOY_BACKUP_RETENTION:-7}" total
