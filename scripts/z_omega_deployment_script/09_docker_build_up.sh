...
docker_build_and_up() {
  ...
  # ---- Prune old history tags + dangling layers ---------------------------
  # Every successful deploy adds a brand-new "${IMAGE_NAME}:<commit>_<ts>"
  # tag above (for history/rollback), on top of ":latest" and ":previous".
  # Nothing ever removed the old ones, so on a box that gets redeployed
  # often (or that sits through a string of failed builds while a bug like
  # the collectstatic crash above gets fixed) this is the single biggest
  # source of runaway disk usage — much bigger than the pruned backups/
  # folder, since every retry also leaves failed-build layers and dangling
  # "<none>" images behind. Keep the most recent N history tags, and always
  # clean up dangling images + the build cache.
  local keep_images="${DEPLOY_IMAGE_RETENTION:-5}"
  local old_tags
  old_tags=$(docker images "${IMAGE_NAME}" --format '{{.Tag}}' \
               | grep -Ev '^(latest|previous)$' \
               | sort -r | tail -n "+$((keep_images + 1))" || true)
  if [[ -n "$old_tags" ]]; then
    info "Pruning old ${IMAGE_NAME} history tags (keeping the most recent ${keep_images})..."
    while IFS= read -r t; do
      [[ -n "$t" ]] && docker rmi "${IMAGE_NAME}:${t}" >/dev/null 2>&1 || true
    done <<< "$old_tags"
  fi
