    ...
    class Meta:
        ...
        constraints = [
            models.CheckConstraint(
                # `check=` (not `condition=`) — Django only renamed this
                # kwarg to `condition=` in 5.1 (and removes `check=` in 6.0).
                # This project is pinned to Django>=4.2,<5.0 (requirements.txt),
                # and 4.2 does not accept `condition=` at all, which is what
                # was crashing collectstatic/migrate with:
                #   TypeError: CheckConstraint.__init__() got an unexpected
                #   keyword argument 'condition'
                # If/when this project upgrades to Django>=5.1, switch this
                # back to `condition=` (and drop `check=`) before Django 6.0.
                check=(
