...
@transaction.atomic
def _commit_records(records: list[dict], request=None) -> dict:
    """
    ...
    Concurrent Allocation Sets: `request` is optional (kept optional so any
    non-HTTP/background caller of this function still works) and is used
    only to resolve which AllocationSet a newly-created CourseAllocation
    row should be tagged with — the COD's active/session set for that
    department if one is chosen, else the department's legacy set. This
    import path never moves or edits an ALREADY-EXISTING allocation's set
    (update_or_create's `defaults` deliberately excludes allocation_set),
    so re-running an import never reshuffles data between sets.
    """
            ...
            # ── CourseAllocation ─────────────────────────────────────────────
                ...
                obj, was_created = CourseAllocation.objects.update_or_create(
                    ...
                    defaults={
                        ...
                        # NOTE: allocation_set is deliberately NOT in `defaults`
                        # — update_or_create would otherwise re-tag an
                        # EXISTING allocation to whatever set is active this
                        # time the import runs. It's set explicitly below,
                        # once, only when the row was just created.
                    },
                )
                if was_created:
                    obj.allocation_set = (
                        get_active_allocation_set(request, dept) if request else None
                    ) or get_or_default_legacy_set(dept)
                    obj.save(update_fields=["allocation_set"])
...
def smart_import_v2_commit(request):
    ...
    try:
        result = _commit_records(records, request=request)
    except Exception as e:
