...
def export_main_pdf(request):
    ...
    Concurrent Allocation Sets: scoped the same way as
    export_import.official_timetables.export_main_pdf and
    timetable.main_timetable_view — optional ?allocation_set_id= (validated,
    aborts with a clear error if unknown), else whatever's active for this
    session on /timetable/dashboard/, else the same "eligible" fallback
    used everywhere else. Applied identically to the main, merged, and lab
    querysets so this PDF can't mix rows from two different sets.
    """
    ...
    # --- Querysets (scoped to the active allocation set(s), see docstring) ---
    ...
    ).order_by("venue", "start_time", "end_time", "day")

    # ✅ Only published merged groups, in scope
    merged_qs = AutoMergedExamGroup.objects.select_related(
        "venue", "base_course__lecturer"
    ).prefetch_related("merged_courses").filter(published=True).filter(
        tt_scope_q(pdf_scope, prefix="base_course__allocation_set")
    ).order_by("venue", "start_time", "end_time", "date")
