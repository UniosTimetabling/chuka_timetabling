...
def export_main_exam_pdf(request):
    ...
    try:
        ...
        # ---------- MAIN EXAMS ----------
        # Scoped to the AllocationSet(s) currently ticked on
        # /timetable/dashboard/ — otherwise this "official" PDF (and the
        # publish button that produces it) pulls every department's/every
        # set's published exams into one document regardless of which
        # set the TT is actually publishing for.
        exams = apply_tt_scope(
            ExamTimetable.objects.select_related("course_allocation__lecturer", "venue"),
            request=request, prefix="course_allocation__allocation_set",
        ...
        lab_exams = apply_tt_scope(
            ...
            request=request, prefix="lab_allocation__allocation_set",
