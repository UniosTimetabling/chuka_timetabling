...
def get_academic_year_and_semester(allocation_set=None):
    # Concurrent Allocation Sets: if the PDF is being published for one
    # specific AllocationSet, the label comes straight from that set — no
    # more guessing from a global majority vote across every allocation in
    # the database, which breaks the moment two different semesters' sets
    # are open at once. Falls back to the old whole-DB heuristic only when
    # no allocation_set is given (existing callers, unchanged behaviour).
...
def publish_exam_timetable_pdf(request):
    ...
    try:
        ...
        # Concurrent Allocation Sets: optional ?allocation_set_id= scopes
        # this exam-timetable publish to one specific set, same as the
        # regular timetable publish above.
        allocation_set = None
        ...
        if allocation_set is not None:
            exams = exams.filter(course_allocation__allocation_set=allocation_set)
        lab_exams = (
...
def publish_regular_timetable_pdf(request):
    ...
    try:
        ...
        # Concurrent Allocation Sets: an optional ?allocation_set_id= scopes
        # this publish run to one specific set (one department's Semester 1,
        # a mixed set, a Special allocation, ...) instead of the whole
        # university's timetable, and the header's academic year/semester
        # text is taken from that set instead of guessed.
        allocation_set = None
        allocation_set_id = request.GET.get("allocation_set_id") or request.POST.get("allocation_set_id")
        if allocation_set_id:
            from course_allocation.models import AllocationSet as _AllocationSet
        ...
        if allocation_set is not None:
            timetables = timetables.filter(course_allocation__allocation_set=allocation_set)
