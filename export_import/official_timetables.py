...
def export_main_pdf(request):
    ...
    try:
        ...
        ).order_by("venue__code", "start_time", "end_time", "day")

        # Concurrent Allocation Sets
        # --------------------------
        # This export used to be unconditionally whole-university (or, with
        # the old `?allocation_set_id=` param, applied ONLY to the main
        # timetable_qs — merged_qs and lab_qs were left unscoped, so a
        # "scoped" PDF could still mix in another set's merged/lab rows).
        #
        # Resolution order, matching every other publish/PDF entry point:
        #   1. explicit ?allocation_set_id= — validated before anything is
        #      queried; an unknown/invalid id aborts with a clear error
        #      instead of silently rendering an empty or wrong PDF.
        #   2. otherwise, whatever AllocationSet(s) are active for this
        #      session on /timetable/dashboard/.
        #   3. otherwise, the same "eligible" fallback (no set, legacy, or
        #      submitted-to-TT) the rest of the system already uses — a
        #      department that has never adopted concurrent sets gets
        #      exactly the same PDF it always got.
        # The SAME resolved scope is applied to timetable_qs, merged_qs and
        # lab_qs, so the PDF can never show one set's lectures next to a
        # different set's merged/lab sessions.
        allocation_set_id = request.GET.get("allocation_set_id")
        if allocation_set_id:
            resolution = resolve_single_tt_allocation_set(request, allocation_set_id=allocation_set_id)
            if not resolution.ok:
                return HttpResponse(
                    f"Cannot generate PDF: {resolution.error}",
            ...
            pdf_scope = resolve_tt_scope(request)

        timetable_qs = timetable_qs.filter(
            tt_scope_q(pdf_scope, prefix="course_allocation__allocation_set")
        )
        ...
        merged_qs = MergedCourseGroupTimetable.objects.select_related(
            ...
            tt_scope_q(pdf_scope, prefix="base_course__allocation_set")
        ).only(
        ...
        lab_qs = LabTimetable.objects.select_related(
            ...
            tt_scope_q(pdf_scope, prefix="lab_allocation__allocation_set")
        ).only(
