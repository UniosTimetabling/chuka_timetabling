...
def export_main_pdf(request):
    ...
    try:
        ...
        # Concurrent Allocation Sets: optional ?allocation_set_id= scopes this
        # export to one set instead of the whole university's timetable.
        allocation_set_id = request.GET.get("allocation_set_id")
        if allocation_set_id:
            timetable_qs = timetable_qs.filter(course_allocation__allocation_set_id=allocation_set_id)
